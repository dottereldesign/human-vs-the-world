...
`,tt=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Ze(e.streamUrl);return`
  ...
  `},nt=e=>e.liquipediaUrl?`
    <a href="${N(e.liquipediaUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${N(e.name)} on Liquipedia">
      ${et({key:`liquipedia`,short:`LP`,icon:ee},`Liquipedia`)}
    </a>
  `:`<span class="is-disabled"><span class="service-icon service-icon-liquipedia" aria-hidden="true"><img src="${ee}" alt="" loading="lazy" /></span><span class="action-label">Liquipedia</span></span>`,rt=e=>e.realName?`<p class="player-card-real-name">${N(e.realName)}</p>`:``,it=e=>{let t={chaemiko:i,fortitude:c,hawk:l,infi:d,leon:f,sok:te}[e.slug];return t?`<img class="player-card-image" src="${t}" alt="${N(e.name)}" loading="lazy" />`:`<span>${N(e.flag)}</span>`},at=e=>e.slug===`infi`?`
  ...
  `:``,H=()=>m?F().flatMap(e=>Ne(e.slug).replays.filter(e=>je(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],ot=()=>[...new Set(H().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),U=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,st=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,ct=e=>{let t=st(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},lt=e=>{let t=ct(e);return t.length<2?``:`${U(t[0].race)} vs ${U(t[1].race)}`},ut=()=>[...new Set(H().map(lt).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),dt=e=>I(e.sourceSlug,e.id),W=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],ft=e=>U(e?.raceDetected||e?.race)===`Human`,G=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},pt=(e,t=[])=>{t.forEach(t=>G(e,t.id,Number(t.count)||0))},K=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||L(e[0]).localeCompare(L(t[0]))).slice(0,t),mt=()=>{let e=new Map(W.map(e=>[e,0])),t=new Map(W.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,te=0,ne=0;return H().forEach(p=>{let re=I(p.sourceSlug,p.id);if(re?.status!==`parsed`)return;u+=1;let ie=(re.players||[]).filter(ft);ie.length&&(G(s,p.map||re.map?.name||`Unknown map`),G(c,lt(p)||`Unknown matchup`),ie.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{W.includes(t.id)&&G(e,t.id)}),W.includes(c[0]?.id)&&G(t,c[0].id),(s.order?.items||[]).forEach(e=>{ze(e.id)?(G(n,e.id),te+=M[e.id]||0):(G(r,e.id),M[e.id]!==null&&(ne+=1))}),pt(i,s.topUnits),pt(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>G(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:te,unknownBoughtPrices:ne}},ht=e=>(dt(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),gt=()=>{let e=new Map;return H().forEach(t=>{ht(t).forEach(t=>{e.has(t)||e.set(t,Le(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},_t=e=>ct(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),vt=e=>{let i=String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:t,human:t,o:r,orc:r,u:p,undead:p,n,nightelf:n};return a[i]?a[i]:null},yt=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>U(t.race||t.raceDetected)===U(e.race));return i.length===1?i[0]:n[t]||null},bt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${N(t)}</span>
      <strong>${N(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Xe(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},q=(e,t=[])=>ct(e).map((e,n)=>{let r=vt(e.race),i=yt(e,n,t);return`
...
`,xt=(e,t)=>!t||t.status!==`parsed`?`
  ...
  `},Y=(e,t)=>t.flatMap(([t,n])=>(n||[]).map(n=>({...n,type:t,playerName:e.name,race:e.raceDetected||e.race}))).sort((e,t)=>(e.ms||0)-(t.ms||0)),Ct=(e,t=`${e.type}`)=>`
  <div class="build-order-row">
    <span>${N(e.time||`0:00`)}</span>
    <div class="timeline-object">
      ${R(e.id)}
      <strong>${N(L(e.id))}</strong>
    </div>
    <em>${N(t)}</em>
  </div>
`,wt=(e,t)=>`
...
`,Tt=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=Ct,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
  ...
  `:`<div class="analysis-empty"><strong>${N(t)} unavailable</strong><span>${N(n)}</span></div>`},Et=e=>Tt({analysis:e,title:`Build order`,emptyMessage:`Run the updated replay analyzer to generate unit and building timeline data.`,getItems:e=>Y(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units]])}),Dt=e=>Tt({analysis:e,title:`Upgrades`,emptyMessage:`No upgrades were parsed for this replay.`,getItems:e=>He(Y(e,[[`Upgrade`,e.order?.upgrades]])),renderRow:e=>Ct(e,Number.isFinite(e.goldCost)?Be(e.goldCost):`Cost unknown`),renderSummary:(e,t)=>{let n=Ue(t,e=>e.goldCost);return We({label:`Known gold spent`,gold:n.gold,unknown:n.unknown})}}),Ot=e=>Tt({analysis:e,title:`Items bought`,emptyMessage:`No shop-price items were parsed for this replay.`,getItems:e=>Y(e,[[`Item`,e.order?.items]]).filter(e=>ze(e.id)),renderSummary:(e,t)=>{let n=Ue(t,e=>M[e.id]);return We({label:`Total spent`,gold:n.gold,unknown:n.unknown})},renderRow:t=>{let n=Ke(t.id,e);return Ct(t,n?`${Re(t.id)} · ${qe(n.chance)} ${n.groupLabel} drop`:Re(t.id))}}),kt=e=>Tt({analysis:e,title:`Items found`,emptyMessage:`No no-shop-price items were parsed for this replay.`,getItems:e=>Y(e,[[`Item`,e.order?.items]]).filter(e=>!ze(e.id)),renderRow:t=>wt(t,e)}),At=e=>{let t=e?.chat||[];return t.length?`
  ...
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},jt=(e=[])=>e.length?e.map(e=>`<span>${N(L(e.id))} × ${N(e.count)}</span>`).join(``):`<span>None parsed</span>`,Mt=e=>{let t=e?.players||[];return t.length?`
    <div class="actions-grid">
      ${t.map(e=>`
            <div class="actions-card">
              <strong>${N(e.name||`Unknown`)}</strong>
              <div class="action-counts">
                ${Object.entries(e.actions||{}).map(([e,t])=>`<span><em>${N(e)}</em>${N(z(t||0))}</span>`).join(``)}
              </div>
  ...
  `:`<div class="analysis-empty"><strong>No action data</strong><span>Run the updated replay analyzer to generate action data.</span></div>`},Nt=(e,t)=>w===`apm`?St(t):w===`build`?Et(t):w===`upgrades`?Dt(t):w===`items-bought`?Ot(t):w===`items-found`?kt(t):w===`chat`?At(t):w===`actions`?Mt(t):xt(e,t),Pt=()=>{let e=Fe(y);if(!e)return``;let t=I(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=xe.some(e=>e.id===w)?w:`overview`;return`
  ...
  `},X=(e=0)=>{let t=Math.max(0,Math.floor(Number(e||0)/1e3)),n=Math.floor(t/60),r=t%60;return`${n}:${String(r).padStart(2,`0`)}`},Ft=e=>{if(!e||e.status!==`parsed`)return[];let t=(e.players||[]).flatMap(e=>[...(e.heroes||[]).map(t=>({id:t.id,ms:0,time:`0:00`,type:`Hero`,playerName:e.name,race:e.raceDetected||e.race,detail:t.level?`Level ${t.level}`:`Hero`})),...Y(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units],[`Upgrade`,He(e.order?.upgrades||[])],[`Item`,e.order?.items]]).map(t=>({...t,playerName:e.name,detail:t.type===`Item`?Re(t.id):t.type===`Upgrade`&&Number.isFinite(t.goldCost)?Be(t.goldCost):t.type}))]),n=(e.chat||[]).map(e=>({id:`chat`,ms:e.timeMS||0,time:e.time||X(e.timeMS||0),type:`Chat`,playerName:e.playerName||`Observer`,detail:e.message||``}));return[...t,...n].sort((e,t)=>(e.ms||0)-(t.ms||0))},It=()=>{b=null,x=!1,S=0,C&&=(clearInterval(C),null),$()},Lt=e=>{b=e,S=0,x=!1,C&&=(clearInterval(C),null),$()},Rt=()=>{let e=Fe(b),t=(e?I(e.sourceSlug,e.id):null)?.duration||0;if(x=!x,C&&=(clearInterval(C),null),!x){$();return}C=setInterval(()=>{S+=1e3,t&&S>=t&&(S=t,x=!1,clearInterval(C),C=null),$()},1e3),$()},zt=()=>{let e=Fe(b);if(!e)return``;let t=I(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=Ft(t),i=t?.duration||e.duration||Math.max(...r.map(e=>e.ms||0),0),a=i?Math.min(100,Math.max(0,S/i*100)):0,o=r.filter(e=>(e.ms||0)<=S).slice(-18),s=r.filter(e=>(e.ms||0)>S).slice(0,18);return`
      ...
      <section class="replay-theater" role="dialog" aria-modal="true" aria-label="Replay theater">
        ...
        <div class="replay-theater-body">
          <section class="replay-theater-map">
            ${e.mapImage?.localPath?`<img src="${N(P(e.mapImage.localPath))}" alt="${N(e.map)} map thumbnail" />`:`<div class="map-placeholder" aria-hidden="true">${N(e.mapShort||`WC3`)}</div>`}
            <div class="replay-theater-map-overlay">
              <span>${N(t?.version?`Patch ${t.version}`:e.version||`Patch unknown`)}</span>
              <strong>${N(t?.durationLabel||e.durationLabel||X(i))}</strong>
          ...
          <section class="replay-theater-controls">
            <div class="replay-theater-clock">
              <strong>${N(X(S))}</strong>
              <span>${N(t?.durationLabel||X(i))}</span>
            </div>
            ...
            <div class="replay-theater-buttons">
              <button type="button" data-theater-play>${x?`Pause`:`Play parsed timeline`}</button>
  ...
  <div class="replay-theater-event">
    <span>${N(e.time||X(e.ms||0))}</span>
    <div>
      <strong>${N(e.type===`Chat`?e.playerName:L(e.id))}</strong>
      <small>${N(e.type===`Chat`?e.detail:`${e.playerName||`Player`} · ${e.detail||e.type}`)}</small>
    </div>
  </div>
`,Vt=e=>pe?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,_t(e)].filter(Boolean).join(` `).toLowerCase().includes(pe.toLowerCase()):!0,Ht=e=>{let t=g===`all`||e.sourceSlug===g,n=me===`all`||e.map===me,r=he===`all`||ht(e).includes(he),i=_===`all`||lt(e)===_;return t&&n&&r&&i&&Vt(e)},Ut=()=>H().filter(Ht).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Wt=e=>{v=Math.min(Math.max(v,1),Math.max(e,1))},Gt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Kt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(v-1)*j+1,r=Math.min(v*j,e),i=Gt(v,t),a=0;return`
    <nav class="replay-pagination" aria-label="Replay pagination">
      <span class="pagination-summary">${n}-${r} of ${e}</span>
      <div class="pagination-controls">
        <button class="pagination-button" type="button" data-page="${v-1}" ${v===1?`disabled`:``}>Prev</button>
        ${i.map(e=>{let t=a&&e-a>1?`<span class="pagination-gap">...</span>`:``;return a=e,`${t}<button class="pagination-button ${e===v?`is-active`:``}" type="button" data-page="${e}" ${e===v?`aria-current="page"`:``}>${e}</button>`}).join(``)}
        <button class="pagination-button" type="button" data-page="${v+1}" ${v===t?`disabled`:``}>Next</button>
      </div>
    </nav>
  `},qt=e=>le?`
    ...
    `:m?e.length?e.map(e=>{let t=e.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`,n=I(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
        <article class="replay-row" data-replay-detail="${N(`${e.sourceSlug}:${e.id}`)}" role="button" tabindex="0" aria-label="Analyze ${N(e.map)} replay">
          <div class="replay-map">
            ${e.mapImage?.localPath?`<img src="${N(P(e.mapImage.localPath))}" alt="${N(e.map)} map thumbnail" loading="lazy" />`:`<div class="map-placeholder" aria-hidden="true">${N(e.mapShort||`WC3`)}</div>`}
            <div>
              <span>${N(t)}</span>
    ...
    `;let e=Ut(),t=Math.ceil(e.length/j);Wt(t);let n=(v-1)*j;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${qt(e.slice(n,n+j))}
    </section>
...
`,en=({id:e,label:t,value:n,options:r})=>{let i=r.find(e=>e.value===n)||r[0];return`
    <div class="filter-control dropdown-filter" data-filter="${N(e)}">
      ...
      <button class="filter-dropdown-button" id="${N(e)}" type="button" aria-haspopup="listbox" aria-expanded="false" aria-labelledby="${N(e)}-label ${N(e)}">
        <span>${i.iconId?R(i.iconId,i.label):``}${N(i.label)}</span>
      ...
      <div class="filter-dropdown-menu" role="listbox" aria-labelledby="${N(e)}-label">
        ${r.map(e=>`
              <button class="filter-dropdown-option ${e.value===n?`is-selected`:``}" type="button" role="option" aria-selected="${e.value===n}" data-value="${N(e.value)}">
                <span>${e.iconId?R(e.iconId,e.label):``}${N(e.label)}</span>
              </button>
            `).join(``)}
      </div>
    </div>
  ...
  <section class="filters" aria-label="Replay filters">
    ...
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${en({id:`map-filter`,label:`Map`,value:me,options:[{value:`all`,label:`All maps`},...ot().map(e=>({value:e,label:e}))]})}
        ${en({id:`hero-filter`,label:`Starting hero`,value:he,options:[{value:`all`,label:`All heroes`},...gt()]})}
        ${en({id:`matchup-filter`,label:`Matchup`,value:_,options:[{value:`all`,label:`All matchups`},...ut().map(e=>({value:e,label:e}))]})}
      </div>
    </div>
  </section>
`,nn=e=>{if(!e.streamUrl)return`<span class="ranking-stream is-empty" aria-hidden="true"></span>`;let t=Ze(e.streamUrl);return`
  ...
  <aside class="rankings-sidebar" aria-label="Human rankings">
    ...
    <div class="rankings-list">
      ${F().map(e=>`
            <div class="ranking-row ${g===e.slug?`is-selected`:``}" data-player-filter="${N(e.slug)}" role="button" tabindex="0" aria-pressed="${g===e.slug}">
...
`,an=e=>e?`
  ...
  <section class="player-card-region ${_e?``:`with-card-intro`}" aria-label="Top Human player cards">
    <div class="player-card-strip">
      ${F().map((e,t)=>{let n=[`is-gold`,`is-silver`,`is-bronze`][t]||``,r=t*760,i=t<3?r+520:0,a=Pe(e);return`
...
`,ln=()=>{let e=H().find(e=>e.mapImage?.localPath&&I(e.sourceSlug,e.id)?.status===`parsed`)||H()[0],t=e?I(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`;return`
    <section class="renderer-page" aria-label="Renderer test">
      ...
      <div class="renderer-layout">
        <section class="renderer-stage" aria-label="Renderer preview">
          ${e?.mapImage?.localPath?`<img src="${N(P(e.mapImage.localPath))}" alt="${N(e.map)} map thumbnail" loading="lazy" />`:`<div class="renderer-placeholder">WC3</div>`}
  ...
  `},un=()=>{try{let e=JSON.parse(localStorage.getItem(`w3AssetsIndex`)||`[]`),t=Array.isArray(e)?e.length:0,n=Array.isArray(e)?e.filter(e=>String(e?.p||``).toLowerCase().endsWith(`.mpq`)).length:0,r=Array.isArray(e)?e.filter(e=>/\.(w3x|w3m)$/i.test(String(e?.p||``))).length:0,i=Array.isArray(e)?e.reduce((e,t)=>e+Number(t?.s||0),0):0,a=localStorage.getItem(`wc3-warsmash-staged-edition`)||`legacy`;return{ready:localStorage.getItem(`w3AssetsReady`)===`1`&&t>0&&a===O,edition:a,fileCount:t,mpqCount:n,mapCount:r,totalMb:(i/1048576).toFixed(1)}}catch{return{ready:!1,edition:`legacy`,fileCount:0,mpqCount:0,mapCount:0,totalMb:`0.0`}}},dn=()=>ge===`windows`?{platform:`Windows`,primary:`C:\\Program Files\\Warcraft III`,secondary:`C:\\Program Files (x86)\\Warcraft III`,pickerHint:`In the folder picker, paste the path into the address bar if it does not open there automatically.`}:{platform:`macOS`,primary:`/Applications/Warcraft III (Legacy)`,secondary:`/Applications/Warcraft III`,pickerHint:`In the folder picker, press Cmd+Shift+G, paste the path, then choose the Warcraft III folder.`},Z={reforged:`2.0.4.23745`,legacy:`1.29.2.9232`},fn=[{id:`legacy`,label:`Legacy MPQ`,build:Z.legacy,status:`Playable now`,note:`Current Warsmash web path mounts classic MPQs and loose maps from this install.`},{id:`reforged`,label:`Reforged`,build:Z.reforged,status:`Experimental CASC test`,note:`Latest Reforged uses CASC assets. This app now has an experimental browser-worker CASC mount path backed by the private sandbox.`}],pn=async(e,t)=>{let n=e;for(let e of t)n=await n.getDirectoryHandle(e,{create:!0});return n},mn=async()=>{let e=await navigator.storage.getDirectory();for(let t of[`w3`,`extracted`])try{await e.removeEntry(t,{recursive:!0})}catch{}},hn=async(e=O)=>{if(!A.running){A={running:!0,done:0,total:0,bytes:0,totalBytes:0,currentName:`Reading local install...`,error:``},$();try{let t=await fetch(`/wc3-local-assets/manifest?edition=${encodeURIComponent(e)}`,{cache:`no-store`});if(!t.ok){let e=await t.json().catch(()=>({}));throw Error(e.error||`No local MPQ-style Warcraft III install was detected.`)}let n=await t.json(),r=Array.isArray(n.files)?n.files:[];if(!r.length)throw Error(`Detected install did not include MPQ/map files Warsmash can use.`);A={running:!0,done:0,total:r.length,bytes:0,totalBytes:Number(n.totalBytes||0),currentName:`Preparing ${n.root||`local install`}`,error:``},$(),await mn();let i=await(await navigator.storage.getDirectory()).getDirectoryHandle(`w3`,{create:!0}),a=[],o=0,s=0;for(let t of r){let c=String(t.path||``);if(!c)continue;let l=await fetch(`/wc3-local-assets/file?edition=${encodeURIComponent(e)}&path=${encodeURIComponent(c)}`);if(!l.ok)throw Error(`Failed to read ${c}`);let u=await l.blob(),d=c.split(`/`).filter(Boolean),f=await(await(await pn(i,d.slice(0,-1))).getFileHandle(d[d.length-1],{create:!0})).createWritable();await f.write(u),await f.close(),o+=1,s+=u.size,a.push({p:c,s:u.size}),A={running:!0,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:c,error:``},(o===r.length||o%3==0)&&$()}localStorage.setItem(`w3AssetsReady`,`1`),localStorage.setItem(`w3AssetsCount`,String(a.length)),localStorage.setItem(`w3AssetsIndex`,JSON.stringify(a)),localStorage.setItem(`w3AssetsSource`,n.root||`detected local install`),localStorage.setItem(`wc3-warsmash-staged-edition`,e),localStorage.setItem(`wc3-warsmash-selected-edition`,e),A={running:!1,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:`Ready`,error:``},$()}catch(e){A={...A,running:!1,error:e instanceof Error?e.message:String(e)},$()}}},gn=async()=>{if(!k.preparing){k={...k,preparing:!0,error:``},$();try{let e=await fetch(`/wc3-reforged-sandbox/prepare`,{cache:`no-store`}),t=await e.json().catch(()=>({}));if(!e.ok)throw Error(t.error||`Failed to prepare Reforged sandbox.`);k={checked:!0,preparing:!1,ready:!0,build:t.build||``,fileCount:Number(t.totals?.configFiles||0)+Number(t.totals?.indexFiles||0)+Number(t.totals?.dataFiles||0),totalGb:(Number(t.totals?.bytes||0)/1073741824).toFixed(2),error:``}}catch(e){k={...k,checked:!0,preparing:!1,ready:!1,error:e instanceof Error?e.message:String(e)}}$()}},_n=()=>{let e=H().find(e=>String(e.id)===`137397`)||H().find(e=>I(e.sourceSlug,e.id)?.status===`parsed`)||H()[0],t=e?I(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`,i=e?`${e.sourceSlug}:${e.id}`:``,a=`/play/?menu=1&assets=${encodeURIComponent(O)}&replay=${encodeURIComponent(i)}&autostart=1`,o=`/assets/`,s=un(),c=dn(),l=fn.find(e=>e.id===O)||fn[0],u=l.id===`legacy`,d=u?s.ready:k.ready,f=()=>O===`reforged`?`
      <div class="wc3-reforged-sandbox ${k.ready?`is-ready`:``} ${k.error?`is-error`:``}">
        <div>
          <span>Private Reforged sandbox</span>
          <strong>${k.ready?`Ready · ${N(k.build||Z.reforged)}`:`Not prepared yet`}</strong>
          <small>${k.ready?`${N(z(k.fileCount))} CASC files indexed, ${N(k.totalGb)} GB referenced read-only.`:`Creates .wc3-assets/reforged/manifest.json and indexes your Reforged CASC data without modifying the install.`}</small>
          ${k.error?`<small>${N(k.error)}</small>`:``}
        </div>
        <button type="button" data-reforged-prepare ${k.preparing?`disabled`:``}>
          ${k.preparing?`Preparing...`:k.ready?`Refresh Sandbox`:`Prepare Reforged Sandbox`}
        </button>
      </div>
    `:``,ee=A.totalBytes?Math.min(100,A.bytes/A.totalBytes*100):0,te=A.running?`${A.done}/${A.total} ${(A.bytes/1048576).toFixed(1)} / ${(A.totalBytes/1048576).toFixed(1)} MB`:``;return`
      ...
      <div class="wc3-viewer-offline">
        ...
        <div class="wc3-viewer-offline-actions">
          ${d?`
              ...
              `:`
                <button class="is-primary" type="button" data-warsmash-auto-stage ${A.running||!u?`disabled`:``}>
                  ${A.running?`Staging Detected Install...`:`Auto Stage Detected Install`}
                </button>
        ...
        ${f()}
        ${A.running||A.error?`
              <div class="wc3-auto-stage-status ${A.error?`is-error`:``}">
                ${A.running?`<div class="wc3-auto-stage-bar"><span style="width: ${ee.toFixed(1)}%;"></span></div><strong>${N(te)}</strong><small>${N(A.currentName)}</small>`:`<strong>Auto staging failed</strong><small>${N(A.error)}</small>`}
              </div>
            `:``}
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                ...
                <dl>
                  ...
                  <div><dt>Assets status</dt><dd>${d?u?`Ready: ${s.fileCount} files staged`:`Sandbox ready: ${k.fileCount} CASC files`:u?`Setup required`:`Prepare Reforged sandbox`}</dd></div>
                ...
                <div class="renderer-actions">
                  ...
                  ${d?`<button class="warsmash-start-button" type="button" data-warsmash-watch="${N(i)}">${V()}<span>${u?`Start Engine Test`:`Start Reforged Engine Test`}</span></button>`:`<button type="button" data-warsmash-auto-stage ${A.running||!u?`disabled`:``}>${A.running?`Staging Detected Install...`:u?`Auto Stage Detected Install`:`Reforged Support Pending`}</button>`}
...
`,vn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${R(e)}${N(Ie(L(e)).replace(` (${e})`,``))}</span>
              <strong>${N(z(t))}</strong>
            </div>
          ...
          `).join(``):`<div class="analysis-empty"><strong>${N(t)}</strong></div>`,bn=()=>{let e=mt(),t=e.humanGames||1,n=K(e.boughtItems,10),r=K(e.foundItems,8);return`
    <section class="statistics-page" aria-label="Replay statistics">
      ...
      <div class="statistics-metrics" aria-label="Replay data summary">
        ${Q(`Parsed replays`,z(e.parsedReplays),`Allowed map pool only`)}
        ${Q(`Human player-games`,z(e.humanGames),`One Human player in most rows`)}
        ${Q(`Average Human APM`,z(e.averageApm),`Parsed APM samples`)}
        ${Q(`Item actions`,z(e.totalItemActions),`Buys, uses, and item commands`)}
      </div>
      ...
      <section class="statistics-panel statistics-hero-panel">
        <div class="statistics-panel-header">
          ...
          <span>${N(z(e.humanGames))} Human games</span>
        </div>
        <div class="statistics-hero-grid">
          ${W.map(n=>{let r=e.heroPicks.get(n)||0,i=e.firstHeroPicks.get(n)||0,a=e.humanGames?r/t*100:0,o=e.humanGames?i/t*100:0;return`
                <article class="statistics-hero-card">
                  ...
                  <dl>
                    <div><dt>Picked</dt><dd>${N(z(r))}</dd></div>
                    <div><dt>Pick rate</dt><dd>${N(qe(a))}</dd></div>
                    <div><dt>First hero</dt><dd>${N(z(i))}</dd></div>
      ...
      <div class="statistics-grid">
        ...
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${N(z([...e.foundItems.values()].reduce((e,t)=>e+t,0)))}</span>
          </div>
          <div class="statistics-list">
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(K(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(K(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(K(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${yn(K(e.mapCounts,8),`No map stats found`)}
          </div>
        </section>
...
const result = await parser.parse("replay.w3g");</code></pre>
      ...
      <div class="statistics-metrics w3gjs-metrics" aria-label="w3gjs parsed data summary">
        ${Q(`Replay files`,z(e.totalReplays),`From the local replay cache`)}
        ${Q(`Parsed replays`,z(e.parsedReplays),`Built from w3gjs output`)}
        ${Q(`Parsed players`,z(e.parsedPlayers),`Names, races, teams, APM`)}
        ${Q(`Timeline events`,z(e.eventCount),`Builds, units, items, heroes, chat`)}
      </div>
      ...
      <div class="w3gjs-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${N(z(e.averageApm))} avg APM</span>
          </div>
          ${St({...n,players:r})}
        </section>
...
`,kn=()=>{let e=h?.players||[];return`
    <section class="money-page" aria-label="Player earnings">
      ...
      <p class="money-note">${N(h?.scope||``)}</p>
    </section>
  `},An=()=>{let e=document.querySelector(`.replay-pager`);e&&(e.innerHTML=Jt(),Ln(),zn())},jn=()=>{v=1},Mn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{let t=e.dataset.playerFilter===g;e.classList.toggle(`is-selected`,t),e.setAttribute(`aria-pressed`,String(t)),e.closest(`.player-card`)?.classList.toggle(`is-selected`,t),e.classList.contains(`ranking-row`)&&e.classList.toggle(`is-selected`,t)})},Nn=e=>{g=g===e?`all`:e,jn(),Mn(),An()},Pn=(e=null)=>{document.querySelectorAll(`.dropdown-filter.is-open`).forEach(t=>{t!==e&&(t.classList.remove(`is-open`),t.querySelector(`.filter-dropdown-button`)?.setAttribute(`aria-expanded`,`false`))})},Fn=()=>{let e=document.querySelector(`.filter-popover`);e&&(e.classList.remove(`is-open`),e.querySelector(`.filters-toggle`)?.setAttribute(`aria-expanded`,`false`),Pn())},In=()=>{let e=document.querySelector(`.filters`);e&&(e.outerHTML=tn(),Vn())},Ln=()=>{document.querySelectorAll(`.pagination-button[data-page]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.page);!Number.isFinite(t)||e.disabled||t===v||(v=t,An(),document.querySelector(`.replay-column`)?.scrollIntoView({block:`start`,behavior:`smooth`}))})})},Rn=()=>{y=null,w=`overview`,$()},zn=()=>{document.querySelectorAll(`.replay-row[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`a, button`)||(y=e.dataset.replayDetail,w=`overview`,$())}),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||t.target.closest(`a, button`)||(t.preventDefault(),y=e.dataset.replayDetail,w=`overview`,$())})}),document.querySelectorAll(`.replay-analysis-button[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),y=e.dataset.replayDetail,w=`overview`,$()})}),document.querySelectorAll(`[data-analysis-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{w=e.dataset.analysisTab||`overview`,$()})}),document.querySelectorAll(`[data-modal-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Rn()})}),document.querySelectorAll(`[data-theater-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&It()})}),document.querySelector(`[data-theater-play]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Rt()}),document.querySelector(`[data-theater-reset]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),S=0,x=!1,C&&=(clearInterval(C),null),$()})},Bn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.ranking-stream`)||Nn(e.dataset.playerFilter)}),e.addEventListener(`keydown`,t=>{e.tagName!==`BUTTON`&&(t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Nn(e.dataset.playerFilter)))})})},Vn=()=>{let e=document.querySelector(`#replay-search`);if(!e)return;e.addEventListener(`input`,e=>{pe=e.target.value,jn(),An()});let t=document.querySelector(`.filter-popover`),n=document.querySelector(`.filters-toggle`);n?.addEventListener(`click`,e=>{e.stopPropagation();let r=t?.classList.toggle(`is-open`)||!1;n.setAttribute(`aria-expanded`,String(r)),r||Pn()}),t?.addEventListener(`click`,e=>{e.stopPropagation()}),document.querySelectorAll(`.dropdown-filter`).forEach(e=>{let t=e.querySelector(`.filter-dropdown-button`),n=e.dataset.filter;t?.addEventListener(`click`,n=>{n.stopPropagation();let r=e.classList.toggle(`is-open`);Pn(e),t.setAttribute(`aria-expanded`,String(r))}),e.querySelectorAll(`.filter-dropdown-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.value||`all`;n===`map-filter`&&(me=t),n===`hero-filter`&&(he=t),n===`matchup-filter`&&(_=t),jn(),Pn(),In(),An()})})})},Hn=()=>{let e=document.querySelector(`.player-card-strip`);if(!e)return;let t=!1,n=0,r=0,i=!1;e.addEventListener(`pointerdown`,a=>{if(a.button===0){if(a.target.closest(`a, button, input, select, textarea, label`)){t=!1,i=!1;return}t=!0,i=!1,n=a.clientX,r=e.scrollLeft,e.classList.add(`is-dragging`),e.setPointerCapture(a.pointerId)}}),e.addEventListener(`pointermove`,a=>{if(!t)return;let o=a.clientX-n;Math.abs(o)>4&&(i=!0),e.scrollLeft=r-o});let a=n=>{t&&(t=!1,e.classList.remove(`is-dragging`),e.hasPointerCapture(n.pointerId)&&e.releasePointerCapture(n.pointerId))};e.addEventListener(`pointerup`,a),e.addEventListener(`pointercancel`,a),e.addEventListener(`click`,e=>{i&&=(e.preventDefault(),e.stopPropagation(),!1)},!0)},Un=e=>`
...
`,Gn=()=>E===`analyze`?`
  ...
  `,Kn=()=>`
  <button class="app-info-button" type="button" aria-label="Open app info" aria-expanded="${T}" aria-controls="app-info-panel" data-info-toggle>
    <span aria-hidden="true">i</span>
  </button>
  <aside class="app-info-panel ${T?`is-open`:``}" id="app-info-panel" aria-hidden="${!T}" aria-label="App info">
    ...
    <div class="app-info-tabs" role="tablist" aria-label="App info sections">
      ${Se.map(e=>`
            <button class="${e.id===E?`is-active`:``}" type="button" role="tab" aria-selected="${e.id===E}" data-info-tab="${N(e.id)}">
              ${N(e.label)}
            </button>
          `).join(``)}
    </div>
...
`,Jn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{T=!T,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{T=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{E=e.dataset.infoTab||`sources`,$()})})},Yn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{D=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),y=null,w=`overview`,D=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{O=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,O),D=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{hn(O)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{gn()})},Xn=()=>{ge=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=Me();document.querySelector(`#app`).innerHTML=`
    <header class="site-header">
      <a class="brand ${ye?``:`with-logo-intro`}" href="#/" aria-label="Human vs The World home">
        <img src="${u}" alt="Human vs The World" />
      </a>
    ...
    ${zt()}
    ${Kn()}
  `;let t=document.querySelector(`.site-header`),n=document.querySelector(`.menu-panel`),r=document.querySelector(`.menu-backdrop`),i=document.querySelector(`.menu-button`),a=()=>{n.classList.remove(`is-open`),r?.classList.remove(`is-open`),n.setAttribute(`aria-hidden`,`true`),i.setAttribute(`aria-expanded`,`false`),i.setAttribute(`aria-label`,`Open menu`)};i.addEventListener(`click`,()=>{let e=n.classList.toggle(`is-open`);r?.classList.toggle(`is-open`,e),n.setAttribute(`aria-hidden`,String(!e)),i.setAttribute(`aria-expanded`,String(e)),i.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),r?.addEventListener(`click`,a),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,a)}),document.documentElement.style.setProperty(`--header-height`,`${t.offsetHeight}px`),Zn(),Vn(),Ln(),zn(),Bn(),Hn(),Jn(),Yn(),document.querySelector(`.player-card-region.with-card-intro`)&&!ve&&(ve=window.setTimeout(()=>{_e=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!be&&(be=window.setTimeout(()=>{ye=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(b){It();return}if(T){T=!1,$();return}if(y){Rn();return}Fn();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-backdrop`),r=document.querySelector(`.menu-button`);!t||!r||(t.classList.remove(`is-open`),n?.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{Fn()}),window.addEventListener(`hashchange`,()=>{D=!1,C&&=(clearInterval(C),null),b=null,x=!1,S=0,g=`all`,me=`all`,he=`all`,_=`all`,jn(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Fe(n)&&(y=null,w=`overview`,Lt(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{Me().type===`wc3-replay-viewer`&&!D&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&Me().type===`wc3-replay-viewer`&&!D&&$()});var Zn=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,Zn,{passive:!0});var Qn=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Xn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([Qn(`replays.json`),Qn(`rankings.json`),Qn(`replay-analysis.json`),Qn(`map-drop-tables.json`),Qn(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?m=t.value:le=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(fe=n.value),r.status===`fulfilled`&&(ue=r.value),i.status===`fulfilled`&&(de=i.value),a.status===`fulfilled`&&(h=a.value),$()})();