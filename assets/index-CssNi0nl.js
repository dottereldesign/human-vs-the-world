...
`,$e=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Ye(e.streamUrl);return`
  ...
  `:``,U=()=>oe?I().flatMap(e=>Me(e.slug).replays.filter(e=>ke(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],it=()=>[...new Set(U().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),W=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,at=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,ot=e=>{let t=at(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},st=e=>{let t=ot(e);return t.length<2?``:`${W(t[0].race)} vs ${W(t[1].race)}`},ct=()=>[...new Set(U().map(st).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),lt=e=>L(e.sourceSlug,e.id),G=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],ut=e=>W(e?.raceDetected||e?.race)===`Human`,K=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},dt=(e,t=[])=>{t.forEach(t=>K(e,t.id,Number(t.count)||0))},q=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||R(e[0]).localeCompare(R(t[0]))).slice(0,t),ft=()=>{let e=new Map(G.map(e=>[e,0])),t=new Map(G.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,p=0,te=0;return U().forEach(m=>{let ne=L(m.sourceSlug,m.id);if(ne?.status!==`parsed`)return;u+=1;let re=(ne.players||[]).filter(ut);re.length&&(K(s,m.map||ne.map?.name||`Unknown map`),K(c,st(m)||`Unknown matchup`),re.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{G.includes(t.id)&&K(e,t.id)}),G.includes(c[0]?.id)&&K(t,c[0].id),(s.order?.items||[]).forEach(e=>{Le(e.id)?(K(n,e.id),p+=N[e.id]||0):(K(r,e.id),N[e.id]!==null&&(te+=1))}),dt(i,s.topUnits),dt(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>K(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:p,unknownBoughtPrices:te}},pt=e=>(lt(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),mt=()=>{let e=new Map;return U().forEach(t=>{pt(t).forEach(t=>{e.has(t)||e.set(t,R(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},ht=e=>ot(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),gt=r=>{let i=String(r||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:e,human:e,o:n,orc:n,u:p,undead:p,n:t,nightelf:t};return a[i]?a[i]:null},_t=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>W(t.race||t.raceDetected)===W(e.race));return i.length===1?i[0]:n[t]||null},vt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${P(t)}</span>
      <strong>${P(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Je(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},J=(e,t=[])=>ot(e).map((e,n)=>{let r=gt(e.race),i=_t(e,n,t);return`
...
`,yt=(e,t)=>!t||t.status!==`parsed`?`
  ...
  `},xt=(e,t)=>t.flatMap(([t,n])=>(n||[]).map(n=>({...n,type:t,playerName:e.name,race:e.raceDetected||e.race}))).sort((e,t)=>(e.ms||0)-(t.ms||0)),St=(e,t=`${e.type}`)=>`
...
`,Ct=(e,t)=>`
  <div class="build-order-row item-found-row">
    ...
    <em>
      ...
      <small>${P(Ke(e.id,t))}</small>
      <small>${P(qe(e.id,t))}</small>
    </em>
  </div>
`,wt=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=St,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
  ...
  `:`<div class="analysis-empty"><strong>${P(t)} unavailable</strong><span>${P(n)}</span></div>`},Tt=e=>wt({analysis:e,title:`Build order`,emptyMessage:`Run the updated replay analyzer to generate unit and building timeline data.`,getItems:e=>xt(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units]])}),Et=e=>wt({analysis:e,title:`Upgrades`,emptyMessage:`No upgrades were parsed for this replay.`,getItems:e=>Be(xt(e,[[`Upgrade`,e.order?.upgrades]])),renderRow:e=>St(e,Number.isFinite(e.goldCost)?Re(e.goldCost):`Cost unknown`),renderSummary:(e,t)=>{let n=Ve(t,e=>e.goldCost);return He({label:`Known gold spent`,gold:n.gold,unknown:n.unknown})}}),Dt=e=>wt({analysis:e,title:`Items bought`,emptyMessage:`No shop-price items were parsed for this replay.`,getItems:e=>xt(e,[[`Item`,e.order?.items]]).filter(e=>Le(e.id)),renderSummary:(e,t)=>{let n=Ve(t,e=>N[e.id]);return He({label:`Total spent`,gold:n.gold,unknown:n.unknown})},renderRow:t=>{let n=We(t.id,e);return St(t,n?`${Ie(t.id)} · ${Ge(n.chance)} ${n.groupLabel} drop`:Ie(t.id))}}),Ot=e=>wt({analysis:e,title:`Items found`,emptyMessage:`No no-shop-price items were parsed for this replay.`,getItems:e=>xt(e,[[`Item`,e.order?.items]]).filter(e=>!Le(e.id)),renderRow:t=>Ct(t,e)}),kt=e=>{let t=e?.chat||[];return t.length?`
  ...
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},At=(e=[])=>e.length?e.map(e=>`<span>${P(R(e.id))} × ${P(e.count)}</span>`).join(``):`<span>None parsed</span>`,jt=e=>{let t=e?.players||[];return t.length?`
  ...
  `:`<div class="analysis-empty"><strong>No action data</strong><span>Run the updated replay analyzer to generate action data.</span></div>`},Mt=(e,t)=>T===`apm`?bt(t):T===`build`?Tt(t):T===`upgrades`?Et(t):T===`items-bought`?Dt(t):T===`items-found`?Ot(t):T===`chat`?kt(t):T===`actions`?jt(t):yt(e,t),Nt=()=>{let e=Pe(b);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=ye.some(e=>e.id===T)?T:`overview`;return`
  ...
  `},X=(e=0)=>{let t=Math.max(0,Math.floor(Number(e||0)/1e3)),n=Math.floor(t/60),r=t%60;return`${n}:${String(r).padStart(2,`0`)}`},Pt=e=>{if(!e||e.status!==`parsed`)return[];let t=(e.players||[]).flatMap(e=>[...(e.heroes||[]).map(t=>({id:t.id,ms:0,time:`0:00`,type:`Hero`,playerName:e.name,race:e.raceDetected||e.race,detail:t.level?`Level ${t.level}`:`Hero`})),...xt(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units],[`Upgrade`,Be(e.order?.upgrades||[])],[`Item`,e.order?.items]]).map(t=>({...t,playerName:e.name,detail:t.type===`Item`?Ie(t.id):t.type===`Upgrade`&&Number.isFinite(t.goldCost)?Re(t.goldCost):t.type}))]),n=(e.chat||[]).map(e=>({id:`chat`,ms:e.timeMS||0,time:e.time||X(e.timeMS||0),type:`Chat`,playerName:e.playerName||`Observer`,detail:e.message||``}));return[...t,...n].sort((e,t)=>(e.ms||0)-(t.ms||0))},Ft=()=>{x=null,S=!1,C=0,w&&=(clearInterval(w),null),$()},It=e=>{x=e,C=0,S=!1,w&&=(clearInterval(w),null),$()},Lt=()=>{let e=Pe(x),t=(e?L(e.sourceSlug,e.id):null)?.duration||0;if(S=!S,w&&=(clearInterval(w),null),!S){$();return}w=setInterval(()=>{C+=1e3,t&&C>=t&&(C=t,S=!1,clearInterval(w),w=null),$()},1e3),$()},Rt=()=>{let e=Pe(x);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=Pt(t),i=t?.duration||e.duration||Math.max(...r.map(e=>e.ms||0),0),a=i?Math.min(100,Math.max(0,C/i*100)):0,o=r.filter(e=>(e.ms||0)<=C).slice(-18),s=r.filter(e=>(e.ms||0)>C).slice(0,18);return`
          ...
          <div class="replay-theater-columns">
            <section>
              ...
              <div class="replay-theater-event-list">
                ${o.length?o.map(zt).join(``):`<div class="timeline-empty">Press play to advance the parsed replay timeline.</div>`}
              </div>
            </section>
            <section>
              <h3>Next Events</h3>
              <div class="replay-theater-event-list">
                ${s.length?s.map(zt).join(``):`<div class="timeline-empty">No more parsed events.</div>`}
              </div>
            </section>
          </div>
        </div>
...
`,Bt=e=>fe?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,ht(e)].filter(Boolean).join(` `).toLowerCase().includes(fe.toLowerCase()):!0,Vt=e=>{let t=h===`all`||e.sourceSlug===h,n=g===`all`||e.map===g,r=_===`all`||pt(e).includes(_),i=v===`all`||st(e)===v;return t&&n&&r&&i&&Bt(e)},Ht=()=>U().filter(Vt).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Ut=e=>{y=Math.min(Math.max(y,1),Math.max(e,1))},Wt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Gt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(y-1)*ve+1,r=Math.min(y*ve,e),i=Wt(y,t),a=0;return`
  ...
  `},Kt=e=>se?`
    ...
    `:oe?e.length?e.map(e=>{let t=e.createdAt?we.format(new Date(e.createdAt)):`Unknown date`,n=L(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
    ...
    `;let e=Ht(),t=Math.ceil(e.length/ve);Ut(t);let n=(y-1)*ve;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${Kt(e.slice(n,n+ve))}
    </section>
    ${e.length?Gt(e.length,Math.max(t,1)):``}
  `},Jt=(e=``)=>`
...
`,$t=({id:e,label:t,value:n,options:r})=>{let i=r.find(e=>e.value===n)||r[0];return`
  ...
  <section class="filters" aria-label="Replay filters">
    <div class="quick-filter">
      ...
      <input id="replay-search" type="search" value="${P(fe)}" placeholder="Search map, opponent, patch..." />
    </div>
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${$t({id:`map-filter`,label:`Map`,value:g,options:[{value:`all`,label:`All maps`},...it().map(e=>({value:e,label:e}))]})}
        ${$t({id:`hero-filter`,label:`Starting hero`,value:_,options:[{value:`all`,label:`All heroes`},...mt()]})}
        ${$t({id:`matchup-filter`,label:`Matchup`,value:v,options:[{value:`all`,label:`All matchups`},...ct().map(e=>({value:e,label:e}))]})}
      </div>
    </div>
  </section>
`,tn=e=>{if(!e.streamUrl)return`<span class="ranking-stream is-empty" aria-hidden="true"></span>`;let t=Ye(e.streamUrl);return`
...
`,rn=e=>e?`
  ...
  `:``,an=()=>`
  <section class="player-card-region ${me?``:`with-card-intro`}" aria-label="Top Human player cards">
...
`,cn=()=>{let e=U().find(e=>e.mapImage?.localPath&&L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?we.format(new Date(e.createdAt)):`Unknown date`;return`
  ...
  `},ln=()=>{try{let e=JSON.parse(localStorage.getItem(`w3AssetsIndex`)||`[]`),t=Array.isArray(e)?e.length:0,n=Array.isArray(e)?e.filter(e=>String(e?.p||``).toLowerCase().endsWith(`.mpq`)).length:0,r=Array.isArray(e)?e.filter(e=>/\.(w3x|w3m)$/i.test(String(e?.p||``))).length:0,i=Array.isArray(e)?e.reduce((e,t)=>e+Number(t?.s||0),0):0,a=localStorage.getItem(`wc3-warsmash-staged-edition`)||`legacy`;return{ready:localStorage.getItem(`w3AssetsReady`)===`1`&&t>0&&a===k,edition:a,fileCount:t,mpqCount:n,mapCount:r,totalMb:(i/1048576).toFixed(1)}}catch{return{ready:!1,edition:`legacy`,fileCount:0,mpqCount:0,mapCount:0,totalMb:`0.0`}}},un=()=>pe===`windows`?{platform:`Windows`,primary:`C:\\Program Files\\Warcraft III`,secondary:`C:\\Program Files (x86)\\Warcraft III`,pickerHint:`In the folder picker, paste the path into the address bar if it does not open there automatically.`}:{platform:`macOS`,primary:`/Applications/Warcraft III (Legacy)`,secondary:`/Applications/Warcraft III`,pickerHint:`In the folder picker, press Cmd+Shift+G, paste the path, then choose the Warcraft III folder.`},Z={reforged:`2.0.4.23745`,legacy:`1.29.2.9232`},dn=[{id:`legacy`,label:`Legacy MPQ`,build:Z.legacy,status:`Playable now`,note:`Current Warsmash web path mounts classic MPQs and loose maps from this install.`},{id:`reforged`,label:`Reforged`,build:Z.reforged,status:`Experimental CASC test`,note:`Latest Reforged uses CASC assets. This app now has an experimental browser-worker CASC mount path backed by the private sandbox.`}],fn=async(e,t)=>{let n=e;for(let e of t)n=await n.getDirectoryHandle(e,{create:!0});return n},pn=async()=>{let e=await navigator.storage.getDirectory();for(let t of[`w3`,`extracted`])try{await e.removeEntry(t,{recursive:!0})}catch{}},mn=async(e=k)=>{if(!j.running){j={running:!0,done:0,total:0,bytes:0,totalBytes:0,currentName:`Reading local install...`,error:``},$();try{let t=await fetch(`/wc3-local-assets/manifest?edition=${encodeURIComponent(e)}`,{cache:`no-store`});if(!t.ok){let e=await t.json().catch(()=>({}));throw Error(e.error||`No local MPQ-style Warcraft III install was detected.`)}let n=await t.json(),r=Array.isArray(n.files)?n.files:[];if(!r.length)throw Error(`Detected install did not include MPQ/map files Warsmash can use.`);j={running:!0,done:0,total:r.length,bytes:0,totalBytes:Number(n.totalBytes||0),currentName:`Preparing ${n.root||`local install`}`,error:``},$(),await pn();let i=await(await navigator.storage.getDirectory()).getDirectoryHandle(`w3`,{create:!0}),a=[],o=0,s=0;for(let t of r){let c=String(t.path||``);if(!c)continue;let l=await fetch(`/wc3-local-assets/file?edition=${encodeURIComponent(e)}&path=${encodeURIComponent(c)}`);if(!l.ok)throw Error(`Failed to read ${c}`);let u=await l.blob(),d=c.split(`/`).filter(Boolean),f=await(await(await fn(i,d.slice(0,-1))).getFileHandle(d[d.length-1],{create:!0})).createWritable();await f.write(u),await f.close(),o+=1,s+=u.size,a.push({p:c,s:u.size}),j={running:!0,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:c,error:``},(o===r.length||o%3==0)&&$()}localStorage.setItem(`w3AssetsReady`,`1`),localStorage.setItem(`w3AssetsCount`,String(a.length)),localStorage.setItem(`w3AssetsIndex`,JSON.stringify(a)),localStorage.setItem(`w3AssetsSource`,n.root||`detected local install`),localStorage.setItem(`wc3-warsmash-staged-edition`,e),localStorage.setItem(`wc3-warsmash-selected-edition`,e),j={running:!1,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:`Ready`,error:``},$()}catch(e){j={...j,running:!1,error:e instanceof Error?e.message:String(e)},$()}}},hn=async()=>{if(!A.preparing){A={...A,preparing:!0,error:``},$();try{let e=await fetch(`/wc3-reforged-sandbox/prepare`,{cache:`no-store`}),t=await e.json().catch(()=>({}));if(!e.ok)throw Error(t.error||`Failed to prepare Reforged sandbox.`);A={checked:!0,preparing:!1,ready:!0,build:t.build||``,fileCount:Number(t.totals?.configFiles||0)+Number(t.totals?.indexFiles||0)+Number(t.totals?.dataFiles||0),totalGb:(Number(t.totals?.bytes||0)/1073741824).toFixed(2),error:``}}catch(e){A={...A,checked:!0,preparing:!1,ready:!1,error:e instanceof Error?e.message:String(e)}}$()}},gn=()=>{let e=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?we.format(new Date(e.createdAt)):`Unknown date`,i=e?`${e.sourceSlug}:${e.id}`:``,a=`/play/?menu=1&assets=${encodeURIComponent(k)}&replay=${encodeURIComponent(i)}&autostart=1`,o=`/assets/`,s=ln(),c=un(),l=dn.find(e=>e.id===k)||dn[0],u=l.id===`legacy`,d=u?s.ready:A.ready,f=()=>k===`reforged`?`
...
`,_n=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${z(e)}${P(Fe(R(e)).replace(` (${e})`,``))}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,vn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-text-row">
              <span>${P(e)}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,yn=()=>{let e=ft(),t=e.humanGames||1,n=q(e.boughtItems,10),r=q(e.foundItems,8);return`
        ...
        <div class="statistics-hero-grid">
          ${G.map(n=>{let r=e.heroPicks.get(n)||0,i=e.firstHeroPicks.get(n)||0,a=e.humanGames?r/t*100:0,o=e.humanGames?i/t*100:0;return`
                <article class="statistics-hero-card">
                  <div>
                    ${z(n)}
                    <h3>${P(Fe(R(n)).replace(` (${n})`,``))}</h3>
                  </div>
                  <dl>
                    <div><dt>Picked</dt><dd>${P(B(r))}</dd></div>
                    <div><dt>Pick rate</dt><dd>${P(Ge(a))}</dd></div>
                    <div><dt>First hero</dt><dd>${P(B(i))}</dd></div>
                    <div><dt>First rate</dt><dd>${P(Ge(o))}</dd></div>
                  </dl>
                </article>
              `}).join(``)}
        </div>
      ...
      <div class="statistics-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(Re(e.knownBoughtGold))}</span>
          </div>
          <div class="statistics-list">
            ${_n(n,`No shop purchases found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(r,`No non-shop item events found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(q(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(q(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(q(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(q(e.mapCounts,8),`No map stats found`)}
          </div>
        </section>
  ...
  `},bn=()=>{let e=U(),t=e.map(e=>({replay:e,analysis:L(e.sourceSlug,e.id)})).filter(e=>e.analysis?.status===`parsed`),n=new Map,r=new Map,i=new Map,a=0,o=0,s=0,c=0,l=0;return t.forEach(({replay:e,analysis:t})=>{K(n,t.version||e.version||`Unknown`),K(r,e.map||t.map?.file||`Unknown map`),a+=t.chat?.length||0,s+=t.chat?.length||0,(t.players||[]).forEach(e=>{o+=1,Number.isFinite(e.apm)&&(c+=e.apm,l+=1),Object.entries(e.actions||{}).forEach(([e,t])=>{K(i,e,t||0)}),s+=(e.heroes||[]).length,s+=(e.order?.buildings||[]).length,s+=(e.order?.units||[]).length,s+=(e.order?.upgrades||[]).length,s+=(e.order?.items||[]).length})}),{totalReplays:e.length,parsedReplays:t.length,parsedPlayers:o,eventCount:s,chatMessages:a,averageApm:l?Math.round(c/l):0,versionCounts:n,mapCounts:r,actionCounts:i}},xn=e=>e.length?e.map(([e,t])=>`
          ...
          `).join(``):`<div class="analysis-empty"><strong>No timeline events parsed yet</strong></div>`,Cn=()=>{let e=bn(),t=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],n=t?L(t.sourceSlug,t.id):null,r=n?.status===`parsed`?n.players:[],i=Pt(n),a=i.slice(0,12),o=i.filter(e=>(e.ms||0)>=18e4).slice(0,12),s=q(e.actionCounts,12),c=q(e.versionCounts,4),l=q(e.mapCounts,6);return`
    ...
    <div class="resources-link-list">
      ${e.links.map(e=>`
            <a href="${P(e.href)}" target="_blank" rel="noreferrer">
              <img src="${P(Tn(e.href))}" alt="" loading="lazy" onerror="this.remove()" />
              <span>${P(e.label)}</span>
...
`,On=()=>{let e=de?.players||[];return`
    <section class="money-page" aria-label="Player earnings">
      ...
      <p class="money-note">${P(de?.scope||``)}</p>
    </section>
  `},kn=()=>{let e=document.querySelector(`.replay-pager`);e&&(e.innerHTML=qt(),In(),Rn())},An=()=>{y=1},jn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{let t=e.dataset.playerFilter===h;e.classList.toggle(`is-selected`,t),e.setAttribute(`aria-pressed`,String(t)),e.closest(`.player-card`)?.classList.toggle(`is-selected`,t),e.classList.contains(`ranking-row`)&&e.classList.toggle(`is-selected`,t)})},Mn=e=>{h=h===e?`all`:e,An(),jn(),kn()},Nn=(e=null)=>{document.querySelectorAll(`.dropdown-filter.is-open`).forEach(t=>{t!==e&&(t.classList.remove(`is-open`),t.querySelector(`.filter-dropdown-button`)?.setAttribute(`aria-expanded`,`false`))})},Pn=()=>{let e=document.querySelector(`.filter-popover`);e&&(e.classList.remove(`is-open`),e.querySelector(`.filters-toggle`)?.setAttribute(`aria-expanded`,`false`),Nn())},Fn=()=>{let e=document.querySelector(`.filters`);e&&(e.outerHTML=en(),Bn())},In=()=>{document.querySelectorAll(`.pagination-button[data-page]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.page);!Number.isFinite(t)||e.disabled||t===y||(y=t,kn(),document.querySelector(`.replay-column`)?.scrollIntoView({block:`start`,behavior:`smooth`}))})})},Ln=()=>{b=null,T=`overview`,$()},Rn=()=>{document.querySelectorAll(`.replay-row[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`a, button`)||(b=e.dataset.replayDetail,T=`overview`,$())}),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||t.target.closest(`a, button`)||(t.preventDefault(),b=e.dataset.replayDetail,T=`overview`,$())})}),document.querySelectorAll(`.replay-analysis-button[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),b=e.dataset.replayDetail,T=`overview`,$()})}),document.querySelectorAll(`[data-analysis-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{T=e.dataset.analysisTab||`overview`,$()})}),document.querySelectorAll(`[data-modal-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Ln()})}),document.querySelectorAll(`[data-theater-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Ft()})}),document.querySelector(`[data-theater-play]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Lt()}),document.querySelector(`[data-theater-reset]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),C=0,S=!1,w&&=(clearInterval(w),null),$()})},zn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.ranking-stream`)||Mn(e.dataset.playerFilter)}),e.addEventListener(`keydown`,t=>{e.tagName!==`BUTTON`&&(t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Mn(e.dataset.playerFilter)))})})},Bn=()=>{let e=document.querySelector(`#replay-search`);if(!e)return;e.addEventListener(`input`,e=>{fe=e.target.value,An(),kn()});let t=document.querySelector(`.filter-popover`),n=document.querySelector(`.filters-toggle`);n?.addEventListener(`click`,e=>{e.stopPropagation();let r=t?.classList.toggle(`is-open`)||!1;n.setAttribute(`aria-expanded`,String(r)),r||Nn()}),t?.addEventListener(`click`,e=>{e.stopPropagation()}),document.querySelectorAll(`.dropdown-filter`).forEach(e=>{let t=e.querySelector(`.filter-dropdown-button`),n=e.dataset.filter;t?.addEventListener(`click`,n=>{n.stopPropagation();let r=e.classList.toggle(`is-open`);Nn(e),t.setAttribute(`aria-expanded`,String(r))}),e.querySelectorAll(`.filter-dropdown-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.value||`all`;n===`map-filter`&&(g=t),n===`hero-filter`&&(_=t),n===`matchup-filter`&&(v=t),An(),Nn(),Fn(),kn()})})})},Vn=()=>{let e=document.querySelector(`.player-card-strip`);if(!e)return;let t=!1,n=0,r=0,i=!1;e.addEventListener(`pointerdown`,a=>{if(a.button===0){if(a.target.closest(`a, button, input, select, textarea, label`)){t=!1,i=!1;return}t=!0,i=!1,n=a.clientX,r=e.scrollLeft,e.classList.add(`is-dragging`),e.setPointerCapture(a.pointerId)}}),e.addEventListener(`pointermove`,a=>{if(!t)return;let o=a.clientX-n;Math.abs(o)>4&&(i=!0),e.scrollLeft=r-o});let a=n=>{t&&(t=!1,e.classList.remove(`is-dragging`),e.hasPointerCapture(n.pointerId)&&e.releasePointerCapture(n.pointerId))};e.addEventListener(`pointerup`,a),e.addEventListener(`pointercancel`,a),e.addEventListener(`click`,e=>{i&&=(e.preventDefault(),e.stopPropagation(),!1)},!0)},Hn=()=>{let e=document.querySelector(`.theme-button`);e&&e.addEventListener(`click`,()=>{M=M===`dark`?`light`:`dark`,localStorage.setItem(`wc3-color-theme`,M),Ae({instant:!0}),e.setAttribute(`aria-label`,M===`dark`?`Switch to light theme`:`Switch to dark theme`)})},Un=e=>`
...
`,Jn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{E=!E,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{E=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{D=e.dataset.infoTab||`sources`,$()})})},Yn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{O=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),b=null,T=`overview`,O=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{k=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,k),O=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{mn(k)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{hn()})},Xn=()=>{pe=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=je();document.querySelector(`#app`).innerHTML=`
    ...
    <aside class="menu-panel" id="site-menu" aria-hidden="true">
      <div class="menu-panel-inner">
        ...
        <nav class="menu-nav" aria-label="Menu">
          ${Jt(e.type===`all`?`is-active`:``)}
          ${Yt(e.type===`builds`?`is-active`:``)}
          ${Xt(e.type===`player-earnings`?`is-active`:``)}
          ${Zt(e.type===`statistics`?`is-active`:``)}
          ${Qt(e.type===`resources`?`is-active`:``)}
        </nav>
    ...
    <main class="page-shell">
      ${e.type===`builds`?sn():e.type===`player-earnings`?On():e.type===`statistics`?yn():e.type===`renderer`?cn():e.type===`wc3-replay-viewer`?gn():e.type===`w3gjs`?Cn():e.type===`resources`?Dn():on()}
    </main>
    ${qn()}
    ...
    ${Rt()}
    ${Kn()}
  `;let t=document.querySelector(`.site-header`),n=document.querySelector(`.menu-panel`),r=document.querySelector(`.menu-button`),i=()=>{n.classList.remove(`is-open`),n.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`)};r.addEventListener(`click`,()=>{let e=n.classList.toggle(`is-open`);n.setAttribute(`aria-hidden`,String(!e)),r.setAttribute(`aria-expanded`,String(e)),r.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,i)}),document.documentElement.style.setProperty(`--header-height`,`${t.offsetHeight}px`),Zn(),Bn(),In(),Rn(),zn(),Vn(),Hn(),Jn(),Yn(),document.querySelector(`.player-card-region.with-card-intro`)&&!he&&(he=window.setTimeout(()=>{me=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!_e&&(_e=window.setTimeout(()=>{ge=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(x){Ft();return}if(E){E=!1,$();return}if(b){Ln();return}Pn();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-button`);!t||!n||(t.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),n.setAttribute(`aria-expanded`,`false`),n.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{Pn()}),window.addEventListener(`hashchange`,()=>{O=!1,w&&=(clearInterval(w),null),x=null,S=!1,C=0,h=`all`,g=`all`,_=`all`,v=`all`,An(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Pe(n)&&(b=null,T=`overview`,It(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{je().type===`wc3-replay-viewer`&&!O&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&je().type===`wc3-replay-viewer`&&!O&&$()});var Zn=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,Zn,{passive:!0});var Qn=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Xn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([Qn(`replays.json`),Qn(`rankings.json`),Qn(`replay-analysis.json`),Qn(`map-drop-tables.json`),Qn(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?oe=t.value:se=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(ue=n.value),r.status===`fulfilled`&&(ce=r.value),i.status===`fulfilled`&&(le=i.value),a.status===`fulfilled`&&(de=a.value),$()})();