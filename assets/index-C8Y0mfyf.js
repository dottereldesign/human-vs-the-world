...
`,Vt=e=>pe?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,_t(e)].filter(Boolean).join(` `).toLowerCase().includes(pe.toLowerCase()):!0,Ht=e=>{let t=g===`all`||e.sourceSlug===g,n=_===`all`||e.map===_,r=me===`all`||ht(e).includes(me),i=he===`all`||lt(e)===he;return t&&n&&r&&i&&Vt(e)},Ut=()=>H().filter(Ht).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Wt=e=>{v=Math.min(Math.max(v,1),Math.max(e,1))},Gt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Kt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(v-1)*j+1,r=Math.min(v*j,e),i=Gt(v,t),a=0;return`
  ...
  <section class="filters" aria-label="Replay filters">
    ...
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${en({id:`map-filter`,label:`Map`,value:_,options:[{value:`all`,label:`All maps`},...ot().map(e=>({value:e,label:e}))]})}
        ${en({id:`hero-filter`,label:`Starting hero`,value:me,options:[{value:`all`,label:`All heroes`},...gt()]})}
        ${en({id:`matchup-filter`,label:`Matchup`,value:he,options:[{value:`all`,label:`All matchups`},...ut().map(e=>({value:e,label:e}))]})}
      </div>
    </div>
  </section>
