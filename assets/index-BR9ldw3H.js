...
`,et=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Xe(e.streamUrl);return`
    <a href="${P(e.streamUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${P(e.name)} ${P(t.label)} stream">
      ${$e(t,t.label)}
    </a>
  `},tt=e=>e.liquipediaUrl?`
    <a href="${P(e.liquipediaUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${P(e.name)} on Liquipedia">
      ${$e({key:`liquipedia`,short:`LP`,icon:d},`Liquipedia`)}
    </a>
  `:`<span class="is-disabled"><span class="service-icon service-icon-liquipedia" aria-hidden="true"><img src="${d}" alt="" loading="lazy" /></span><span class="action-label">Liquipedia</span></span>`,nt=e=>e.realName?`<p class="player-card-real-name">${P(e.realName)}</p>`:``,rt=e=>{let t={chaemiko:r,fortitude:o,hawk:s,infi:l,leon:u,sok:f}[e.slug];return t?`<img class="player-card-image" src="${t}" alt="${P(e.name)}" loading="lazy" />`:`<span>${P(e.flag)}</span>`},it=e=>e.slug===`infi`?`
  ...
  `:``,U=()=>m?I().flatMap(e=>Me(e.slug).replays.filter(e=>ke(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],at=()=>[...new Set(U().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),W=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,ot=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,st=e=>{let t=ot(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},ct=e=>{let t=st(e);return t.length<2?``:`${W(t[0].race)} vs ${W(t[1].race)}`},lt=()=>[...new Set(U().map(ct).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),ut=e=>L(e.sourceSlug,e.id),G=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],dt=e=>W(e?.raceDetected||e?.race)===`Human`,K=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},ft=(e,t=[])=>{t.forEach(t=>K(e,t.id,Number(t.count)||0))},q=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||R(e[0]).localeCompare(R(t[0]))).slice(0,t),pt=()=>{let e=new Map(G.map(e=>[e,0])),t=new Map(G.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,te=0,ne=0;return U().forEach(p=>{let re=L(p.sourceSlug,p.id);if(re?.status!==`parsed`)return;u+=1;let ie=(re.players||[]).filter(dt);ie.length&&(K(s,p.map||re.map?.name||`Unknown map`),K(c,ct(p)||`Unknown matchup`),ie.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{G.includes(t.id)&&K(e,t.id)}),G.includes(c[0]?.id)&&K(t,c[0].id),(s.order?.items||[]).forEach(e=>{Re(e.id)?(K(n,e.id),te+=N[e.id]||0):(K(r,e.id),N[e.id]!==null&&(ne+=1))}),ft(i,s.topUnits),ft(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>K(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:te,unknownBoughtPrices:ne}},mt=e=>(ut(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),ht=()=>{let e=new Map;return U().forEach(t=>{mt(t).forEach(t=>{e.has(t)||e.set(t,Ie(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},gt=e=>st(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),_t=r=>{let i=String(r||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:e,human:e,o:n,orc:n,u:te,undead:te,n:t,nightelf:t};return a[i]?a[i]:null},vt=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>W(t.race||t.raceDetected)===W(e.race));return i.length===1?i[0]:n[t]||null},yt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${P(t)}</span>
      <strong>${P(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Ye(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},J=(e,t=[])=>st(e).map((e,n)=>{let r=_t(e.race),i=vt(e,n,t);return`
...
`,Tt=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=Ct,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
  ...
  `:`<div class="analysis-empty"><strong>${P(t)} unavailable</strong><span>${P(n)}</span></div>`},Et=e=>Tt({analysis:e,title:`Build order`,emptyMessage:`Run the updated replay analyzer to generate unit and building timeline data.`,getItems:e=>St(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units]])}),Dt=e=>Tt({analysis:e,title:`Upgrades`,emptyMessage:`No upgrades were parsed for this replay.`,getItems:e=>Ve(St(e,[[`Upgrade`,e.order?.upgrades]])),renderRow:e=>Ct(e,Number.isFinite(e.goldCost)?ze(e.goldCost):`Cost unknown`),renderSummary:(e,t)=>{let n=He(t,e=>e.goldCost);return Ue({label:`Known gold spent`,gold:n.gold,unknown:n.unknown})}}),Ot=e=>Tt({analysis:e,title:`Items bought`,emptyMessage:`No shop-price items were parsed for this replay.`,getItems:e=>St(e,[[`Item`,e.order?.items]]).filter(e=>Re(e.id)),renderSummary:(e,t)=>{let n=He(t,e=>N[e.id]);return Ue({label:`Total spent`,gold:n.gold,unknown:n.unknown})},renderRow:t=>{let n=Ge(t.id,e);return Ct(t,n?`${Le(t.id)} · ${Ke(n.chance)} ${n.groupLabel} drop`:Le(t.id))}}),kt=e=>Tt({analysis:e,title:`Items found`,emptyMessage:`No no-shop-price items were parsed for this replay.`,getItems:e=>St(e,[[`Item`,e.order?.items]]).filter(e=>!Re(e.id)),renderRow:t=>wt(t,e)}),At=e=>{let t=e?.chat||[];return t.length?`
  ...
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},jt=(e=[])=>e.length?e.map(e=>`<span>${P(R(e.id))} × ${P(e.count)}</span>`).join(``):`<span>None parsed</span>`,Mt=e=>{let t=e?.players||[];return t.length?`
  ...
  `:`<div class="analysis-empty"><strong>No action data</strong><span>Run the updated replay analyzer to generate action data.</span></div>`},Nt=(e,t)=>T===`apm`?xt(t):T===`build`?Et(t):T===`upgrades`?Dt(t):T===`items-bought`?Ot(t):T===`items-found`?kt(t):T===`chat`?At(t):T===`actions`?Mt(t):bt(e,t),Pt=()=>{let e=Pe(b);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=ye.some(e=>e.id===T)?T:`overview`;return`
...
`,Vt=e=>de?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,gt(e)].filter(Boolean).join(` `).toLowerCase().includes(de.toLowerCase()):!0,Ht=e=>{let t=g===`all`||e.sourceSlug===g,n=_===`all`||e.map===_,r=v===`all`||mt(e).includes(v),i=fe===`all`||ct(e)===fe;return t&&n&&r&&i&&Vt(e)},Ut=()=>U().filter(Ht).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Wt=e=>{y=Math.min(Math.max(y,1),Math.max(e,1))},Gt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Kt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(y-1)*ve+1,r=Math.min(y*ve,e),i=Gt(y,t),a=0;return`
  ...
  `},qt=e=>se?`
    ...
    `:m?e.length?e.map(e=>{let t=e.createdAt?we.format(new Date(e.createdAt)):`Unknown date`,n=L(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
    ...
    `;let e=Ut(),t=Math.ceil(e.length/ve);Wt(t);let n=(y-1)*ve;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${qt(e.slice(n,n+ve))}
    </section>
  ...
  <section class="filters" aria-label="Replay filters">
    ...
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${en({id:`map-filter`,label:`Map`,value:_,options:[{value:`all`,label:`All maps`},...at().map(e=>({value:e,label:e}))]})}
        ${en({id:`hero-filter`,label:`Starting hero`,value:v,options:[{value:`all`,label:`All heroes`},...ht()]})}
        ${en({id:`matchup-filter`,label:`Matchup`,value:fe,options:[{value:`all`,label:`All matchups`},...lt().map(e=>({value:e,label:e}))]})}
      </div>
    </div>
  </section>
`,nn=e=>{if(!e.streamUrl)return`<span class="ranking-stream is-empty" aria-hidden="true"></span>`;let t=Xe(e.streamUrl);return`
...
`,ln=()=>{let e=U().find(e=>e.mapImage?.localPath&&L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?we.format(new Date(e.createdAt)):`Unknown date`;return`
...
`,vn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${z(e)}${P(Fe(R(e)).replace(` (${e})`,``))}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,yn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-text-row">
              <span>${P(e)}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,bn=()=>{let e=pt(),t=e.humanGames||1,n=q(e.boughtItems,10),r=q(e.foundItems,8);return`
          ...
          ${G.map(n=>{let r=e.heroPicks.get(n)||0,i=e.firstHeroPicks.get(n)||0,a=e.humanGames?r/t*100:0,o=e.humanGames?i/t*100:0;return`
                <article class="statistics-hero-card">
                  ...
                  <dl>
                    ...
                    <div><dt>Pick rate</dt><dd>${P(Ke(a))}</dd></div>
                    <div><dt>First hero</dt><dd>${P(B(i))}</dd></div>
                    <div><dt>First rate</dt><dd>${P(Ke(o))}</dd></div>
                  </dl>
                </article>
              `}).join(``)}
        </div>
      ...
      <div class="statistics-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(ze(e.knownBoughtGold))}</span>
          </div>
          <div class="statistics-list">
            ${vn(n,`No shop purchases found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(r,`No non-shop item events found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(q(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(q(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(q(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${yn(q(e.mapCounts,8),`No map stats found`)}
          </div>
        </section>
  ...
  `},xn=()=>{let e=U(),t=e.map(e=>({replay:e,analysis:L(e.sourceSlug,e.id)})).filter(e=>e.analysis?.status===`parsed`),n=new Map,r=new Map,i=new Map,a=0,o=0,s=0,c=0,l=0;return t.forEach(({replay:e,analysis:t})=>{K(n,t.version||e.version||`Unknown`),K(r,e.map||t.map?.file||`Unknown map`),a+=t.chat?.length||0,s+=t.chat?.length||0,(t.players||[]).forEach(e=>{o+=1,Number.isFinite(e.apm)&&(c+=e.apm,l+=1),Object.entries(e.actions||{}).forEach(([e,t])=>{K(i,e,t||0)}),s+=(e.heroes||[]).length,s+=(e.order?.buildings||[]).length,s+=(e.order?.units||[]).length,s+=(e.order?.upgrades||[]).length,s+=(e.order?.items||[]).length})}),{totalReplays:e.length,parsedReplays:t.length,parsedPlayers:o,eventCount:s,chatMessages:a,averageApm:l?Math.round(c/l):0,versionCounts:n,mapCounts:r,actionCounts:i}},Sn=e=>e.length?e.map(([e,t])=>`
          ...
          `).join(``):`<div class="analysis-empty"><strong>No timeline events parsed yet</strong></div>`,wn=()=>{let e=xn(),t=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],n=t?L(t.sourceSlug,t.id):null,r=n?.status===`parsed`?n.players:[],i=Ft(n),a=i.slice(0,12),o=i.filter(e=>(e.ms||0)>=18e4).slice(0,12),s=q(e.actionCounts,12),c=q(e.versionCounts,4),l=q(e.mapCounts,6);return`
    ...
    <div class="resources-link-list">
      ${e.links.map(e=>`
            <a href="${P(e.href)}" target="_blank" rel="noreferrer">
              <img src="${P(En(e.href))}" alt="" loading="lazy" onerror="this.remove()" />
              <span>${P(e.label)}</span>
...
`,kn=()=>{let e=h?.players||[];return`
  ...
  `},An=()=>{let e=document.querySelector(`.replay-pager`);e&&(e.innerHTML=Jt(),Ln(),zn())},jn=()=>{y=1},Mn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{let t=e.dataset.playerFilter===g;e.classList.toggle(`is-selected`,t),e.setAttribute(`aria-pressed`,String(t)),e.closest(`.player-card`)?.classList.toggle(`is-selected`,t),e.classList.contains(`ranking-row`)&&e.classList.toggle(`is-selected`,t)})},Nn=e=>{g=g===e?`all`:e,jn(),Mn(),An()},Pn=(e=null)=>{document.querySelectorAll(`.dropdown-filter.is-open`).forEach(t=>{t!==e&&(t.classList.remove(`is-open`),t.querySelector(`.filter-dropdown-button`)?.setAttribute(`aria-expanded`,`false`))})},Fn=()=>{let e=document.querySelector(`.filter-popover`);e&&(e.classList.remove(`is-open`),e.querySelector(`.filters-toggle`)?.setAttribute(`aria-expanded`,`false`),Pn())},In=()=>{let e=document.querySelector(`.filters`);e&&(e.outerHTML=tn(),Vn())},Ln=()=>{document.querySelectorAll(`.pagination-button[data-page]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.page);!Number.isFinite(t)||e.disabled||t===y||(y=t,An(),document.querySelector(`.replay-column`)?.scrollIntoView({block:`start`,behavior:`smooth`}))})})},Rn=()=>{b=null,T=`overview`,$()},zn=()=>{document.querySelectorAll(`.replay-row[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`a, button`)||(b=e.dataset.replayDetail,T=`overview`,$())}),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||t.target.closest(`a, button`)||(t.preventDefault(),b=e.dataset.replayDetail,T=`overview`,$())})}),document.querySelectorAll(`.replay-analysis-button[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),b=e.dataset.replayDetail,T=`overview`,$()})}),document.querySelectorAll(`[data-analysis-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{T=e.dataset.analysisTab||`overview`,$()})}),document.querySelectorAll(`[data-modal-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Rn()})}),document.querySelectorAll(`[data-theater-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&It()})}),document.querySelector(`[data-theater-play]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Rt()}),document.querySelector(`[data-theater-reset]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),C=0,S=!1,w&&=(clearInterval(w),null),$()})},Bn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.ranking-stream`)||Nn(e.dataset.playerFilter)}),e.addEventListener(`keydown`,t=>{e.tagName!==`BUTTON`&&(t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Nn(e.dataset.playerFilter)))})})},Vn=()=>{let e=document.querySelector(`#replay-search`);if(!e)return;e.addEventListener(`input`,e=>{de=e.target.value,jn(),An()});let t=document.querySelector(`.filter-popover`),n=document.querySelector(`.filters-toggle`);n?.addEventListener(`click`,e=>{e.stopPropagation();let r=t?.classList.toggle(`is-open`)||!1;n.setAttribute(`aria-expanded`,String(r)),r||Pn()}),t?.addEventListener(`click`,e=>{e.stopPropagation()}),document.querySelectorAll(`.dropdown-filter`).forEach(e=>{let t=e.querySelector(`.filter-dropdown-button`),n=e.dataset.filter;t?.addEventListener(`click`,n=>{n.stopPropagation();let r=e.classList.toggle(`is-open`);Pn(e),t.setAttribute(`aria-expanded`,String(r))}),e.querySelectorAll(`.filter-dropdown-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.value||`all`;n===`map-filter`&&(_=t),n===`hero-filter`&&(v=t),n===`matchup-filter`&&(fe=t),jn(),Pn(),In(),An()})})})},Hn=()=>{let e=document.querySelector(`.player-card-strip`);if(!e)return;let t=!1,n=0,r=0,i=!1;e.addEventListener(`pointerdown`,a=>{if(a.button===0){if(a.target.closest(`a, button, input, select, textarea, label`)){t=!1,i=!1;return}t=!0,i=!1,n=a.clientX,r=e.scrollLeft,e.classList.add(`is-dragging`),e.setPointerCapture(a.pointerId)}}),e.addEventListener(`pointermove`,a=>{if(!t)return;let o=a.clientX-n;Math.abs(o)>4&&(i=!0),e.scrollLeft=r-o});let a=n=>{t&&(t=!1,e.classList.remove(`is-dragging`),e.hasPointerCapture(n.pointerId)&&e.releasePointerCapture(n.pointerId))};e.addEventListener(`pointerup`,a),e.addEventListener(`pointercancel`,a),e.addEventListener(`click`,e=>{i&&=(e.preventDefault(),e.stopPropagation(),!1)},!0)},Un=()=>{let e=document.querySelector(`.theme-button`);e&&e.addEventListener(`click`,()=>{M=M===`dark`?`light`:`dark`,localStorage.setItem(`wc3-color-theme`,M),Ae({instant:!0}),e.setAttribute(`aria-label`,M===`dark`?`Switch to light theme`:`Switch to dark theme`)})},Wn=e=>`
...
`,Yn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{E=!E,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{E=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{D=e.dataset.infoTab||`sources`,$()})})},Xn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{O=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),b=null,T=`overview`,O=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{k=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,k),O=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{hn(k)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{gn()})},Zn=()=>{pe=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=je();document.querySelector(`#app`).innerHTML=`
    ...
    <aside class="menu-panel" id="site-menu" aria-hidden="true">
      <div class="menu-panel-inner">
        ...
        <nav class="menu-nav" aria-label="Menu">
          ...
          ${Zt(e.type===`player-earnings`?`is-active`:``)}
    ...
    <main class="page-shell">
      ${e.type===`builds`?cn():e.type===`player-earnings`?kn():e.type===`statistics`?bn():e.type===`renderer`?ln():e.type===`wc3-replay-viewer`?_n():e.type===`w3gjs`?wn():e.type===`resources`?On():sn()}
    </main>
    ${Jn()}
    ...
    ${zt()}
    ${qn()}
  `;let t=document.querySelector(`.site-header`),n=document.querySelector(`.menu-panel`),r=document.querySelector(`.menu-button`),i=()=>{n.classList.remove(`is-open`),n.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`)};r.addEventListener(`click`,()=>{let e=n.classList.toggle(`is-open`);n.setAttribute(`aria-hidden`,String(!e)),r.setAttribute(`aria-expanded`,String(e)),r.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,i)}),document.documentElement.style.setProperty(`--header-height`,`${t.offsetHeight}px`),Qn(),Vn(),Ln(),zn(),Bn(),Hn(),Un(),Yn(),Xn(),document.querySelector(`.player-card-region.with-card-intro`)&&!he&&(he=window.setTimeout(()=>{me=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!_e&&(_e=window.setTimeout(()=>{ge=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(x){It();return}if(E){E=!1,$();return}if(b){Rn();return}Fn();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-button`);!t||!n||(t.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),n.setAttribute(`aria-expanded`,`false`),n.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{Fn()}),window.addEventListener(`hashchange`,()=>{O=!1,w&&=(clearInterval(w),null),x=null,S=!1,C=0,g=`all`,_=`all`,v=`all`,fe=`all`,jn(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Pe(n)&&(b=null,T=`overview`,Lt(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{je().type===`wc3-replay-viewer`&&!O&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&je().type===`wc3-replay-viewer`&&!O&&$()});var Qn=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,Qn,{passive:!0});var $n=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Zn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([$n(`replays.json`),$n(`rankings.json`),$n(`replay-analysis.json`),$n(`map-drop-tables.json`),$n(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?m=t.value:se=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(ue=n.value),r.status===`fulfilled`&&(ce=r.value),i.status===`fulfilled`&&(le=i.value),a.status===`fulfilled`&&(h=a.value),$()})();