...
`,$e=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Ye(e.streamUrl);return`
  ...
  `:``,H=()=>m?F().flatMap(e=>je(e.slug).replays.filter(e=>Oe(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],it=()=>[...new Set(H().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),U=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,at=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,ot=e=>{let t=at(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},st=e=>{let t=ot(e);return t.length<2?``:`${U(t[0].race)} vs ${U(t[1].race)}`},ct=()=>[...new Set(H().map(st).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),lt=e=>I(e.sourceSlug,e.id),W=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],ut=e=>U(e?.raceDetected||e?.race)===`Human`,G=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},dt=(e,t=[])=>{t.forEach(t=>G(e,t.id,Number(t.count)||0))},K=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||L(e[0]).localeCompare(L(t[0]))).slice(0,t),ft=()=>{let e=new Map(W.map(e=>[e,0])),t=new Map(W.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,te=0,ne=0;return H().forEach(p=>{let re=I(p.sourceSlug,p.id);if(re?.status!==`parsed`)return;u+=1;let ie=(re.players||[]).filter(ut);ie.length&&(G(s,p.map||re.map?.name||`Unknown map`),G(c,st(p)||`Unknown matchup`),ie.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{W.includes(t.id)&&G(e,t.id)}),W.includes(c[0]?.id)&&G(t,c[0].id),(s.order?.items||[]).forEach(e=>{Le(e.id)?(G(n,e.id),te+=N[e.id]||0):(G(r,e.id),N[e.id]!==null&&(ne+=1))}),dt(i,s.topUnits),dt(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>G(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:te,unknownBoughtPrices:ne}},pt=e=>(lt(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),mt=()=>{let e=new Map;return H().forEach(t=>{pt(t).forEach(t=>{e.has(t)||e.set(t,Fe(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},ht=e=>ot(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),gt=r=>{let i=String(r||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:e,human:e,o:n,orc:n,u:te,undead:te,n:t,nightelf:t};return a[i]?a[i]:null},_t=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>U(t.race||t.raceDetected)===U(e.race));return i.length===1?i[0]:n[t]||null},vt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${P(t)}</span>
      <strong>${P(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Je(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},q=(e,t=[])=>ot(e).map((e,n)=>{let r=gt(e.race),i=_t(e,n,t);return`
...
`,wt=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=St,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
  ...
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},At=(e=[])=>e.length?e.map(e=>`<span>${P(L(e.id))} × ${P(e.count)}</span>`).join(``):`<span>None parsed</span>`,jt=e=>{let t=e?.players||[];return t.length?`
    <div class="actions-grid">
      ${t.map(e=>`
            <div class="actions-card">
              <strong>${P(e.name||`Unknown`)}</strong>
              <div class="action-counts">
                ${Object.entries(e.actions||{}).map(([e,t])=>`<span><em>${P(e)}</em>${P(z(t||0))}</span>`).join(``)}
              </div>
      ...
      <section class="replay-theater" role="dialog" aria-modal="true" aria-label="Replay theater">
        ...
        <div class="replay-theater-body">
          <section class="replay-theater-map">
            ${e.mapImage?.localPath?`<img src="${P(Ae(e.mapImage.localPath))}" alt="${P(e.map)} map thumbnail" />`:`<div class="map-placeholder" aria-hidden="true">${P(e.mapShort||`WC3`)}</div>`}
            <div class="replay-theater-map-overlay">
              <span>${P(t?.version?`Patch ${t.version}`:e.version||`Patch unknown`)}</span>
              <strong>${P(t?.durationLabel||e.durationLabel||Y(i))}</strong>
          ...
          <section class="replay-theater-controls">
            <div class="replay-theater-clock">
              <strong>${P(Y(C))}</strong>
              <span>${P(t?.durationLabel||Y(i))}</span>
            </div>
  ...
  <div class="replay-theater-event">
    <span>${P(e.time||Y(e.ms||0))}</span>
    <div>
      <strong>${P(e.type===`Chat`?e.playerName:L(e.id))}</strong>
      <small>${P(e.type===`Chat`?e.detail:`${e.playerName||`Player`} · ${e.detail||e.type}`)}</small>
    </div>
  </div>
`,Bt=e=>de?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,ht(e)].filter(Boolean).join(` `).toLowerCase().includes(de.toLowerCase()):!0,Vt=e=>{let t=g===`all`||e.sourceSlug===g,n=fe===`all`||e.map===fe,r=_===`all`||pt(e).includes(_),i=v===`all`||st(e)===v;return t&&n&&r&&i&&Bt(e)},Ht=()=>H().filter(Vt).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Ut=e=>{y=Math.min(Math.max(y,1),Math.max(e,1))},Wt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Gt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(y-1)*M+1,r=Math.min(y*M,e),i=Wt(y,t),a=0;return`
  ...
  `},Kt=e=>se?`
    ...
    `:m?e.length?e.map(e=>{let t=e.createdAt?Ce.format(new Date(e.createdAt)):`Unknown date`,n=I(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
        <article class="replay-row" data-replay-detail="${P(`${e.sourceSlug}:${e.id}`)}" role="button" tabindex="0" aria-label="Analyze ${P(e.map)} replay">
          <div class="replay-map">
            ${e.mapImage?.localPath?`<img src="${P(Ae(e.mapImage.localPath))}" alt="${P(e.map)} map thumbnail" loading="lazy" />`:`<div class="map-placeholder" aria-hidden="true">${P(e.mapShort||`WC3`)}</div>`}
            <div>
              <span>${P(t)}</span>
    ...
    `;let e=Ht(),t=Math.ceil(e.length/M);Ut(t);let n=(y-1)*M;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${Kt(e.slice(n,n+M))}
    </section>
...
`,$t=({id:e,label:t,value:n,options:r})=>{let i=r.find(e=>e.value===n)||r[0];return`
    <div class="filter-control dropdown-filter" data-filter="${P(e)}">
      ...
      <button class="filter-dropdown-button" id="${P(e)}" type="button" aria-haspopup="listbox" aria-expanded="false" aria-labelledby="${P(e)}-label ${P(e)}">
        <span>${i.iconId?R(i.iconId,i.label):``}${P(i.label)}</span>
      ...
      <div class="filter-dropdown-menu" role="listbox" aria-labelledby="${P(e)}-label">
        ${r.map(e=>`
              <button class="filter-dropdown-option ${e.value===n?`is-selected`:``}" type="button" role="option" aria-selected="${e.value===n}" data-value="${P(e.value)}">
                <span>${e.iconId?R(e.iconId,e.label):``}${P(e.label)}</span>
              </button>
            `).join(``)}
      </div>
    </div>
  ...
  <section class="filters" aria-label="Replay filters">
    <div class="quick-filter">
      ...
      <input id="replay-search" type="search" value="${P(de)}" placeholder="Search map, opponent, patch..." />
    </div>
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${$t({id:`map-filter`,label:`Map`,value:fe,options:[{value:`all`,label:`All maps`},...it().map(e=>({value:e,label:e}))]})}
...
`,tn=e=>{if(!e.streamUrl)return`<span class="ranking-stream is-empty" aria-hidden="true"></span>`;let t=Ye(e.streamUrl);return`
  ...
  <aside class="rankings-sidebar" aria-label="Human rankings">
    <div class="rankings-header">
      ...
      <small>Top ${F().length}</small>
    </div>
    <div class="rankings-list">
      ${F().map(e=>`
            <div class="ranking-row ${g===e.slug?`is-selected`:``}" data-player-filter="${P(e.slug)}" role="button" tabindex="0" aria-pressed="${g===e.slug}">
...
`,rn=e=>e?`
  ...
  <section class="player-card-region ${me?``:`with-card-intro`}" aria-label="Top Human player cards">
    <div class="player-card-strip">
      ${F().map((e,t)=>{let n=[`is-gold`,`is-silver`,`is-bronze`][t]||``,r=t*760,i=t<3?r+520:0,a=Me(e);return`
...
`,cn=()=>{let e=H().find(e=>e.mapImage?.localPath&&I(e.sourceSlug,e.id)?.status===`parsed`)||H()[0],t=e?I(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ce.format(new Date(e.createdAt)):`Unknown date`;return`
    <section class="renderer-page" aria-label="Renderer test">
      ...
      <div class="renderer-layout">
        <section class="renderer-stage" aria-label="Renderer preview">
          ${e?.mapImage?.localPath?`<img src="${P(Ae(e.mapImage.localPath))}" alt="${P(e.map)} map thumbnail" loading="lazy" />`:`<div class="renderer-placeholder">WC3</div>`}
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                ...
                <div class="renderer-actions">
                  ...
                  ${d?`<button class="warsmash-start-button" type="button" data-warsmash-watch="${P(i)}">${V()}<span>${u?`Start Engine Test`:`Start Reforged Engine Test`}</span></button>`:`<button type="button" data-warsmash-auto-stage ${j.running||!u?`disabled`:``}>${j.running?`Staging Detected Install...`:u?`Auto Stage Detected Install`:`Reforged Support Pending`}</button>`}
...
`,_n=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${R(e)}${P(Pe(L(e)).replace(` (${e})`,``))}</span>
              <strong>${P(z(t))}</strong>
            </div>
          ...
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,yn=()=>{let e=ft(),t=e.humanGames||1,n=K(e.boughtItems,10),r=K(e.foundItems,8);return`
    <section class="statistics-page" aria-label="Replay statistics">
      ...
      <div class="statistics-metrics" aria-label="Replay data summary">
        ${Z(`Parsed replays`,z(e.parsedReplays),`Allowed map pool only`)}
        ${Z(`Human player-games`,z(e.humanGames),`One Human player in most rows`)}
        ${Z(`Average Human APM`,z(e.averageApm),`Parsed APM samples`)}
        ${Z(`Item actions`,z(e.totalItemActions),`Buys, uses, and item commands`)}
      </div>
      ...
      <section class="statistics-panel statistics-hero-panel">
        <div class="statistics-panel-header">
          ...
          <span>${P(z(e.humanGames))} Human games</span>
        </div>
        <div class="statistics-hero-grid">
          ${W.map(n=>{let r=e.heroPicks.get(n)||0,i=e.firstHeroPicks.get(n)||0,a=e.humanGames?r/t*100:0,o=e.humanGames?i/t*100:0;return`
                <article class="statistics-hero-card">
                  ...
                  <dl>
                    <div><dt>Picked</dt><dd>${P(z(r))}</dd></div>
                    <div><dt>Pick rate</dt><dd>${P(Ge(a))}</dd></div>
                    <div><dt>First hero</dt><dd>${P(z(i))}</dd></div>
      ...
      <div class="statistics-grid">
        ...
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(z([...e.foundItems.values()].reduce((e,t)=>e+t,0)))}</span>
          </div>
          <div class="statistics-list">
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(K(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(K(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${_n(K(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
  ...
  `},bn=()=>{let e=H(),t=e.map(e=>({replay:e,analysis:I(e.sourceSlug,e.id)})).filter(e=>e.analysis?.status===`parsed`),n=new Map,r=new Map,i=new Map,a=0,o=0,s=0,c=0,l=0;return t.forEach(({replay:e,analysis:t})=>{G(n,t.version||e.version||`Unknown`),G(r,e.map||t.map?.file||`Unknown map`),a+=t.chat?.length||0,s+=t.chat?.length||0,(t.players||[]).forEach(e=>{o+=1,Number.isFinite(e.apm)&&(c+=e.apm,l+=1),Object.entries(e.actions||{}).forEach(([e,t])=>{G(i,e,t||0)}),s+=(e.heroes||[]).length,s+=(e.order?.buildings||[]).length,s+=(e.order?.units||[]).length,s+=(e.order?.upgrades||[]).length,s+=(e.order?.items||[]).length})}),{totalReplays:e.length,parsedReplays:t.length,parsedPlayers:o,eventCount:s,chatMessages:a,averageApm:l?Math.round(c/l):0,versionCounts:n,mapCounts:r,actionCounts:i}},xn=e=>e.length?e.map(([e,t])=>`
          ...
          `).join(``):`<div class="analysis-empty"><strong>No timeline events parsed yet</strong></div>`,Cn=()=>{let e=bn(),t=H().find(e=>String(e.id)===`137397`)||H().find(e=>I(e.sourceSlug,e.id)?.status===`parsed`)||H()[0],n=t?I(t.sourceSlug,t.id):null,r=n?.status===`parsed`?n.players:[],i=Pt(n),a=i.slice(0,12),o=i.filter(e=>(e.ms||0)>=18e4).slice(0,12),s=K(e.actionCounts,12),c=K(e.versionCounts,4),l=K(e.mapCounts,6);return`
...
const result = await parser.parse("replay.w3g");</code></pre>
      ...
      <div class="statistics-metrics w3gjs-metrics" aria-label="w3gjs parsed data summary">
        ${Z(`Replay files`,z(e.totalReplays),`From the local replay cache`)}
        ${Z(`Parsed replays`,z(e.parsedReplays),`Built from w3gjs output`)}
        ${Z(`Parsed players`,z(e.parsedPlayers),`Names, races, teams, APM`)}
        ${Z(`Timeline events`,z(e.eventCount),`Builds, units, items, heroes, chat`)}
      </div>
      ...
      <div class="w3gjs-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(z(e.averageApm))} avg APM</span>
          </div>
          ${bt({...n,players:r})}
        </section>
...
`,Kn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{E=!E,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{E=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{D=e.dataset.infoTab||`sources`,$()})})},qn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{O=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),b=null,T=`overview`,O=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{k=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,k),O=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{mn(k)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{hn()})},Jn=()=>{pe=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=ke();document.querySelector(`#app`).innerHTML=`
    ...
    ${Rt()}
    ${Wn()}
  `;let t=document.querySelector(`.site-header`),n=document.querySelector(`.menu-panel`),r=document.querySelector(`.menu-button`),i=()=>{n.classList.remove(`is-open`),n.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`)};r.addEventListener(`click`,()=>{let e=n.classList.toggle(`is-open`);n.setAttribute(`aria-hidden`,String(!e)),r.setAttribute(`aria-expanded`,String(e)),r.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,i)}),document.documentElement.style.setProperty(`--header-height`,`${t.offsetHeight}px`),Yn(),Bn(),In(),Rn(),zn(),Vn(),Kn(),qn(),document.querySelector(`.player-card-region.with-card-intro`)&&!he&&(he=window.setTimeout(()=>{me=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!_e&&(_e=window.setTimeout(()=>{ge=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(x){Ft();return}if(E){E=!1,$();return}if(b){Ln();return}Pn();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-button`);!t||!n||(t.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),n.setAttribute(`aria-expanded`,`false`),n.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{Pn()}),window.addEventListener(`hashchange`,()=>{O=!1,w&&=(clearInterval(w),null),x=null,S=!1,C=0,g=`all`,fe=`all`,_=`all`,v=`all`,An(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Ne(n)&&(b=null,T=`overview`,It(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{ke().type===`wc3-replay-viewer`&&!O&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&ke().type===`wc3-replay-viewer`&&!O&&$()});var Yn=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,Yn,{passive:!0});var Xn=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Jn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([Xn(`replays.json`),Xn(`rankings.json`),Xn(`replay-analysis.json`),Xn(`map-drop-tables.json`),Xn(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?m=t.value:se=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(ue=n.value),r.status===`fulfilled`&&(ce=r.value),i.status===`fulfilled`&&(le=i.value),a.status===`fulfilled`&&(h=a.value),$()})();