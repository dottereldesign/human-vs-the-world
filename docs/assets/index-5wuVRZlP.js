...
`,Qe=({key:e,short:t,icon:n},r)=>`
  <span class="service-icon service-icon-${F(e)}" aria-hidden="true">
    ${n?`<img src="${n}" alt="" loading="lazy" />`:F(t)}
  </span>
  <span class="action-label">${F(r)}</span>
`,$e=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Ye(e.streamUrl);return`
    <a href="${F(e.streamUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${F(e.name)} ${F(t.label)} stream">
      ${Qe(t,t.label)}
    </a>
  `},et=e=>e.liquipediaUrl?`
    <a href="${F(e.liquipediaUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${F(e.name)} on Liquipedia">
      ${Qe({key:`liquipedia`,short:`LP`,icon:d},`Liquipedia`)}
    </a>
  `:`<span class="is-disabled"><span class="service-icon service-icon-liquipedia" aria-hidden="true"><img src="${d}" alt="" loading="lazy" /></span><span class="action-label">Liquipedia</span></span>`,tt=e=>e.realName?`<p class="player-card-real-name">${F(e.realName)}</p>`:``,nt=e=>{let t={chaemiko:r,fortitude:o,hawk:s,infi:l,leon:u,sok:f}[e.slug];return t?`<img class="player-card-image" src="${t}" alt="${F(e.name)}" loading="lazy" />`:`<span>${F(e.flag)}</span>`},rt=e=>e.slug===`infi`?`
  ...
  `:``,U=()=>oe?I().flatMap(e=>Me(e.slug).replays.filter(e=>Oe(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],it=()=>[...new Set(U().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),W=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,at=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,ot=e=>{let t=at(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},st=e=>{let t=ot(e);return t.length<2?``:`${W(t[0].race)} vs ${W(t[1].race)}`},ct=()=>[...new Set(U().map(st).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),lt=e=>L(e.sourceSlug,e.id),ut=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],dt=e=>W(e?.raceDetected||e?.race)===`Human`,G=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},ft=(e,t=[])=>{t.forEach(t=>G(e,t.id,Number(t.count)||0))},K=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||R(e[0]).localeCompare(R(t[0]))).slice(0,t),pt=()=>{let e=new Map(ut.map(e=>[e,0])),t=new Map(ut.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,p=0,te=0;return U().forEach(m=>{let ne=L(m.sourceSlug,m.id);if(ne?.status!==`parsed`)return;u+=1;let re=(ne.players||[]).filter(dt);re.length&&(G(s,m.map||ne.map?.name||`Unknown map`),G(c,st(m)||`Unknown matchup`),re.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{ut.includes(t.id)&&G(e,t.id)}),ut.includes(c[0]?.id)&&G(t,c[0].id),(s.order?.items||[]).forEach(e=>{Le(e.id)?(G(n,e.id),p+=P[e.id]||0):(G(r,e.id),P[e.id]!==null&&(te+=1))}),ft(i,s.topUnits),ft(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>G(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:p,unknownBoughtPrices:te}},mt=e=>(lt(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),ht=()=>{let e=new Map;return U().forEach(t=>{mt(t).forEach(t=>{e.has(t)||e.set(t,R(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},gt=e=>ot(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),_t=r=>{let i=String(r||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:e,human:e,o:n,orc:n,u:p,undead:p,n:t,nightelf:t};return a[i]?a[i]:null},vt=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>W(t.race||t.raceDetected)===W(e.race));return i.length===1?i[0]:n[t]||null},yt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${F(t)}</span>
      <strong>${F(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Je(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},q=(e,t=[])=>ot(e).map((e,n)=>{let r=_t(e.race),i=vt(e,n,t);return`
        <span class="matchup-player" tabindex="0">
          ${r?`<img src="${r}" alt="${F(e.race)}" loading="lazy" />`:``}
          <span>${F(e.name)}</span>
          ${yt(i)}
        </span>
      `}).join(`<span class="versus">vs</span>`),J=(e,t)=>`
  <div class="analysis-pill">
    <span>${F(e)}</span>
    <strong>${F(t??`Unknown`)}</strong>
  </div>
`,bt=(e,t)=>!t||t.status!==`parsed`?`
      <div class="analysis-empty">
        <strong>Analysis unavailable</strong>
        <span>${F(t?.reason||`Run npm run analyze:replays to parse this replay.`)}</span>
      </div>
    `:`
    <div class="analysis-overview">
      <div class="analysis-pill-grid">
        ${J(`Map`,e.map)}
        ${J(`Duration`,t.durationLabel)}
        ${J(`Matchup`,t.matchup)}
        ${J(`Patch`,t.version)}
        ${J(`Game`,t.gameName)}
        ${J(`Chat`,`${t.chatCount||0} messages`)}
      </div>
      <div class="analysis-player-grid">
        ${(t.players||[]).map(e=>`
              <div class="analysis-player-card">
                <span class="analysis-player-meta">${F(e.raceDetected||e.race||`Unknown race`)}</span>
                <strong>${F(e.name||`Unknown`)}</strong>
                <div class="analysis-stat-line">
                  <span>${F(e.apm||0)} APM</span>
                  <span>${F(e.heroCount||0)} heroes</span>
                </div>
                <div class="analysis-heroes">
                  ${(e.heroes||[]).map(e=>Je(e)).join(``)}
                </div>
              </div>
            `).join(``)}
      </div>
    </div>
  `,xt=e=>{let t=(e?.players||[]).map(e=>({...e,chartApmTimeline:(e.apmTimeline||[]).slice(1,-1)})),n=Math.max(...t.flatMap(e=>e.chartApmTimeline||[]),1);if(!t.some(e=>e.chartApmTimeline?.length))return`<div class="analysis-empty"><strong>No APM timeline</strong><span>Run the updated replay analyzer to generate APM buckets.</span></div>`;let r={top:26,right:28,bottom:36,left:44},i=720-r.left-r.right,a=280-r.top-r.bottom,o=Math.max(100,Math.ceil(n/50)*50),s=Math.max(...t.map(e=>e.chartApmTimeline?.length||0),1),c=(e,t,n)=>({x:r.left+(n<=1?i/2:t/(n-1)*i),y:r.top+a-Math.min(e,o)/o*a}),l=e=>e.length?e.length===1?`M ${e[0].x} ${e[0].y}`:e.reduce((t,n,r)=>{if(r===0)return`M ${n.x} ${n.y}`;let i=e[r-1],a=(i.x+n.x)/2,o=(i.y+n.y)/2;return`${t} Q ${i.x} ${i.y} ${a} ${o}`},``)+` T ${e[e.length-1].x} ${e[e.length-1].y}`:``,u=[o,Math.round(o*.75),Math.round(o*.5),Math.round(o*.25),0],d=[0,Math.floor((s-1)/2),s-1].filter((e,t,n)=>n.indexOf(e)===t);return`
    <div class="apm-chart">
        ...
        <div class="apm-legend">
          ${t.map((e,t)=>`
                <span class="apm-legend-item apm-series-${t+1}">
                  ...
                  ${F(e.name||`Unknown`)} · ${F(e.apm||0)} avg
                </span>
              `).join(``)}
        </div>
      </div>
                ...
                <g class="apm-series apm-series-${t+1}">
                  ...
                  ${r.map((t,r)=>`
                        <circle class="apm-point" cx="${t.x}" cy="${t.y}" r="3.2">
                          <title>${F(e.name||`Player`)} · ${F(n[r])} APM · ${r}m</title>
                        </circle>
                      `).join(``)}
                </g>
              `}).join(``)}
      ...
      <div class="apm-summary-grid">
        ${t.map(e=>`
            <div class="apm-summary-card">
              <span>${F(e.name||`Unknown`)}</span>
              <strong>${F(e.apm||0)} APM</strong>
              <em>Peak ${F(Math.max(...e.chartApmTimeline||[0]))} · Low ${F(Math.min(...e.chartApmTimeline||[0]))}</em>
            </div>
          `).join(``)}
      </div>
    </div>
  `},St=(e,t)=>t.flatMap(([t,n])=>(n||[]).map(n=>({...n,type:t,playerName:e.name,race:e.raceDetected||e.race}))).sort((e,t)=>(e.ms||0)-(t.ms||0)),Ct=(e,t=`${e.type}`)=>`
  <div class="build-order-row">
    <span>${F(e.time||`0:00`)}</span>
    <div class="timeline-object">
      ${z(e.id)}
      <strong>${F(R(e.id))}</strong>
    </div>
    <em>${F(t)}</em>
  </div>
`,wt=(e,t)=>`
  <div class="build-order-row item-found-row">
    ...
    <em>
      ...
      <small>${F(Ke(e.id,t))}</small>
      <small>${F(qe(e.id,t))}</small>
    </em>
  </div>
`,Tt=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=Ct,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
    <div class="timeline-columns">
      ${o.map(e=>{let t=r(e),n=a?a(e,t):``;return`
            <section class="timeline-player-column">
              <header>
                <span>${F(e.raceDetected||e.race||`Unknown race`)}</span>
                <strong>${F(e.name||`Unknown`)}</strong>
                ${n}
              </header>
  ...
  `:`<div class="analysis-empty"><strong>${F(t)} unavailable</strong><span>${F(n)}</span></div>`},Et=e=>Tt({analysis:e,title:`Build order`,emptyMessage:`Run the updated replay analyzer to generate unit and building timeline data.`,getItems:e=>St(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units]])}),Dt=e=>Tt({analysis:e,title:`Upgrades`,emptyMessage:`No upgrades were parsed for this replay.`,getItems:e=>Be(St(e,[[`Upgrade`,e.order?.upgrades]])),renderRow:e=>Ct(e,Number.isFinite(e.goldCost)?Re(e.goldCost):`Cost unknown`),renderSummary:(e,t)=>{let n=Ve(t,e=>e.goldCost);return He({label:`Known gold spent`,gold:n.gold,unknown:n.unknown})}}),Ot=e=>Tt({analysis:e,title:`Items bought`,emptyMessage:`No shop-price items were parsed for this replay.`,getItems:e=>St(e,[[`Item`,e.order?.items]]).filter(e=>Le(e.id)),renderSummary:(e,t)=>{let n=Ve(t,e=>P[e.id]);return He({label:`Total spent`,gold:n.gold,unknown:n.unknown})},renderRow:t=>{let n=We(t.id,e);return Ct(t,n?`${Ie(t.id)} · ${Ge(n.chance)} ${n.groupLabel} drop`:Ie(t.id))}}),kt=e=>Tt({analysis:e,title:`Items found`,emptyMessage:`No no-shop-price items were parsed for this replay.`,getItems:e=>St(e,[[`Item`,e.order?.items]]).filter(e=>!Le(e.id)),renderRow:t=>wt(t,e)}),At=e=>{let t=e?.chat||[];return t.length?`
    <div class="chatlog-list">
      ${t.map(e=>`
            <div class="chatlog-row">
              <span>${F(e.time||`0:00`)}</span>
              <strong>${F(e.playerName||`Unknown`)}</strong>
              <p>${F(e.message||``)}</p>
            </div>
          `).join(``)}
    </div>
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},jt=(e=[])=>e.length?e.map(e=>`<span>${F(R(e.id))} × ${F(e.count)}</span>`).join(``):`<span>None parsed</span>`,Mt=e=>{let t=e?.players||[];return t.length?`
    <div class="actions-grid">
      ${t.map(e=>`
            <div class="actions-card">
              <strong>${F(e.name||`Unknown`)}</strong>
              <div class="action-counts">
                ${Object.entries(e.actions||{}).map(([e,t])=>`<span><em>${F(e)}</em>${F(B(t||0))}</span>`).join(``)}
              </div>
  ...
  `:`<div class="analysis-empty"><strong>No action data</strong><span>Run the updated replay analyzer to generate action data.</span></div>`},Nt=(e,t)=>E===`apm`?xt(t):E===`build`?Et(t):E===`upgrades`?Dt(t):E===`items-bought`?Ot(t):E===`items-found`?kt(t):E===`chat`?At(t):E===`actions`?Mt(t):bt(e,t),Pt=()=>{let e=Pe(x);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=ve.some(e=>e.id===E)?E:`overview`;return`
    <div class="analysis-modal-shell" role="presentation" data-modal-close>
      <section class="analysis-modal" role="dialog" aria-modal="true" aria-label="Replay analysis">
        <header class="analysis-modal-header">
          <div>
            <span>${F(e.sourceFlag)} ${F(e.sourceName)}</span>
            <h2>${F(e.map)}</h2>
            <div class="analysis-modal-matchup">${q(e,n)}</div>
          </div>
        ...
        <nav class="analysis-tabs" aria-label="Replay analysis tabs">
          ${ve.map(e=>`
                <button class="${e.id===r?`is-active`:``}" type="button" data-analysis-tab="${F(e.id)}">
                  ${F(e.label)}
                </button>
              `).join(``)}
        </nav>
  ...
  `},Y=(e=0)=>{let t=Math.max(0,Math.floor(Number(e||0)/1e3)),n=Math.floor(t/60),r=t%60;return`${n}:${String(r).padStart(2,`0`)}`},Ft=e=>{if(!e||e.status!==`parsed`)return[];let t=(e.players||[]).flatMap(e=>[...(e.heroes||[]).map(t=>({id:t.id,ms:0,time:`0:00`,type:`Hero`,playerName:e.name,race:e.raceDetected||e.race,detail:t.level?`Level ${t.level}`:`Hero`})),...St(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units],[`Upgrade`,Be(e.order?.upgrades||[])],[`Item`,e.order?.items]]).map(t=>({...t,playerName:e.name,detail:t.type===`Item`?Ie(t.id):t.type===`Upgrade`&&Number.isFinite(t.goldCost)?Re(t.goldCost):t.type}))]),n=(e.chat||[]).map(e=>({id:`chat`,ms:e.timeMS||0,time:e.time||Y(e.timeMS||0),type:`Chat`,playerName:e.playerName||`Observer`,detail:e.message||``}));return[...t,...n].sort((e,t)=>(e.ms||0)-(t.ms||0))},It=()=>{S=null,C=!1,w=0,T&&=(clearInterval(T),null),$()},Lt=e=>{S=e,w=0,C=!1,T&&=(clearInterval(T),null),$()},Rt=()=>{let e=Pe(S),t=(e?L(e.sourceSlug,e.id):null)?.duration||0;if(C=!C,T&&=(clearInterval(T),null),!C){$();return}T=setInterval(()=>{w+=1e3,t&&w>=t&&(w=t,C=!1,clearInterval(T),T=null),$()},1e3),$()},zt=()=>{let e=Pe(S);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=Ft(t),i=t?.duration||e.duration||Math.max(...r.map(e=>e.ms||0),0),a=i?Math.min(100,Math.max(0,w/i*100)):0,o=r.filter(e=>(e.ms||0)<=w).slice(-18),s=r.filter(e=>(e.ms||0)>w).slice(0,18);return`
      ...
      <section class="replay-theater" role="dialog" aria-modal="true" aria-label="Replay theater">
        <header class="replay-theater-header">
          <div>
            ...
            <h2>${F(e.sourceFlag)} ${F(e.sourceName)} on ${F(e.map)}</h2>
            <div class="analysis-modal-matchup">${q(e,n)}</div>
          </div>
        ...
        <div class="replay-theater-body">
          <section class="replay-theater-map">
            ${e.mapImage?.localPath?`<img src="${F(e.mapImage.localPath)}" alt="${F(e.map)} map thumbnail" />`:`<div class="map-placeholder" aria-hidden="true">${F(e.mapShort||`WC3`)}</div>`}
            <div class="replay-theater-map-overlay">
              <span>${F(t?.version?`Patch ${t.version}`:e.version||`Patch unknown`)}</span>
              <strong>${F(t?.durationLabel||e.durationLabel||Y(i))}</strong>
              <small>${F(e.localPath||t?.sourceFile||``)}</small>
            </div>
          </section>
          <section class="replay-theater-controls">
            <div class="replay-theater-clock">
              <strong>${F(Y(w))}</strong>
              <span>${F(t?.durationLabel||Y(i))}</span>
            </div>
            ...
            <div class="replay-theater-buttons">
              <button type="button" data-theater-play>${C?`Pause`:`Play parsed timeline`}</button>
              <button type="button" data-theater-reset>Restart</button>
              <button class="replay-analysis-button" type="button" data-replay-detail="${F(`${e.sourceSlug}:${e.id}`)}">Open Analysis Tabs</button>
            </div>
          </section>
          <div class="replay-theater-columns">
            <section>
              ...
              <div class="replay-theater-event-list">
                ${o.length?o.map(Bt).join(``):`<div class="timeline-empty">Press play to advance the parsed replay timeline.</div>`}
              </div>
            </section>
            <section>
              <h3>Next Events</h3>
              <div class="replay-theater-event-list">
                ${s.length?s.map(Bt).join(``):`<div class="timeline-empty">No more parsed events.</div>`}
              </div>
            </section>
          </div>
        </div>
  ...
  <div class="replay-theater-event">
    <span>${F(e.time||Y(e.ms||0))}</span>
    <div>
      <strong>${F(e.type===`Chat`?e.playerName:R(e.id))}</strong>
      <small>${F(e.type===`Chat`?e.detail:`${e.playerName||`Player`} · ${e.detail||e.type}`)}</small>
    </div>
  </div>
`,Vt=e=>de?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,gt(e)].filter(Boolean).join(` `).toLowerCase().includes(de.toLowerCase()):!0,Ht=e=>{let t=g===`all`||e.sourceSlug===g,n=_===`all`||e.map===_,r=v===`all`||mt(e).includes(v),i=y===`all`||st(e)===y;return t&&n&&r&&i&&Vt(e)},Ut=()=>U().filter(Ht).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Wt=e=>{b=Math.min(Math.max(b,1),Math.max(e,1))},Gt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Kt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(b-1)*_e+1,r=Math.min(b*_e,e),i=Gt(b,t),a=0;return`
    <nav class="replay-pagination" aria-label="Replay pagination">
      <span class="pagination-summary">${n}-${r} of ${e}</span>
      <div class="pagination-controls">
        <button class="pagination-button" type="button" data-page="${b-1}" ${b===1?`disabled`:``}>Prev</button>
        ${i.map(e=>{let t=a&&e-a>1?`<span class="pagination-gap">...</span>`:``;return a=e,`${t}<button class="pagination-button ${e===b?`is-active`:``}" type="button" data-page="${e}" ${e===b?`aria-current="page"`:``}>${e}</button>`}).join(``)}
        <button class="pagination-button" type="button" data-page="${b+1}" ${b===t?`disabled`:``}>Next</button>
      </div>
    </nav>
  `},qt=e=>se?`
    ...
    `:oe?e.length?e.map(e=>{let t=e.createdAt?Ce.format(new Date(e.createdAt)):`Unknown date`,n=L(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
        <article class="replay-row" data-replay-detail="${F(`${e.sourceSlug}:${e.id}`)}" role="button" tabindex="0" aria-label="Analyze ${F(e.map)} replay">
          <div class="replay-map">
            ${e.mapImage?.localPath?`<img src="${F(e.mapImage.localPath)}" alt="${F(e.map)} map thumbnail" loading="lazy" />`:`<div class="map-placeholder" aria-hidden="true">${F(e.mapShort||`WC3`)}</div>`}
            <div>
              <span>${F(t)}</span>
              <strong>${F(e.map)}</strong>
              <small>${F(e.sourceFlag)} ${F(e.sourceName)}</small>
            </div>
          </div>
          <div class="replay-match">
            <div class="replay-matchup">${q(e,r)}</div>
            <div class="replay-actions">
              ${i?`<span class="replay-duration">${F(i)}</span>`:``}
              <a href="${e.url}" target="_blank" rel="noreferrer">Info</a>
              <a href="${e.downloadUrl}" target="_blank" rel="noreferrer">Download</a>
            </div>
          </div>
          <button class="replay-analysis-button" type="button" data-replay-detail="${F(`${e.sourceSlug}:${e.id}`)}">
            ${H()}
            <span>Analyze</span>
          </button>
        </article>
    ...
    `;let e=Ut(),t=Math.ceil(e.length/_e);Wt(t);let n=(b-1)*_e;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${qt(e.slice(n,n+_e))}
    </section>
    ${e.length?Kt(e.length,Math.max(t,1)):``}
  `},Yt=(e=``)=>`
...
`,Zt=(e=``)=>`
  <a class="${e}" href="#/human-money" data-page-link>
    ${V(`coins`)}
...
`,en=({id:e,label:t,value:n,options:r})=>{let i=r.find(e=>e.value===n)||r[0];return`
    <div class="filter-control dropdown-filter" data-filter="${F(e)}">
      <span class="filter-label" id="${F(e)}-label">${F(t)}</span>
      <button class="filter-dropdown-button" id="${F(e)}" type="button" aria-haspopup="listbox" aria-expanded="false" aria-labelledby="${F(e)}-label ${F(e)}">
        <span>${i.iconId?z(i.iconId,i.label):``}${F(i.label)}</span>
        <span class="dropdown-chevron" aria-hidden="true"></span>
      </button>
      <div class="filter-dropdown-menu" role="listbox" aria-labelledby="${F(e)}-label">
        ${r.map(e=>`
              <button class="filter-dropdown-option ${e.value===n?`is-selected`:``}" type="button" role="option" aria-selected="${e.value===n}" data-value="${F(e.value)}">
                <span>${e.iconId?z(e.iconId,e.label):``}${F(e.label)}</span>
              </button>
            `).join(``)}
      </div>
    </div>
  ...
  <section class="filters" aria-label="Replay filters">
    <div class="quick-filter">
      ...
      <input id="replay-search" type="search" value="${F(de)}" placeholder="Search map, opponent, patch..." />
    </div>
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${en({id:`map-filter`,label:`Map`,value:_,options:[{value:`all`,label:`All maps`},...it().map(e=>({value:e,label:e}))]})}
        ${en({id:`hero-filter`,label:`Starting hero`,value:v,options:[{value:`all`,label:`All heroes`},...ht()]})}
        ${en({id:`matchup-filter`,label:`Matchup`,value:y,options:[{value:`all`,label:`All matchups`},...ct().map(e=>({value:e,label:e}))]})}
      </div>
    </div>
  </section>
`,nn=e=>{if(!e.streamUrl)return`<span class="ranking-stream is-empty" aria-hidden="true"></span>`;let t=Ye(e.streamUrl);return`
    <a class="ranking-stream" href="${F(e.streamUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${F(e.name)} ${F(t.label)} stream">
      <span class="service-icon service-icon-${F(t.key)}" aria-hidden="true">
        ${t.icon?`<img src="${t.icon}" alt="" loading="lazy" />`:F(t.short)}
      </span>
    </a>
  `},rn=()=>`
  <aside class="rankings-sidebar" aria-label="Human rankings">
    ...
    <div class="rankings-list">
      ${I().map(e=>`
            <div class="ranking-row ${g===e.slug?`is-selected`:``}" data-player-filter="${F(e.slug)}" role="button" tabindex="0" aria-pressed="${g===e.slug}">
...
`,an=e=>e?`
  ...
  `:``,on=()=>`
  <section class="player-card-region ${pe?``:`with-card-intro`}" aria-label="Top Human player cards">
    <div class="player-card-strip">
      ${I().map((e,t)=>{let n=[`is-gold`,`is-silver`,`is-bronze`][t]||``,r=t*760,i=t<3?r+520:0,a=Ne(e);return`
              ...
              <div class="player-card-body">
                <div class="player-card-summary">
                  ...
                  <h2 class="player-card-name">
                    <span aria-hidden="true">${F(e.flag)}</span>
                    <span>${F(e.name)}</span>
                  </h2>
                  ${a?`<p class="player-card-earnings">${F(a.earningsLabel)}</p>`:``}
                  ${tt(e)}
                </div>
                <div class="player-card-actions">
                  ${$e(e)}
                  ${et(e)}
                  <a class="download-pack" href="${F(e.replayPackUrl||`replay-packs/${e.slug}-2026-human-wins.zip`)}" download aria-label="Download ${F(e.name)} 2026 Human wins replay pack">
                    ${Xe()}
...
`,ln=()=>{let e=U().find(e=>e.mapImage?.localPath&&L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ce.format(new Date(e.createdAt)):`Unknown date`;return`
    <section class="renderer-page" aria-label="Renderer test">
      ...
      <div class="renderer-layout">
        <section class="renderer-stage" aria-label="Renderer preview">
          ${e?.mapImage?.localPath?`<img src="${F(e.mapImage.localPath)}" alt="${F(e.map)} map thumbnail" loading="lazy" />`:`<div class="renderer-placeholder">WC3</div>`}
          <div class="renderer-stage-overlay">
            <span>Renderer bridge pending</span>
            <strong>${F(e?.map||`Waiting for replay data`)}</strong>
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                <h2>${F(e.sourceFlag)} ${F(e.sourceName)} on ${F(e.map)}</h2>
                <dl>
                  <div><dt>Date</dt><dd>${F(r)}</dd></div>
                  <div><dt>Replay ID</dt><dd>${F(e.id)}</dd></div>
                  <div><dt>Local file</dt><dd><code>${F(e.localPath)}</code></dd></div>
                  <div><dt>Players</dt><dd>${q(e,n)}</dd></div>
                </dl>
                <div class="renderer-actions">
                  ...
                  <a href="${F(e.downloadUrl)}" target="_blank" rel="noreferrer">Download replay</a>
                </div>
  ...
  `},un=()=>{try{let e=JSON.parse(localStorage.getItem(`w3AssetsIndex`)||`[]`),t=Array.isArray(e)?e.length:0,n=Array.isArray(e)?e.filter(e=>String(e?.p||``).toLowerCase().endsWith(`.mpq`)).length:0,r=Array.isArray(e)?e.filter(e=>/\.(w3x|w3m)$/i.test(String(e?.p||``))).length:0,i=Array.isArray(e)?e.reduce((e,t)=>e+Number(t?.s||0),0):0,a=localStorage.getItem(`wc3-warsmash-staged-edition`)||`legacy`;return{ready:localStorage.getItem(`w3AssetsReady`)===`1`&&t>0&&a===A,edition:a,fileCount:t,mpqCount:n,mapCount:r,totalMb:(i/1048576).toFixed(1)}}catch{return{ready:!1,edition:`legacy`,fileCount:0,mpqCount:0,mapCount:0,totalMb:`0.0`}}},dn=()=>fe===`windows`?{platform:`Windows`,primary:`C:\\Program Files\\Warcraft III`,secondary:`C:\\Program Files (x86)\\Warcraft III`,pickerHint:`In the folder picker, paste the path into the address bar if it does not open there automatically.`}:{platform:`macOS`,primary:`/Applications/Warcraft III (Legacy)`,secondary:`/Applications/Warcraft III`,pickerHint:`In the folder picker, press Cmd+Shift+G, paste the path, then choose the Warcraft III folder.`},X={reforged:`2.0.4.23745`,legacy:`1.29.2.9232`},fn=[{id:`legacy`,label:`Legacy MPQ`,build:X.legacy,status:`Playable now`,note:`Current Warsmash web path mounts classic MPQs and loose maps from this install.`},{id:`reforged`,label:`Reforged`,build:X.reforged,status:`Experimental CASC test`,note:`Latest Reforged uses CASC assets. This app now has an experimental browser-worker CASC mount path backed by the private sandbox.`}],pn=async(e,t)=>{let n=e;for(let e of t)n=await n.getDirectoryHandle(e,{create:!0});return n},mn=async()=>{let e=await navigator.storage.getDirectory();for(let t of[`w3`,`extracted`])try{await e.removeEntry(t,{recursive:!0})}catch{}},hn=async(e=A)=>{if(!M.running){M={running:!0,done:0,total:0,bytes:0,totalBytes:0,currentName:`Reading local install...`,error:``},$();try{let t=await fetch(`/wc3-local-assets/manifest?edition=${encodeURIComponent(e)}`,{cache:`no-store`});if(!t.ok){let e=await t.json().catch(()=>({}));throw Error(e.error||`No local MPQ-style Warcraft III install was detected.`)}let n=await t.json(),r=Array.isArray(n.files)?n.files:[];if(!r.length)throw Error(`Detected install did not include MPQ/map files Warsmash can use.`);M={running:!0,done:0,total:r.length,bytes:0,totalBytes:Number(n.totalBytes||0),currentName:`Preparing ${n.root||`local install`}`,error:``},$(),await mn();let i=await(await navigator.storage.getDirectory()).getDirectoryHandle(`w3`,{create:!0}),a=[],o=0,s=0;for(let t of r){let c=String(t.path||``);if(!c)continue;let l=await fetch(`/wc3-local-assets/file?edition=${encodeURIComponent(e)}&path=${encodeURIComponent(c)}`);if(!l.ok)throw Error(`Failed to read ${c}`);let u=await l.blob(),d=c.split(`/`).filter(Boolean),f=await(await(await pn(i,d.slice(0,-1))).getFileHandle(d[d.length-1],{create:!0})).createWritable();await f.write(u),await f.close(),o+=1,s+=u.size,a.push({p:c,s:u.size}),M={running:!0,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:c,error:``},(o===r.length||o%3==0)&&$()}localStorage.setItem(`w3AssetsReady`,`1`),localStorage.setItem(`w3AssetsCount`,String(a.length)),localStorage.setItem(`w3AssetsIndex`,JSON.stringify(a)),localStorage.setItem(`w3AssetsSource`,n.root||`detected local install`),localStorage.setItem(`wc3-warsmash-staged-edition`,e),localStorage.setItem(`wc3-warsmash-selected-edition`,e),M={running:!1,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:`Ready`,error:``},$()}catch(e){M={...M,running:!1,error:e instanceof Error?e.message:String(e)},$()}}},gn=async()=>{if(!j.preparing){j={...j,preparing:!0,error:``},$();try{let e=await fetch(`/wc3-reforged-sandbox/prepare`,{cache:`no-store`}),t=await e.json().catch(()=>({}));if(!e.ok)throw Error(t.error||`Failed to prepare Reforged sandbox.`);j={checked:!0,preparing:!1,ready:!0,build:t.build||``,fileCount:Number(t.totals?.configFiles||0)+Number(t.totals?.indexFiles||0)+Number(t.totals?.dataFiles||0),totalGb:(Number(t.totals?.bytes||0)/1073741824).toFixed(2),error:``}}catch(e){j={...j,checked:!0,preparing:!1,ready:!1,error:e instanceof Error?e.message:String(e)}}$()}},_n=()=>{let e=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ce.format(new Date(e.createdAt)):`Unknown date`,i=e?`${e.sourceSlug}:${e.id}`:``,a=`/play/?menu=1&assets=${encodeURIComponent(A)}&replay=${encodeURIComponent(i)}&autostart=1`,o=`/assets/`,s=un(),c=dn(),l=fn.find(e=>e.id===A)||fn[0],u=l.id===`legacy`,d=u?s.ready:j.ready,f=()=>A===`reforged`?`
      <div class="wc3-reforged-sandbox ${j.ready?`is-ready`:``} ${j.error?`is-error`:``}">
        <div>
          <span>Private Reforged sandbox</span>
          <strong>${j.ready?`Ready · ${F(j.build||X.reforged)}`:`Not prepared yet`}</strong>
          <small>${j.ready?`${F(B(j.fileCount))} CASC files indexed, ${F(j.totalGb)} GB referenced read-only.`:`Creates .wc3-assets/reforged/manifest.json and indexes your Reforged CASC data without modifying the install.`}</small>
          ${j.error?`<small>${F(j.error)}</small>`:``}
        </div>
        <button type="button" data-reforged-prepare ${j.preparing?`disabled`:``}>
          ${j.preparing?`Preparing...`:j.ready?`Refresh Sandbox`:`Prepare Reforged Sandbox`}
        </button>
      </div>
    `:``,ee=M.totalBytes?Math.min(100,M.bytes/M.totalBytes*100):0,p=M.running?`${M.done}/${M.total} ${(M.bytes/1048576).toFixed(1)} / ${(M.totalBytes/1048576).toFixed(1)} MB`:``;return`
    <section class="renderer-page wc3-viewer-page" aria-label="WC3 replay viewer">
      ...
      <div class="wc3-viewer-offline">
        ...
        ${f()}
        ${M.running||M.error?`
              <div class="wc3-auto-stage-status ${M.error?`is-error`:``}">
                ${M.running?`<div class="wc3-auto-stage-bar"><span style="width: ${ee.toFixed(1)}%;"></span></div><strong>${F(p)}</strong><small>${F(M.currentName)}</small>`:`<strong>Auto staging failed</strong><small>${F(M.error)}</small>`}
              </div>
            `:``}
        <div class="wc3-install-hint">
          <span>Suggested ${F(c.platform)} folder</span>
          <code>${F(l.id===`legacy`?c.primary:c.secondary)}</code>
          <small>${F(l.note)}</small>
    ...
    `}
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                ...
                <dl>
                  <div><dt>Date</dt><dd>${F(r)}</dd></div>
                  <div><dt>Replay ID</dt><dd>${F(e.id)}</dd></div>
                  <div><dt>Local replay</dt><dd><code>${F(e.localPath)}</code></dd></div>
                  <div><dt>Selected version</dt><dd>${F(l.label)} · ${F(l.status)}</dd></div>
                  <div><dt>Assets status</dt><dd>${d?u?`Ready: ${s.fileCount} files staged`:`Sandbox ready: ${j.fileCount} CASC files`:u?`Setup required`:`Prepare Reforged sandbox`}</dd></div>
                  <div><dt>Suggested folder</dt><dd><code>${F(l.id===`legacy`?c.primary:c.secondary)}</code></dd></div>
                  <div><dt>Players</dt><dd>${q(e,n)}</dd></div>
                </dl>
                <div class="renderer-actions">
                  <button class="replay-analysis-button" type="button" data-replay-detail="${F(`${e.sourceSlug}:${e.id}`)}">
                  ...
                  ${d?`<button class="warsmash-start-button" type="button" data-warsmash-watch="${F(i)}">${H()}<span>${u?`Start Engine Test`:`Start Reforged Engine Test`}</span></button>`:`<button type="button" data-warsmash-auto-stage ${M.running||!u?`disabled`:``}>${M.running?`Staging Detected Install...`:u?`Auto Stage Detected Install`:`Reforged Support Pending`}</button>`}
      ...
      <section class="renderer-research">
        ...
        <article>
          <h2>Patch Support</h2>
          <p>Your Reforged install is detected as <code>${X.reforged}</code>. The worker now has an experimental CASC mount path, while legacy remains available through classic MPQs from <code>${X.legacy}</code>.</p>
        </article>
        <article>
...
`,Q=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${z(e)}${F(Fe(R(e)).replace(` (${e})`,``))}</span>
              <strong>${F(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${F(t)}</strong></div>`,vn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-text-row">
              <span>${F(e)}</span>
              <strong>${F(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${F(t)}</strong></div>`,yn=()=>{let e=pt(),t=e.humanGames||1,n=K(e.boughtItems,10),r=K(e.foundItems,8);return`
    <section class="statistics-page" aria-label="Replay statistics">
      ...
      <div class="statistics-metrics" aria-label="Replay data summary">
        ${Z(`Parsed replays`,B(e.parsedReplays),`Allowed map pool only`)}
        ${Z(`Human player-games`,B(e.humanGames),`One Human player in most rows`)}
        ${Z(`Average Human APM`,B(e.averageApm),`Parsed APM samples`)}
        ${Z(`Item actions`,B(e.totalItemActions),`Buys, uses, and item commands`)}
      </div>
      ...
      <section class="statistics-panel statistics-hero-panel">
        <div class="statistics-panel-header">
          ...
          <span>${F(B(e.humanGames))} Human games</span>
        </div>
        <div class="statistics-hero-grid">
          ${ut.map(n=>{let r=e.heroPicks.get(n)||0,i=e.firstHeroPicks.get(n)||0,a=e.humanGames?r/t*100:0,o=e.humanGames?i/t*100:0;return`
                <article class="statistics-hero-card">
                  <div>
                    ${z(n)}
                    <h3>${F(Fe(R(n)).replace(` (${n})`,``))}</h3>
                  </div>
                  <dl>
                    <div><dt>Picked</dt><dd>${F(B(r))}</dd></div>
                    <div><dt>Pick rate</dt><dd>${F(Ge(a))}</dd></div>
                    <div><dt>First hero</dt><dd>${F(B(i))}</dd></div>
                    <div><dt>First rate</dt><dd>${F(Ge(o))}</dd></div>
                  </dl>
                </article>
              `}).join(``)}
        </div>
      ...
      <div class="statistics-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${F(Re(e.knownBoughtGold))}</span>
          </div>
          <div class="statistics-list">
            ${Q(n,`No shop purchases found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${F(B([...e.foundItems.values()].reduce((e,t)=>e+t,0)))}</span>
          </div>
          <div class="statistics-list">
            ${Q(r,`No non-shop item events found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${Q(K(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${Q(K(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${Q(K(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(K(e.mapCounts,8),`No map stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${vn(K(e.matchupCounts,8),`No matchup stats found`)}
          </div>
        </section>
      </div>
    </section>
  `},bn=()=>{let e=U(),t=e.map(e=>({replay:e,analysis:L(e.sourceSlug,e.id)})).filter(e=>e.analysis?.status===`parsed`),n=new Map,r=new Map,i=new Map,a=0,o=0,s=0,c=0,l=0;return t.forEach(({replay:e,analysis:t})=>{G(n,t.version||e.version||`Unknown`),G(r,e.map||t.map?.file||`Unknown map`),a+=t.chat?.length||0,s+=t.chat?.length||0,(t.players||[]).forEach(e=>{o+=1,Number.isFinite(e.apm)&&(c+=e.apm,l+=1),Object.entries(e.actions||{}).forEach(([e,t])=>{G(i,e,t||0)}),s+=(e.heroes||[]).length,s+=(e.order?.buildings||[]).length,s+=(e.order?.units||[]).length,s+=(e.order?.upgrades||[]).length,s+=(e.order?.items||[]).length})}),{totalReplays:e.length,parsedReplays:t.length,parsedPlayers:o,eventCount:s,chatMessages:a,averageApm:l?Math.round(c/l):0,versionCounts:n,mapCounts:r,actionCounts:i}},xn=e=>e.length?e.map(([e,t])=>`
            <div class="w3gjs-action-row">
              <span>${F(e)}</span>
              <strong>${F(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>No action data parsed yet</strong></div>`,Sn=e=>e.length?e.map(e=>`
          ...
          `).join(``):`<div class="analysis-empty"><strong>No timeline events parsed yet</strong></div>`,Cn=()=>{let e=bn(),t=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],n=t?L(t.sourceSlug,t.id):null,r=n?.status===`parsed`?n.players:[],i=Ft(n),a=i.slice(0,12),o=i.filter(e=>(e.ms||0)>=18e4).slice(0,12),s=K(e.actionCounts,12),c=K(e.versionCounts,4),l=K(e.mapCounts,6);return`
...
const result = await parser.parse("replay.w3g");</code></pre>
      ...
      <div class="statistics-metrics w3gjs-metrics" aria-label="w3gjs parsed data summary">
        ${Z(`Replay files`,B(e.totalReplays),`From the local replay cache`)}
        ${Z(`Parsed replays`,B(e.parsedReplays),`Built from w3gjs output`)}
        ${Z(`Parsed players`,B(e.parsedPlayers),`Names, races, teams, APM`)}
        ${Z(`Timeline events`,B(e.eventCount),`Builds, units, items, heroes, chat`)}
      </div>
      ...
      <section class="w3gjs-showcase">
        <div class="w3gjs-replay-card">
          ...
          ${t?`
                <h2>${F(t.sourceFlag)} ${F(t.sourceName)} on ${F(t.map)}</h2>
                <div class="analysis-modal-matchup">${q(t,r)}</div>
                <dl>
                  <div><dt>Game</dt><dd>${F(n?.gameName||`Unknown`)}</dd></div>
                  <div><dt>Patch</dt><dd>${F(n?.version||t.version||`Unknown`)}</dd></div>
                  <div><dt>Build</dt><dd>${F(n?.buildNumber||`Unknown`)}</dd></div>
                  <div><dt>Duration</dt><dd>${F(n?.durationLabel||t.durationLabel||`Unknown`)}</dd></div>
                  <div><dt>Map file</dt><dd><code>${F(n?.map?.file||t.map||`Unknown`)}</code></dd></div>
                  <div><dt>Local file</dt><dd><code>${F(t.localPath||n?.sourceFile||``)}</code></dd></div>
                </dl>
                <button class="replay-analysis-button" type="button" data-replay-detail="${F(`${t.sourceSlug}:${t.id}`)}">
                  ${H()}
      ...
      <div class="w3gjs-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${F(B(e.averageApm))} avg APM</span>
          </div>
          ${xt({...n,players:r})}
        </section>
...
`,On=()=>{let e=h?.players||[];return`
    <section class="money-page" aria-label="Player earnings">
      ...
      <div class="money-table" role="table" aria-label="Human earnings leaderboard">
        ...
        ${e.length?e.map(e=>`
                    <div class="money-row" role="row">
                      <span>#${F(e.rank)}</span>
                      <strong>${F(e.name)}</strong>
                      <span>${F(e.flag||``)} ${F(e.country||`Unknown`)}</span>
                      <span>${F(e.race||`Human`)}</span>
                      <strong class="money-amount">${F(e.earningsLabel)}</strong>
                    </div>
      ...
      <p class="money-note">${F(h?.scope||``)}</p>
    </section>
  `},kn=()=>{let e=document.querySelector(`.replay-pager`);e&&(e.innerHTML=Jt(),In(),Rn())},An=()=>{b=1},jn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{let t=e.dataset.playerFilter===g;e.classList.toggle(`is-selected`,t),e.setAttribute(`aria-pressed`,String(t)),e.closest(`.player-card`)?.classList.toggle(`is-selected`,t),e.classList.contains(`ranking-row`)&&e.classList.toggle(`is-selected`,t)})},Mn=e=>{g=g===e?`all`:e,An(),jn(),kn()},Nn=(e=null)=>{document.querySelectorAll(`.dropdown-filter.is-open`).forEach(t=>{t!==e&&(t.classList.remove(`is-open`),t.querySelector(`.filter-dropdown-button`)?.setAttribute(`aria-expanded`,`false`))})},Pn=()=>{let e=document.querySelector(`.filter-popover`);e&&(e.classList.remove(`is-open`),e.querySelector(`.filters-toggle`)?.setAttribute(`aria-expanded`,`false`),Nn())},Fn=()=>{let e=document.querySelector(`.filters`);e&&(e.outerHTML=tn(),Bn())},In=()=>{document.querySelectorAll(`.pagination-button[data-page]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.page);!Number.isFinite(t)||e.disabled||t===b||(b=t,kn(),document.querySelector(`.replay-column`)?.scrollIntoView({block:`start`,behavior:`smooth`}))})})},Ln=()=>{x=null,E=`overview`,$()},Rn=()=>{document.querySelectorAll(`.replay-row[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`a, button`)||(x=e.dataset.replayDetail,E=`overview`,$())}),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||t.target.closest(`a, button`)||(t.preventDefault(),x=e.dataset.replayDetail,E=`overview`,$())})}),document.querySelectorAll(`.replay-analysis-button[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),x=e.dataset.replayDetail,E=`overview`,$()})}),document.querySelectorAll(`[data-analysis-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{E=e.dataset.analysisTab||`overview`,$()})}),document.querySelectorAll(`[data-modal-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Ln()})}),document.querySelectorAll(`[data-theater-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&It()})}),document.querySelector(`[data-theater-play]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Rt()}),document.querySelector(`[data-theater-reset]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),w=0,C=!1,T&&=(clearInterval(T),null),$()})},zn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.ranking-stream`)||Mn(e.dataset.playerFilter)}),e.addEventListener(`keydown`,t=>{e.tagName!==`BUTTON`&&(t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Mn(e.dataset.playerFilter)))})})},Bn=()=>{let e=document.querySelector(`#replay-search`);if(!e)return;e.addEventListener(`input`,e=>{de=e.target.value,An(),kn()});let t=document.querySelector(`.filter-popover`),n=document.querySelector(`.filters-toggle`);n?.addEventListener(`click`,e=>{e.stopPropagation();let r=t?.classList.toggle(`is-open`)||!1;n.setAttribute(`aria-expanded`,String(r)),r||Nn()}),t?.addEventListener(`click`,e=>{e.stopPropagation()}),document.querySelectorAll(`.dropdown-filter`).forEach(e=>{let t=e.querySelector(`.filter-dropdown-button`),n=e.dataset.filter;t?.addEventListener(`click`,n=>{n.stopPropagation();let r=e.classList.toggle(`is-open`);Nn(e),t.setAttribute(`aria-expanded`,String(r))}),e.querySelectorAll(`.filter-dropdown-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.value||`all`;n===`map-filter`&&(_=t),n===`hero-filter`&&(v=t),n===`matchup-filter`&&(y=t),An(),Nn(),Fn(),kn()})})})},Vn=()=>{let e=document.querySelector(`.player-card-strip`);if(!e)return;let t=!1,n=0,r=0,i=!1;e.addEventListener(`pointerdown`,a=>{if(a.button===0){if(a.target.closest(`a, button, input, select, textarea, label`)){t=!1,i=!1;return}t=!0,i=!1,n=a.clientX,r=e.scrollLeft,e.classList.add(`is-dragging`),e.setPointerCapture(a.pointerId)}}),e.addEventListener(`pointermove`,a=>{if(!t)return;let o=a.clientX-n;Math.abs(o)>4&&(i=!0),e.scrollLeft=r-o});let a=n=>{t&&(t=!1,e.classList.remove(`is-dragging`),e.hasPointerCapture(n.pointerId)&&e.releasePointerCapture(n.pointerId))};e.addEventListener(`pointerup`,a),e.addEventListener(`pointercancel`,a),e.addEventListener(`click`,e=>{i&&=(e.preventDefault(),e.stopPropagation(),!1)},!0)},Hn=()=>{let e=document.querySelector(`.theme-button`);e&&e.addEventListener(`click`,()=>{N=N===`dark`?`light`:`dark`,localStorage.setItem(`wc3-color-theme`,N),ke({instant:!0}),e.setAttribute(`aria-label`,N===`dark`?`Switch to light theme`:`Switch to dark theme`)})},Un=e=>`
  <ul class="app-info-link-list">
    ${e.map(e=>`
          <li>
            <a href="${F(e.href)}" target="_blank" rel="noreferrer">${F(e.label)}</a>
            <span>${F(e.description)}</span>
          </li>
        `).join(``)}
  </ul>
`,Wn=e=>`
...
`,Gn=()=>O===`analyze`?`
  ...
  `,Kn=()=>`
  <button class="app-info-button" type="button" aria-label="Open app info" aria-expanded="${D}" aria-controls="app-info-panel" data-info-toggle>
    <span aria-hidden="true">i</span>
  </button>
  <aside class="app-info-panel ${D?`is-open`:``}" id="app-info-panel" aria-hidden="${!D}" aria-label="App info">
    ...
    <div class="app-info-tabs" role="tablist" aria-label="App info sections">
      ${ye.map(e=>`
            <button class="${e.id===O?`is-active`:``}" type="button" role="tab" aria-selected="${e.id===O}" data-info-tab="${F(e.id)}">
              ${F(e.label)}
            </button>
          `).join(``)}
    </div>
...
`,Jn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{D=!D,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{D=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{O=e.dataset.infoTab||`sources`,$()})})},Yn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{k=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),x=null,E=`overview`,k=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{A=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,A),k=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{hn(A)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{gn()})},Xn=()=>{fe=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=Ae();document.querySelector(`#app`).innerHTML=`
    <header class="site-header">
      <a class="brand ${he?``:`with-logo-intro`}" href="#/" aria-label="Human vs The World home">
        <img src="${c}" alt="Human vs The World" />
      </a>
      <button class="theme-button" type="button" aria-label="${N===`dark`?`Switch to light theme`:`Switch to dark theme`}">
    ...
    <aside class="menu-panel" id="site-menu" aria-hidden="true">
      <div class="menu-panel-inner">
        ...
        <nav class="menu-nav" aria-label="Menu">
          ${Yt(e.type===`all`?`is-active`:``)}
          ${Xt(e.type===`builds`?`is-active`:``)}
          ${Zt(e.type===`human-money`?`is-active`:``)}
          ${Qt(e.type===`statistics`?`is-active`:``)}
          ${$t(e.type===`resources`?`is-active`:``)}
        </nav>
    ...
    <main class="page-shell">
      ${e.type===`builds`?cn():e.type===`human-money`?On():e.type===`statistics`?yn():e.type===`renderer`?ln():e.type===`wc3-replay-viewer`?_n():e.type===`w3gjs`?Cn():e.type===`resources`?Dn():sn()}
    </main>
    ${qn()}
    ...
    ${zt()}
    ${Kn()}
  `;let t=document.querySelector(`.site-header`),n=document.querySelector(`.menu-panel`),r=document.querySelector(`.menu-button`),i=()=>{n.classList.remove(`is-open`),n.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`)};r.addEventListener(`click`,()=>{let e=n.classList.toggle(`is-open`);n.setAttribute(`aria-hidden`,String(!e)),r.setAttribute(`aria-expanded`,String(e)),r.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,i)}),document.documentElement.style.setProperty(`--header-height`,`${t.offsetHeight}px`),Zn(),Bn(),In(),Rn(),zn(),Vn(),Hn(),Jn(),Yn(),document.querySelector(`.player-card-region.with-card-intro`)&&!me&&(me=window.setTimeout(()=>{pe=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!ge&&(ge=window.setTimeout(()=>{he=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(S){It();return}if(D){D=!1,$();return}if(x){Ln();return}Pn();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-button`);!t||!n||(t.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),n.setAttribute(`aria-expanded`,`false`),n.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{Pn()}),window.addEventListener(`hashchange`,()=>{k=!1,T&&=(clearInterval(T),null),S=null,C=!1,w=0,g=`all`,_=`all`,v=`all`,y=`all`,An(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Pe(n)&&(x=null,E=`overview`,Lt(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{Ae().type===`wc3-replay-viewer`&&!k&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&Ae().type===`wc3-replay-viewer`&&!k&&$()});var Zn=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,Zn,{passive:!0});var Qn=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Xn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([Qn(`replays.json`),Qn(`rankings.json`),Qn(`replay-analysis.json`),Qn(`map-drop-tables.json`),Qn(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?oe=t.value:se=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(ue=n.value),r.status===`fulfilled`&&(ce=r.value),i.status===`fulfilled`&&(le=i.value),a.status===`fulfilled`&&(h=a.value),$()})();