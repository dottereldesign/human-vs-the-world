...
`,tt=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Ze(e.streamUrl);return`
  ...
  `},nt=e=>e.liquipediaUrl?`
    <a href="${P(e.liquipediaUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${P(e.name)} on Liquipedia">
      ${et({key:`liquipedia`,short:`LP`,icon:te},`Liquipedia`)}
    </a>
  `:`<span class="is-disabled"><span class="service-icon service-icon-liquipedia" aria-hidden="true"><img src="${te}" alt="" loading="lazy" /></span><span class="action-label">Liquipedia</span></span>`,rt=e=>e.realName?`<p class="player-card-real-name">${P(e.realName)}</p>`:``,it=e=>{let t={chaemiko:i,fortitude:l,hawk:u,infi:f,leon:ee,sok:ne}[e.slug];return t?`<img class="player-card-image" src="${t}" alt="${P(e.name)}" loading="lazy" />`:`<span>${P(e.flag)}</span>`},at=e=>e.slug===`infi`?`
  ...
  `:``,U=()=>p?I().flatMap(e=>Ne(e.slug).replays.filter(e=>je(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],ot=()=>[...new Set(U().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),W=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,st=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,ct=e=>{let t=st(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},lt=e=>{let t=ct(e);return t.length<2?``:`${W(t[0].race)} vs ${W(t[1].race)}`},ut=()=>[...new Set(U().map(lt).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),dt=e=>L(e.sourceSlug,e.id),G=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],ft=e=>W(e?.raceDetected||e?.race)===`Human`,K=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},pt=(e,t=[])=>{t.forEach(t=>K(e,t.id,Number(t.count)||0))},q=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||R(e[0]).localeCompare(R(t[0]))).slice(0,t),mt=()=>{let e=new Map(G.map(e=>[e,0])),t=new Map(G.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,te=0,ne=0;return U().forEach(re=>{let ie=L(re.sourceSlug,re.id);if(ie?.status!==`parsed`)return;u+=1;let ae=(ie.players||[]).filter(ft);ae.length&&(K(s,re.map||ie.map?.name||`Unknown map`),K(c,lt(re)||`Unknown matchup`),ae.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{G.includes(t.id)&&K(e,t.id)}),G.includes(c[0]?.id)&&K(t,c[0].id),(s.order?.items||[]).forEach(e=>{ze(e.id)?(K(n,e.id),te+=N[e.id]||0):(K(r,e.id),N[e.id]!==null&&(ne+=1))}),pt(i,s.topUnits),pt(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>K(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:te,unknownBoughtPrices:ne}},ht=e=>(dt(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),gt=()=>{let e=new Map;return U().forEach(t=>{ht(t).forEach(t=>{e.has(t)||e.set(t,Le(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},_t=e=>ct(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),vt=e=>{let i=String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:t,human:t,o:r,orc:r,u:ie,undead:ie,n,nightelf:n};return a[i]?a[i]:null},yt=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>W(t.race||t.raceDetected)===W(e.race));return i.length===1?i[0]:n[t]||null},bt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${P(t)}</span>
      <strong>${P(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Xe(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},J=(e,t=[])=>ct(e).map((e,n)=>{let r=vt(e.race),i=yt(e,n,t);return`
...
`,Et=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=wt,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
  ...
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},Mt=(e=[])=>e.length?e.map(e=>`<span>${P(R(e.id))} × ${P(e.count)}</span>`).join(``):`<span>None parsed</span>`,Nt=e=>{let t=e?.players||[];return t.length?`
        ...
        <div class="replay-theater-body">
          ...
          <section class="replay-theater-controls">
            ...
            <div class="replay-theater-buttons">
              ...
              <button class="replay-analysis-button" type="button" data-replay-detail="${P(`${e.sourceSlug}:${e.id}`)}">Open Analysis Tabs</button>
            </div>
          </section>
...
`,Ht=e=>he?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,_t(e)].filter(Boolean).join(` `).toLowerCase().includes(he.toLowerCase()):!0,Ut=e=>{let t=h===`all`||e.sourceSlug===h,n=g===`all`||e.map===g,r=_===`all`||ht(e).includes(_),i=v===`all`||lt(e)===v;return t&&n&&r&&i&&Ht(e)},Wt=()=>U().filter(Ut).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Gt=e=>{y=Math.min(Math.max(y,1),Math.max(e,1))},Kt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},qt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(y-1)*M+1,r=Math.min(y*M,e),i=Kt(y,t),a=0;return`
  ...
  `},Jt=e=>de?`
    ...
    `:p?e.length?e.map(e=>{let t=e.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`,n=L(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
        <article class="replay-row" data-replay-detail="${P(`${e.sourceSlug}:${e.id}`)}" role="button" tabindex="0" aria-label="Analyze ${P(e.map)} replay">
          ...
          <button class="replay-analysis-button" type="button" data-replay-detail="${P(`${e.sourceSlug}:${e.id}`)}">
            ${H()}
            <span>Analyze</span>
          </button>
        </article>
    ...
    `;let e=Wt(),t=Math.ceil(e.length/M);Gt(t);let n=(y-1)*M;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${Jt(e.slice(n,n+M))}
    </section>
...
`,un=()=>{let e=U().find(e=>e.mapImage?.localPath&&L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`;return`
      ...
      <div class="wc3-viewer-offline">
        ...
        ${j.running||j.error?`
              <div class="wc3-auto-stage-status ${j.error?`is-error`:``}">
                ${j.running?`<div class="wc3-auto-stage-bar"><span style="width: ${ee.toFixed(1)}%;"></span></div><strong>${P(te)}</strong><small>${P(j.currentName)}</small>`:`<strong>Auto staging failed</strong><small>${P(j.error)}</small>`}
              </div>
            `:``}
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                ...
                <div class="renderer-actions">
                  ...
                  ${d?`<button class="warsmash-start-button" type="button" data-warsmash-watch="${P(i)}">${H()}<span>${u?`Start Engine Test`:`Start Reforged Engine Test`}</span></button>`:`<button type="button" data-warsmash-auto-stage ${j.running||!u?`disabled`:``}>${j.running?`Staging Detected Install...`:u?`Auto Stage Detected Install`:`Reforged Support Pending`}</button>`}
...
`,yn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${z(e)}${P(Ie(R(e)).replace(` (${e})`,``))}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,bn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-text-row">
              <span>${P(e)}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,xn=()=>{let e=mt(),t=e.humanGames||1,n=q(e.boughtItems,10),r=q(e.foundItems,8);return`
    <section class="statistics-page" aria-label="Replay statistics">
      ...
      <div class="statistics-grid">
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${yn(r,`No non-shop item events found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${yn(q(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${yn(q(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${yn(q(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${bn(q(e.mapCounts,8),`No map stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${bn(q(e.matchupCounts,8),`No matchup stats found`)}
          </div>
        </section>
      </div>
    </section>
  `},Sn=()=>{let e=U(),t=e.map(e=>({replay:e,analysis:L(e.sourceSlug,e.id)})).filter(e=>e.analysis?.status===`parsed`),n=new Map,r=new Map,i=new Map,a=0,o=0,s=0,c=0,l=0;return t.forEach(({replay:e,analysis:t})=>{K(n,t.version||e.version||`Unknown`),K(r,e.map||t.map?.file||`Unknown map`),a+=t.chat?.length||0,s+=t.chat?.length||0,(t.players||[]).forEach(e=>{o+=1,Number.isFinite(e.apm)&&(c+=e.apm,l+=1),Object.entries(e.actions||{}).forEach(([e,t])=>{K(i,e,t||0)}),s+=(e.heroes||[]).length,s+=(e.order?.buildings||[]).length,s+=(e.order?.units||[]).length,s+=(e.order?.upgrades||[]).length,s+=(e.order?.items||[]).length})}),{totalReplays:e.length,parsedReplays:t.length,parsedPlayers:o,eventCount:s,chatMessages:a,averageApm:l?Math.round(c/l):0,versionCounts:n,mapCounts:r,actionCounts:i}},Cn=e=>e.length?e.map(([e,t])=>`
            <div class="w3gjs-action-row">
              <span>${P(e)}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>No action data parsed yet</strong></div>`,wn=e=>e.length?e.map(e=>`
          ...
          `).join(``):`<div class="analysis-empty"><strong>No timeline events parsed yet</strong></div>`,Tn=()=>{let e=Sn(),t=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],n=t?L(t.sourceSlug,t.id):null,r=n?.status===`parsed`?n.players:[],i=It(n),a=i.slice(0,12),o=i.filter(e=>(e.ms||0)>=18e4).slice(0,12),s=q(e.actionCounts,12),c=q(e.versionCounts,4),l=q(e.mapCounts,6);return`
      ...
      <section class="w3gjs-showcase">
        <div class="w3gjs-replay-card">
          ...
          ${t?`
                ...
                <button class="replay-analysis-button" type="button" data-replay-detail="${P(`${t.sourceSlug}:${t.id}`)}">
                  ${H()}
    ...
    <div class="resources-link-list">
      ${e.links.map(e=>`
            <a href="${P(e.href)}" target="_blank" rel="noreferrer">
              <img src="${P(Dn(e.href))}" alt="" loading="lazy" onerror="this.remove()" />
              <span>${P(e.label)}</span>
...
`,An=()=>{let e=m?.players||[];return`
    <section class="money-page" aria-label="Player earnings">
      ...
      <p class="money-note">${P(m?.scope||``)}</p>
    </section>
  `},jn=()=>{let e=document.querySelector(`.replay-pager`);e&&(e.innerHTML=Yt(),Rn(),Bn())},Mn=()=>{y=1},Nn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{let t=e.dataset.playerFilter===h;e.classList.toggle(`is-selected`,t),e.setAttribute(`aria-pressed`,String(t)),e.closest(`.player-card`)?.classList.toggle(`is-selected`,t),e.classList.contains(`ranking-row`)&&e.classList.toggle(`is-selected`,t)})},Pn=e=>{h=h===e?`all`:e,Mn(),Nn(),jn()},Fn=(e=null)=>{document.querySelectorAll(`.dropdown-filter.is-open`).forEach(t=>{t!==e&&(t.classList.remove(`is-open`),t.querySelector(`.filter-dropdown-button`)?.setAttribute(`aria-expanded`,`false`))})},In=()=>{let e=document.querySelector(`.filter-popover`);e&&(e.classList.remove(`is-open`),e.querySelector(`.filters-toggle`)?.setAttribute(`aria-expanded`,`false`),Fn())},Ln=()=>{let e=document.querySelector(`.filters`);e&&(e.outerHTML=nn(),Hn())},Rn=()=>{document.querySelectorAll(`.pagination-button[data-page]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.page);!Number.isFinite(t)||e.disabled||t===y||(y=t,jn(),document.querySelector(`.replay-column`)?.scrollIntoView({block:`start`,behavior:`smooth`}))})})},zn=()=>{b=null,T=`overview`,$()},Bn=()=>{document.querySelectorAll(`.replay-row[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`a, button`)||(b=e.dataset.replayDetail,T=`overview`,$())}),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||t.target.closest(`a, button`)||(t.preventDefault(),b=e.dataset.replayDetail,T=`overview`,$())})}),document.querySelectorAll(`.replay-analysis-button[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),b=e.dataset.replayDetail,T=`overview`,$()})}),document.querySelectorAll(`[data-analysis-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{T=e.dataset.analysisTab||`overview`,$()})}),document.querySelectorAll(`[data-modal-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&zn()})}),document.querySelectorAll(`[data-theater-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Lt()})}),document.querySelector(`[data-theater-play]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),zt()}),document.querySelector(`[data-theater-reset]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),C=0,S=!1,w&&=(clearInterval(w),null),$()})},Vn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.ranking-stream`)||Pn(e.dataset.playerFilter)}),e.addEventListener(`keydown`,t=>{e.tagName!==`BUTTON`&&(t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Pn(e.dataset.playerFilter)))})})},Hn=()=>{let e=document.querySelector(`#replay-search`);if(!e)return;e.addEventListener(`input`,e=>{he=e.target.value,Mn(),jn()});let t=document.querySelector(`.filter-popover`),n=document.querySelector(`.filters-toggle`);n?.addEventListener(`click`,e=>{e.stopPropagation();let r=t?.classList.toggle(`is-open`)||!1;n.setAttribute(`aria-expanded`,String(r)),r||Fn()}),t?.addEventListener(`click`,e=>{e.stopPropagation()}),document.querySelectorAll(`.dropdown-filter`).forEach(e=>{let t=e.querySelector(`.filter-dropdown-button`),n=e.dataset.filter;t?.addEventListener(`click`,n=>{n.stopPropagation();let r=e.classList.toggle(`is-open`);Fn(e),t.setAttribute(`aria-expanded`,String(r))}),e.querySelectorAll(`.filter-dropdown-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.value||`all`;n===`map-filter`&&(g=t),n===`hero-filter`&&(_=t),n===`matchup-filter`&&(v=t),Mn(),Fn(),Ln(),jn()})})})},Un=()=>{let e=document.querySelector(`.player-card-strip`);if(!e)return;let t=!1,n=0,r=0,i=!1;e.addEventListener(`pointerdown`,a=>{if(a.button===0){if(a.target.closest(`a, button, input, select, textarea, label`)){t=!1,i=!1;return}t=!0,i=!1,n=a.clientX,r=e.scrollLeft,e.classList.add(`is-dragging`),e.setPointerCapture(a.pointerId)}}),e.addEventListener(`pointermove`,a=>{if(!t)return;let o=a.clientX-n;Math.abs(o)>4&&(i=!0),e.scrollLeft=r-o});let a=n=>{t&&(t=!1,e.classList.remove(`is-dragging`),e.hasPointerCapture(n.pointerId)&&e.releasePointerCapture(n.pointerId))};e.addEventListener(`pointerup`,a),e.addEventListener(`pointercancel`,a),e.addEventListener(`click`,e=>{i&&=(e.preventDefault(),e.stopPropagation(),!1)},!0)},Wn=e=>`
...
`,Yn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{E=!E,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{E=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{D=e.dataset.infoTab||`sources`,$()})})},Xn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{O=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),b=null,T=`overview`,O=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{k=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,k),O=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{gn(k)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{_n()})},Zn=()=>{ge=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=Me(),t=e.type===`all`;document.querySelector(`#app`).innerHTML=`
    ...
    <main class="page-shell ${t?`page-shell-home`:`page-shell-subpage`}">
      ${e.type===`builds`?ln():e.type===`player-earnings`?An():e.type===`statistics`?xn():e.type===`renderer`?un():e.type===`wc3-replay-viewer`?vn():e.type===`w3gjs`?Tn():e.type===`resources`?kn():cn()}
    </main>
    ${Jn()}
    ...
    ${Bt()}
    ${qn()}
  `;let n=document.querySelector(`.site-header`),r=document.querySelector(`.menu-panel`),i=document.querySelector(`.menu-backdrop`),a=document.querySelector(`.menu-button`),o=()=>{r.classList.remove(`is-open`),i?.classList.remove(`is-open`),r.setAttribute(`aria-hidden`,`true`),a.setAttribute(`aria-expanded`,`false`),a.setAttribute(`aria-label`,`Open menu`)};a.addEventListener(`click`,()=>{let e=r.classList.toggle(`is-open`);i?.classList.toggle(`is-open`,e),r.setAttribute(`aria-hidden`,String(!e)),a.setAttribute(`aria-expanded`,String(e)),a.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),i?.addEventListener(`click`,o),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,o)}),document.documentElement.style.setProperty(`--header-height`,`${n.offsetHeight}px`),Qn(),Hn(),Rn(),Bn(),Vn(),Un(),Yn(),Xn(),document.querySelector(`.player-card-region.with-card-intro`)&&!ve&&(ve=window.setTimeout(()=>{_e=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!be&&(be=window.setTimeout(()=>{ye=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(x){Lt();return}if(E){E=!1,$();return}if(b){zn();return}In();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-backdrop`),r=document.querySelector(`.menu-button`);!t||!r||(t.classList.remove(`is-open`),n?.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{In()}),window.addEventListener(`hashchange`,()=>{O=!1,w&&=(clearInterval(w),null),x=null,S=!1,C=0,h=`all`,g=`all`,_=`all`,v=`all`,Mn(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Fe(n)&&(b=null,T=`overview`,Rt(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{Me().type===`wc3-replay-viewer`&&!O&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&Me().type===`wc3-replay-viewer`&&!O&&$()});var Qn=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,Qn,{passive:!0});var $n=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Zn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([$n(`replays.json`),$n(`rankings.json`),$n(`replay-analysis.json`),$n(`map-drop-tables.json`),$n(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?p=t.value:de=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(me=n.value),r.status===`fulfilled`&&(fe=r.value),i.status===`fulfilled`&&(pe=i.value),a.status===`fulfilled`&&(m=a.value),$()})();