...
`,tt=({key:e,short:t,icon:n},r)=>`
  <span class="service-icon service-icon-${P(e)}" aria-hidden="true">
    ${n?`<img src="${n}" alt="" loading="lazy" />`:P(t)}
  </span>
  <span class="action-label">${P(r)}</span>
`,nt=e=>{if(!e.streamUrl)return`<span class="is-disabled"><span class="service-icon service-icon-stream" aria-hidden="true">St</span><span class="action-label">Stream</span></span>`;let t=Ze(e.streamUrl);return`
    <a href="${P(e.streamUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${P(e.name)} ${P(t.label)} stream">
      ${tt(t,t.label)}
    </a>
  `},rt=e=>e.liquipediaUrl?`
    <a href="${P(e.liquipediaUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${P(e.name)} on Liquipedia">
      ${tt({key:`liquipedia`,short:`LP`,icon:p},`Liquipedia`)}
    </a>
  `:`<span class="is-disabled"><span class="service-icon service-icon-liquipedia" aria-hidden="true"><img src="${p}" alt="" loading="lazy" /></span><span class="action-label">Liquipedia</span></span>`,it=e=>e.realName?`<p class="player-card-real-name">${P(e.realName)}</p>`:``,at=e=>{let t={chaemiko:i,fortitude:l,hawk:u,infi:f,leon:ee,sok:te}[e.slug];return t?`<img class="player-card-image" src="${t}" alt="${P(e.name)}" loading="lazy" />`:`<span>${P(e.flag)}</span>`},ot=e=>e.slug===`infi`?`
  ...
  `:``,U=()=>le?I().flatMap(e=>Ne(e.slug).replays.filter(e=>je(e.map)).map(t=>({...t,sourceSlug:e.slug,sourceName:e.name,sourceFlag:e.flag}))):[],st=()=>[...new Set(U().map(e=>e.map).filter(Boolean))].sort((e,t)=>e.localeCompare(t)),W=e=>({h:`Human`,human:`Human`,o:`Orc`,orc:`Orc`,u:`Undead`,undead:`Undead`,n:`Night Elf`,nightelf:`Night Elf`})[String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``)]||e||`Unknown`,ct=e=>(e.players||[]).find(t=>t.name?.toLowerCase()===e.sourceName?.toLowerCase())||e.players?.[0]||null,lt=e=>{let t=ct(e);return t?[t,...(e.players||[]).filter(e=>e!==t)]:e.players||[]},ut=e=>{let t=lt(e);return t.length<2?``:`${W(t[0].race)} vs ${W(t[1].race)}`},dt=()=>[...new Set(U().map(ut).filter(Boolean))].sort((e,t)=>!e.startsWith(`Human vs`)-+!t.startsWith(`Human vs`)||e.localeCompare(t)),ft=e=>L(e.sourceSlug,e.id),G=[`Hamg`,`Hmkg`,`Hpal`,`Hblm`],pt=e=>W(e?.raceDetected||e?.race)===`Human`,K=(e,t,n=1)=>{!t||!Number.isFinite(n)||e.set(t,(e.get(t)||0)+n)},mt=(e,t=[])=>{t.forEach(t=>K(e,t.id,Number(t.count)||0))},q=(e,t=8)=>[...e.entries()].filter(([,e])=>e>0).sort((e,t)=>t[1]-e[1]||R(e[0]).localeCompare(R(t[0]))).slice(0,t),ht=()=>{let e=new Map(G.map(e=>[e,0])),t=new Map(G.map(e=>[e,0])),n=new Map,r=new Map,i=new Map,a=new Map,o=new Map,s=new Map,c=new Map,l=0,u=0,d=0,f=0,ee=0,p=0,te=0;return U().forEach(m=>{let ne=L(m.sourceSlug,m.id);if(ne?.status!==`parsed`)return;u+=1;let re=(ne.players||[]).filter(pt);re.length&&(K(s,m.map||ne.map?.name||`Unknown map`),K(c,ut(m)||`Unknown matchup`),re.forEach(s=>{l+=1,Number.isFinite(s.apm)&&(d+=s.apm,f+=1),ee+=s.actions?.item||0;let c=s.heroes||[];c.forEach(t=>{G.includes(t.id)&&K(e,t.id)}),G.includes(c[0]?.id)&&K(t,c[0].id),(s.order?.items||[]).forEach(e=>{ze(e.id)?(K(n,e.id),p+=N[e.id]||0):(K(r,e.id),N[e.id]!==null&&(te+=1))}),mt(i,s.topUnits),mt(a,s.topBuildings),(s.order?.upgrades||[]).forEach(e=>K(o,e.id))}))}),{heroPicks:e,firstHeroPicks:t,boughtItems:n,foundItems:r,unitCounts:i,buildingCounts:a,upgradeCounts:o,mapCounts:s,matchupCounts:c,humanGames:l,parsedReplays:u,averageApm:f?Math.round(d/f):0,totalItemActions:ee,knownBoughtGold:p,unknownBoughtPrices:te}},gt=e=>(ft(e)?.players||[]).map(e=>e.heroes?.[0]?.id).filter(Boolean),_t=()=>{let e=new Map;return U().forEach(t=>{gt(t).forEach(t=>{e.has(t)||e.set(t,Le(t))})}),[...e.entries()].sort(([,e],[,t])=>e.localeCompare(t)).map(([e,t])=>({value:e,label:t,iconId:e}))},vt=e=>lt(e).map(e=>`${e.name}${e.race?` (${e.race})`:``}`).join(` vs `),yt=e=>{let i=String(e||``).toLowerCase().replaceAll(` `,``).replaceAll(`_`,``).replaceAll(`-`,``),a={h:t,human:t,o:r,orc:r,u:ne,undead:ne,n,nightelf:n};return a[i]?a[i]:null},bt=(e,t,n)=>{if(!n.length)return null;let r=n.find(t=>t.name?.toLowerCase()===e.name?.toLowerCase());if(r)return r;let i=n.filter(t=>W(t.race||t.raceDetected)===W(e.race));return i.length===1?i[0]:n[t]||null},xt=e=>{if(!e)return``;let t=e.name||`Unknown`,n=e.apm?`${e.apm} APM`:`APM unavailable`,r=(e.heroes||[]).slice(0,3);return`
    <span class="matchup-tooltip" role="tooltip">
      <span>${P(t)}</span>
      <strong>${P(n)}</strong>
      ${r.length?`<span class="matchup-tooltip-heroes">${r.map(e=>Xe(e,{compact:!0})).join(``)}</span>`:``}
    </span>
  `},J=(e,t=[])=>lt(e).map((e,n)=>{let r=yt(e.race),i=bt(e,n,t);return`
        <span class="matchup-player" tabindex="0">
          ${r?`<img src="${r}" alt="${P(e.race)}" loading="lazy" />`:``}
          <span>${P(e.name)}</span>
          ${xt(i)}
        </span>
      `}).join(`<span class="versus">vs</span>`),Y=(e,t)=>`
  <div class="analysis-pill">
    <span>${P(e)}</span>
    <strong>${P(t??`Unknown`)}</strong>
  </div>
`,St=(e,t)=>!t||t.status!==`parsed`?`
      <div class="analysis-empty">
        <strong>Analysis unavailable</strong>
        <span>${P(t?.reason||`Run npm run analyze:replays to parse this replay.`)}</span>
      </div>
    `:`
    <div class="analysis-overview">
      <div class="analysis-pill-grid">
        ${Y(`Map`,e.map)}
        ${Y(`Duration`,t.durationLabel)}
        ${Y(`Matchup`,t.matchup)}
        ${Y(`Patch`,t.version)}
        ${Y(`Game`,t.gameName)}
        ${Y(`Chat`,`${t.chatCount||0} messages`)}
      </div>
      <div class="analysis-player-grid">
        ${(t.players||[]).map(e=>`
              <div class="analysis-player-card">
                <span class="analysis-player-meta">${P(e.raceDetected||e.race||`Unknown race`)}</span>
                <strong>${P(e.name||`Unknown`)}</strong>
                <div class="analysis-stat-line">
                  <span>${P(e.apm||0)} APM</span>
                  <span>${P(e.heroCount||0)} heroes</span>
                </div>
        ...
        <div class="apm-legend">
          ${t.map((e,t)=>`
                <span class="apm-legend-item apm-series-${t+1}">
                  ...
                  ${P(e.name||`Unknown`)} · ${P(e.apm||0)} avg
                </span>
              `).join(``)}
        </div>
      </div>
                ...
                <g class="apm-series apm-series-${t+1}">
                  ...
                  ${r.map((t,r)=>`
                        <circle class="apm-point" cx="${t.x}" cy="${t.y}" r="3.2">
                          <title>${P(e.name||`Player`)} · ${P(n[r])} APM · ${r}m</title>
                        </circle>
                      `).join(``)}
                </g>
              `}).join(``)}
      ...
      <div class="apm-summary-grid">
        ${t.map(e=>`
            <div class="apm-summary-card">
              <span>${P(e.name||`Unknown`)}</span>
              <strong>${P(e.apm||0)} APM</strong>
              <em>Peak ${P(Math.max(...e.chartApmTimeline||[0]))} · Low ${P(Math.min(...e.chartApmTimeline||[0]))}</em>
            </div>
          `).join(``)}
      </div>
    </div>
  `},wt=(e,t)=>t.flatMap(([t,n])=>(n||[]).map(n=>({...n,type:t,playerName:e.name,race:e.raceDetected||e.race}))).sort((e,t)=>(e.ms||0)-(t.ms||0)),Tt=(e,t=`${e.type}`)=>`
  <div class="build-order-row">
    <span>${P(e.time||`0:00`)}</span>
    <div class="timeline-object">
      ${z(e.id)}
      <strong>${P(R(e.id))}</strong>
    </div>
    <em>${P(t)}</em>
  </div>
`,Et=(e,t)=>`
  <div class="build-order-row item-found-row">
    ...
    <em>
      ...
      <small>${P(Je(e.id,t))}</small>
      <small>${P(Ye(e.id,t))}</small>
    </em>
  </div>
`,Dt=({analysis:e,title:t,emptyMessage:n,getItems:r,renderRow:i=Tt,renderSummary:a=null})=>{let o=e?.players||[];return o.some(e=>r(e).length)?`
    <div class="timeline-columns">
      ${o.map(e=>{let t=r(e),n=a?a(e,t):``;return`
            <section class="timeline-player-column">
              <header>
                <span>${P(e.raceDetected||e.race||`Unknown race`)}</span>
                <strong>${P(e.name||`Unknown`)}</strong>
                ${n}
              </header>
  ...
  `:`<div class="analysis-empty"><strong>${P(t)} unavailable</strong><span>${P(n)}</span></div>`},Ot=e=>Dt({analysis:e,title:`Build order`,emptyMessage:`Run the updated replay analyzer to generate unit and building timeline data.`,getItems:e=>wt(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units]])}),kt=e=>Dt({analysis:e,title:`Upgrades`,emptyMessage:`No upgrades were parsed for this replay.`,getItems:e=>He(wt(e,[[`Upgrade`,e.order?.upgrades]])),renderRow:e=>Tt(e,Number.isFinite(e.goldCost)?Be(e.goldCost):`Cost unknown`),renderSummary:(e,t)=>{let n=Ue(t,e=>e.goldCost);return We({label:`Known gold spent`,gold:n.gold,unknown:n.unknown})}}),At=e=>Dt({analysis:e,title:`Items bought`,emptyMessage:`No shop-price items were parsed for this replay.`,getItems:e=>wt(e,[[`Item`,e.order?.items]]).filter(e=>ze(e.id)),renderSummary:(e,t)=>{let n=Ue(t,e=>N[e.id]);return We({label:`Total spent`,gold:n.gold,unknown:n.unknown})},renderRow:t=>{let n=Ke(t.id,e);return Tt(t,n?`${Re(t.id)} · ${qe(n.chance)} ${n.groupLabel} drop`:Re(t.id))}}),jt=e=>Dt({analysis:e,title:`Items found`,emptyMessage:`No no-shop-price items were parsed for this replay.`,getItems:e=>wt(e,[[`Item`,e.order?.items]]).filter(e=>!ze(e.id)),renderRow:t=>Et(t,e)}),Mt=e=>{let t=e?.chat||[];return t.length?`
    <div class="chatlog-list">
      ${t.map(e=>`
            <div class="chatlog-row">
              <span>${P(e.time||`0:00`)}</span>
              <strong>${P(e.playerName||`Unknown`)}</strong>
              <p>${P(e.message||``)}</p>
            </div>
          `).join(``)}
    </div>
  `:`<div class="analysis-empty"><strong>No chat messages</strong><span>This replay has no parsed player chat.</span></div>`},Nt=(e=[])=>e.length?e.map(e=>`<span>${P(R(e.id))} × ${P(e.count)}</span>`).join(``):`<span>None parsed</span>`,Pt=e=>{let t=e?.players||[];return t.length?`
    <div class="actions-grid">
      ${t.map(e=>`
            <div class="actions-card">
              <strong>${P(e.name||`Unknown`)}</strong>
              <div class="action-counts">
                ${Object.entries(e.actions||{}).map(([e,t])=>`<span><em>${P(e)}</em>${P(B(t||0))}</span>`).join(``)}
              </div>
  ...
  `:`<div class="analysis-empty"><strong>No action data</strong><span>Run the updated replay analyzer to generate action data.</span></div>`},Ft=(e,t)=>T===`apm`?Ct(t):T===`build`?Ot(t):T===`upgrades`?kt(t):T===`items-bought`?At(t):T===`items-found`?jt(t):T===`chat`?Mt(t):T===`actions`?Pt(t):St(e,t),It=()=>{let e=Fe(b);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=xe.some(e=>e.id===T)?T:`overview`;return`
    <div class="analysis-modal-shell" role="presentation" data-modal-close>
      <section class="analysis-modal" role="dialog" aria-modal="true" aria-label="Replay analysis">
        <header class="analysis-modal-header">
          <div>
            <span>${P(e.sourceFlag)} ${P(e.sourceName)}</span>
            <h2>${P(e.map)}</h2>
            <div class="analysis-modal-matchup">${J(e,n)}</div>
          </div>
        ...
        <nav class="analysis-tabs" aria-label="Replay analysis tabs">
          ${xe.map(e=>`
                <button class="${e.id===r?`is-active`:``}" type="button" data-analysis-tab="${P(e.id)}">
                  ${P(e.label)}
                </button>
              `).join(``)}
        </nav>
  ...
  `},X=(e=0)=>{let t=Math.max(0,Math.floor(Number(e||0)/1e3)),n=Math.floor(t/60),r=t%60;return`${n}:${String(r).padStart(2,`0`)}`},Lt=e=>{if(!e||e.status!==`parsed`)return[];let t=(e.players||[]).flatMap(e=>[...(e.heroes||[]).map(t=>({id:t.id,ms:0,time:`0:00`,type:`Hero`,playerName:e.name,race:e.raceDetected||e.race,detail:t.level?`Level ${t.level}`:`Hero`})),...wt(e,[[`Building`,e.order?.buildings],[`Unit`,e.order?.units],[`Upgrade`,He(e.order?.upgrades||[])],[`Item`,e.order?.items]]).map(t=>({...t,playerName:e.name,detail:t.type===`Item`?Re(t.id):t.type===`Upgrade`&&Number.isFinite(t.goldCost)?Be(t.goldCost):t.type}))]),n=(e.chat||[]).map(e=>({id:`chat`,ms:e.timeMS||0,time:e.time||X(e.timeMS||0),type:`Chat`,playerName:e.playerName||`Observer`,detail:e.message||``}));return[...t,...n].sort((e,t)=>(e.ms||0)-(t.ms||0))},Rt=()=>{x=null,S=!1,C=0,w&&=(clearInterval(w),null),$()},zt=e=>{x=e,C=0,S=!1,w&&=(clearInterval(w),null),$()},Bt=()=>{let e=Fe(x),t=(e?L(e.sourceSlug,e.id):null)?.duration||0;if(S=!S,w&&=(clearInterval(w),null),!S){$();return}w=setInterval(()=>{C+=1e3,t&&C>=t&&(C=t,S=!1,clearInterval(w),w=null),$()},1e3),$()},Vt=()=>{let e=Fe(x);if(!e)return``;let t=L(e.sourceSlug,e.id),n=t?.status===`parsed`?t.players:[],r=Lt(t),i=t?.duration||e.duration||Math.max(...r.map(e=>e.ms||0),0),a=i?Math.min(100,Math.max(0,C/i*100)):0,o=r.filter(e=>(e.ms||0)<=C).slice(-18),s=r.filter(e=>(e.ms||0)>C).slice(0,18);return`
      ...
      <section class="replay-theater" role="dialog" aria-modal="true" aria-label="Replay theater">
        <header class="replay-theater-header">
          <div>
            ...
            <h2>${P(e.sourceFlag)} ${P(e.sourceName)} on ${P(e.map)}</h2>
            <div class="analysis-modal-matchup">${J(e,n)}</div>
          </div>
        ...
        <div class="replay-theater-body">
          <section class="replay-theater-map">
            ${e.mapImage?.localPath?`<img src="${P(F(e.mapImage.localPath))}" alt="${P(e.map)} map thumbnail" />`:`<div class="map-placeholder" aria-hidden="true">${P(e.mapShort||`WC3`)}</div>`}
            <div class="replay-theater-map-overlay">
              <span>${P(t?.version?`Patch ${t.version}`:e.version||`Patch unknown`)}</span>
              <strong>${P(t?.durationLabel||e.durationLabel||X(i))}</strong>
              <small>${P(e.localPath||t?.sourceFile||``)}</small>
            </div>
          </section>
          <section class="replay-theater-controls">
            <div class="replay-theater-clock">
              <strong>${P(X(C))}</strong>
              <span>${P(t?.durationLabel||X(i))}</span>
            </div>
          ...
          <div class="replay-theater-columns">
            <section>
              ...
              <div class="replay-theater-event-list">
                ${o.length?o.map(Ht).join(``):`<div class="timeline-empty">Press play to advance the parsed replay timeline.</div>`}
              </div>
            </section>
            <section>
              <h3>Next Events</h3>
              <div class="replay-theater-event-list">
                ${s.length?s.map(Ht).join(``):`<div class="timeline-empty">No more parsed events.</div>`}
              </div>
            </section>
          </div>
        </div>
  ...
  <div class="replay-theater-event">
    <span>${P(e.time||X(e.ms||0))}</span>
    <div>
      <strong>${P(e.type===`Chat`?e.playerName:R(e.id))}</strong>
      <small>${P(e.type===`Chat`?e.detail:`${e.playerName||`Player`} · ${e.detail||e.type}`)}</small>
    </div>
  </div>
`,Ut=e=>he?[e.map,e.mapShort,e.type,e.origin,e.filetype,e.version,vt(e)].filter(Boolean).join(` `).toLowerCase().includes(he.toLowerCase()):!0,Wt=e=>{let t=h===`all`||e.sourceSlug===h,n=g===`all`||e.map===g,r=_===`all`||gt(e).includes(_),i=v===`all`||ut(e)===v;return t&&n&&r&&i&&Ut(e)},Gt=()=>U().filter(Wt).sort((e,t)=>new Date(t.createdAt||0)-new Date(e.createdAt||0)),Kt=e=>{y=Math.min(Math.max(y,1),Math.max(e,1))},qt=(e,t)=>{let n=new Set([1,t]);for(let r=e-2;r<=e+2;r+=1)r>=1&&r<=t&&n.add(r);return[...n].sort((e,t)=>e-t)},Jt=(e,t)=>{if(t<=1)return`<div class="replay-pagination is-simple"><span>${e} replay${e===1?``:`s`}</span></div>`;let n=(y-1)*M+1,r=Math.min(y*M,e),i=qt(y,t),a=0;return`
  ...
  `},Yt=e=>ue?`
    ...
    `:le?e.length?e.map(e=>{let t=e.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`,n=L(e.sourceSlug,e.id),r=n?.status===`parsed`?n.players:[],i=n?.durationLabel||e.durationLabel;return`
        <article class="replay-row">
          <div class="replay-map">
            ${e.mapImage?.localPath?`<img src="${P(F(e.mapImage.localPath))}" alt="${P(e.map)} map thumbnail" loading="lazy" />`:`<div class="map-placeholder" aria-hidden="true">${P(e.mapShort||`WC3`)}</div>`}
            <div>
              <span>${P(t)}</span>
              <strong>${P(e.map)}</strong>
              <small>${P(e.sourceFlag)} ${P(e.sourceName)}</small>
            </div>
          </div>
          <div class="replay-match">
            <div class="replay-matchup">${J(e,r)}</div>
            <div class="replay-actions">
              ${i?`<span class="replay-duration">${P(i)}</span>`:``}
    ...
    `;let e=Gt(),t=Math.ceil(e.length/M);Kt(t);let n=(y-1)*M;return`
    <section class="replay-list" aria-label="Recent replay rows">
      ${Yt(e.slice(n,n+M))}
    </section>
    ${e.length?Jt(e.length,Math.max(t,1)):``}
  `},Zt=(e=``)=>`
...
`,nn=({id:e,label:t,value:n,options:r})=>{let i=r.find(e=>e.value===n)||r[0];return`
    <div class="filter-control dropdown-filter" data-filter="${P(e)}">
      <span class="filter-label" id="${P(e)}-label">${P(t)}</span>
      <button class="filter-dropdown-button" id="${P(e)}" type="button" aria-haspopup="listbox" aria-expanded="false" aria-labelledby="${P(e)}-label ${P(e)}">
        <span>${i.iconId?z(i.iconId,i.label):``}${P(i.label)}</span>
        <span class="dropdown-chevron" aria-hidden="true"></span>
      </button>
      <div class="filter-dropdown-menu" role="listbox" aria-labelledby="${P(e)}-label">
        ${r.map(e=>`
              <button class="filter-dropdown-option ${e.value===n?`is-selected`:``}" type="button" role="option" aria-selected="${e.value===n}" data-value="${P(e.value)}">
                <span>${e.iconId?z(e.iconId,e.label):``}${P(e.label)}</span>
              </button>
            `).join(``)}
      </div>
    </div>
  ...
  <section class="filters" aria-label="Replay filters">
    <div class="quick-filter">
      ...
      <input id="replay-search" type="search" value="${P(he)}" placeholder="Search map, opponent, patch..." />
    </div>
    <div class="filter-popover">
      ...
      <div class="filter-popover-panel" id="advanced-filters">
        ${nn({id:`map-filter`,label:`Map`,value:g,options:[{value:`all`,label:`All maps`},...st().map(e=>({value:e,label:e}))]})}
        ${nn({id:`hero-filter`,label:`Starting hero`,value:_,options:[{value:`all`,label:`All heroes`},..._t()]})}
        ${nn({id:`matchup-filter`,label:`Matchup`,value:v,options:[{value:`all`,label:`All matchups`},...dt().map(e=>({value:e,label:e}))]})}
      </div>
    </div>
  </section>
`,an=e=>{if(!e.streamUrl)return`<span class="ranking-stream is-empty" aria-hidden="true"></span>`;let t=Ze(e.streamUrl);return`
    <a class="ranking-stream" href="${P(e.streamUrl)}" target="_blank" rel="noreferrer" aria-label="Open ${P(e.name)} ${P(t.label)} stream">
      <span class="service-icon service-icon-${P(t.key)}" aria-hidden="true">
        ${t.icon?`<img src="${t.icon}" alt="" loading="lazy" />`:P(t.short)}
      </span>
    </a>
  `},on=()=>`
  <aside class="rankings-sidebar" aria-label="Human rankings">
    <div class="rankings-header">
      ...
      <small>Top ${I().length}</small>
    </div>
    <div class="rankings-list">
      ${I().map(e=>`
            <div class="ranking-row ${h===e.slug?`is-selected`:``}" data-player-filter="${P(e.slug)}" role="button" tabindex="0" aria-pressed="${h===e.slug}">
...
`,sn=e=>e?`
  ...
  `:``,cn=()=>`
  <section class="player-card-region ${_e?``:`with-card-intro`}" aria-label="Top Human player cards">
    <div class="player-card-strip">
      ${I().map((e,t)=>{let n=[`is-gold`,`is-silver`,`is-bronze`][t]||``,r=t*760,i=t<3?r+520:0,a=Pe(e);return`
              ...
              <div class="player-card-body">
                <div class="player-card-summary">
                  ...
                  <h2 class="player-card-name">
                    <span aria-hidden="true">${P(e.flag)}</span>
                    <span>${P(e.name)}</span>
                  </h2>
                  ${a?`<p class="player-card-earnings">${P(a.earningsLabel)}</p>`:``}
                  ${it(e)}
                </div>
                <div class="player-card-actions">
                  ${nt(e)}
                  ${rt(e)}
                  <a class="download-pack" href="${P(e.replayPackUrl||`replay-packs/${e.slug}-2026-human-wins.zip`)}" download aria-label="Download ${P(e.name)} 2026 Human wins replay pack">
                    ${Qe()}
...
`,dn=()=>{let e=U().find(e=>e.mapImage?.localPath&&L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`;return`
    <section class="renderer-page" aria-label="Renderer test">
      ...
      <div class="renderer-layout">
        <section class="renderer-stage" aria-label="Renderer preview">
          ${e?.mapImage?.localPath?`<img src="${P(F(e.mapImage.localPath))}" alt="${P(e.map)} map thumbnail" loading="lazy" />`:`<div class="renderer-placeholder">WC3</div>`}
          <div class="renderer-stage-overlay">
            <span>Renderer bridge pending</span>
            <strong>${P(e?.map||`Waiting for replay data`)}</strong>
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                <h2>${P(e.sourceFlag)} ${P(e.sourceName)} on ${P(e.map)}</h2>
                <dl>
                  <div><dt>Date</dt><dd>${P(r)}</dd></div>
                  <div><dt>Replay ID</dt><dd>${P(e.id)}</dd></div>
                  <div><dt>Local file</dt><dd><code>${P(e.localPath)}</code></dd></div>
                  <div><dt>Players</dt><dd>${J(e,n)}</dd></div>
                </dl>
                <div class="renderer-actions">
                  ${H(`Analysis coming soon`)}
                  <a href="${P(e.downloadUrl)}" target="_blank" rel="noreferrer">Download replay</a>
                </div>
  ...
  `},fn=()=>{try{let e=JSON.parse(localStorage.getItem(`w3AssetsIndex`)||`[]`),t=Array.isArray(e)?e.length:0,n=Array.isArray(e)?e.filter(e=>String(e?.p||``).toLowerCase().endsWith(`.mpq`)).length:0,r=Array.isArray(e)?e.filter(e=>/\.(w3x|w3m)$/i.test(String(e?.p||``))).length:0,i=Array.isArray(e)?e.reduce((e,t)=>e+Number(t?.s||0),0):0,a=localStorage.getItem(`wc3-warsmash-staged-edition`)||`legacy`;return{ready:localStorage.getItem(`w3AssetsReady`)===`1`&&t>0&&a===k,edition:a,fileCount:t,mpqCount:n,mapCount:r,totalMb:(i/1048576).toFixed(1)}}catch{return{ready:!1,edition:`legacy`,fileCount:0,mpqCount:0,mapCount:0,totalMb:`0.0`}}},pn=()=>ge===`windows`?{platform:`Windows`,primary:`C:\\Program Files\\Warcraft III`,secondary:`C:\\Program Files (x86)\\Warcraft III`,pickerHint:`In the folder picker, paste the path into the address bar if it does not open there automatically.`}:{platform:`macOS`,primary:`/Applications/Warcraft III (Legacy)`,secondary:`/Applications/Warcraft III`,pickerHint:`In the folder picker, press Cmd+Shift+G, paste the path, then choose the Warcraft III folder.`},Z={reforged:`2.0.4.23745`,legacy:`1.29.2.9232`},mn=[{id:`legacy`,label:`Legacy MPQ`,build:Z.legacy,status:`Playable now`,note:`Current Warsmash web path mounts classic MPQs and loose maps from this install.`},{id:`reforged`,label:`Reforged`,build:Z.reforged,status:`Experimental CASC test`,note:`Latest Reforged uses CASC assets. This app now has an experimental browser-worker CASC mount path backed by the private sandbox.`}],hn=async(e,t)=>{let n=e;for(let e of t)n=await n.getDirectoryHandle(e,{create:!0});return n},gn=async()=>{let e=await navigator.storage.getDirectory();for(let t of[`w3`,`extracted`])try{await e.removeEntry(t,{recursive:!0})}catch{}},_n=async(e=k)=>{if(!j.running){j={running:!0,done:0,total:0,bytes:0,totalBytes:0,currentName:`Reading local install...`,error:``},$();try{let t=await fetch(`/wc3-local-assets/manifest?edition=${encodeURIComponent(e)}`,{cache:`no-store`});if(!t.ok){let e=await t.json().catch(()=>({}));throw Error(e.error||`No local MPQ-style Warcraft III install was detected.`)}let n=await t.json(),r=Array.isArray(n.files)?n.files:[];if(!r.length)throw Error(`Detected install did not include MPQ/map files Warsmash can use.`);j={running:!0,done:0,total:r.length,bytes:0,totalBytes:Number(n.totalBytes||0),currentName:`Preparing ${n.root||`local install`}`,error:``},$(),await gn();let i=await(await navigator.storage.getDirectory()).getDirectoryHandle(`w3`,{create:!0}),a=[],o=0,s=0;for(let t of r){let c=String(t.path||``);if(!c)continue;let l=await fetch(`/wc3-local-assets/file?edition=${encodeURIComponent(e)}&path=${encodeURIComponent(c)}`);if(!l.ok)throw Error(`Failed to read ${c}`);let u=await l.blob(),d=c.split(`/`).filter(Boolean),f=await(await(await hn(i,d.slice(0,-1))).getFileHandle(d[d.length-1],{create:!0})).createWritable();await f.write(u),await f.close(),o+=1,s+=u.size,a.push({p:c,s:u.size}),j={running:!0,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:c,error:``},(o===r.length||o%3==0)&&$()}localStorage.setItem(`w3AssetsReady`,`1`),localStorage.setItem(`w3AssetsCount`,String(a.length)),localStorage.setItem(`w3AssetsIndex`,JSON.stringify(a)),localStorage.setItem(`w3AssetsSource`,n.root||`detected local install`),localStorage.setItem(`wc3-warsmash-staged-edition`,e),localStorage.setItem(`wc3-warsmash-selected-edition`,e),j={running:!1,done:o,total:r.length,bytes:s,totalBytes:Number(n.totalBytes||s),currentName:`Ready`,error:``},$()}catch(e){j={...j,running:!1,error:e instanceof Error?e.message:String(e)},$()}}},vn=async()=>{if(!A.preparing){A={...A,preparing:!0,error:``},$();try{let e=await fetch(`/wc3-reforged-sandbox/prepare`,{cache:`no-store`}),t=await e.json().catch(()=>({}));if(!e.ok)throw Error(t.error||`Failed to prepare Reforged sandbox.`);A={checked:!0,preparing:!1,ready:!0,build:t.build||``,fileCount:Number(t.totals?.configFiles||0)+Number(t.totals?.indexFiles||0)+Number(t.totals?.dataFiles||0),totalGb:(Number(t.totals?.bytes||0)/1073741824).toFixed(2),error:``}}catch(e){A={...A,checked:!0,preparing:!1,ready:!1,error:e instanceof Error?e.message:String(e)}}$()}},yn=()=>{let e=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],t=e?L(e.sourceSlug,e.id):null,n=t?.status===`parsed`?t.players:[],r=e?.createdAt?Ee.format(new Date(e.createdAt)):`Unknown date`,i=e?`${e.sourceSlug}:${e.id}`:``,a=`/play/?menu=1&assets=${encodeURIComponent(k)}&replay=${encodeURIComponent(i)}&autostart=1`,o=`/assets/`,s=fn(),c=pn(),l=mn.find(e=>e.id===k)||mn[0],u=l.id===`legacy`,d=u?s.ready:A.ready,f=()=>k===`reforged`?`
      <div class="wc3-reforged-sandbox ${A.ready?`is-ready`:``} ${A.error?`is-error`:``}">
        <div>
          <span>Private Reforged sandbox</span>
          <strong>${A.ready?`Ready · ${P(A.build||Z.reforged)}`:`Not prepared yet`}</strong>
          <small>${A.ready?`${P(B(A.fileCount))} CASC files indexed, ${P(A.totalGb)} GB referenced read-only.`:`Creates .wc3-assets/reforged/manifest.json and indexes your Reforged CASC data without modifying the install.`}</small>
          ${A.error?`<small>${P(A.error)}</small>`:``}
        </div>
    ...
    `:``,ee=j.totalBytes?Math.min(100,j.bytes/j.totalBytes*100):0,p=j.running?`${j.done}/${j.total} ${(j.bytes/1048576).toFixed(1)} / ${(j.totalBytes/1048576).toFixed(1)} MB`:``;return`
      ...
      <div class="wc3-viewer-offline">
        ...
        ${j.running||j.error?`
              <div class="wc3-auto-stage-status ${j.error?`is-error`:``}">
                ${j.running?`<div class="wc3-auto-stage-bar"><span style="width: ${ee.toFixed(1)}%;"></span></div><strong>${P(p)}</strong><small>${P(j.currentName)}</small>`:`<strong>Auto staging failed</strong><small>${P(j.error)}</small>`}
              </div>
            `:``}
        <div class="wc3-install-hint">
          <span>Suggested ${P(c.platform)} folder</span>
          <code>${P(l.id===`legacy`?c.primary:c.secondary)}</code>
          <small>${P(l.note)}</small>
    ...
    `}
        ...
        <aside class="renderer-test-card">
          ...
          ${e?`
                ...
                <dl>
                  <div><dt>Date</dt><dd>${P(r)}</dd></div>
                  <div><dt>Replay ID</dt><dd>${P(e.id)}</dd></div>
                  <div><dt>Local replay</dt><dd><code>${P(e.localPath)}</code></dd></div>
                  <div><dt>Selected version</dt><dd>${P(l.label)} · ${P(l.status)}</dd></div>
                  ...
                  <div><dt>Suggested folder</dt><dd><code>${P(l.id===`legacy`?c.primary:c.secondary)}</code></dd></div>
                  <div><dt>Players</dt><dd>${J(e,n)}</dd></div>
                </dl>
                <div class="renderer-actions">
                  ${H(`Analysis coming soon`)}
                  ${d?`<button class="warsmash-start-button" type="button" data-warsmash-watch="${P(i)}">${et()}<span>${u?`Start Engine Test`:`Start Reforged Engine Test`}</span></button>`:`<button type="button" data-warsmash-auto-stage ${j.running||!u?`disabled`:``}>${j.running?`Staging Detected Install...`:u?`Auto Stage Detected Install`:`Reforged Support Pending`}</button>`}
      ...
      <section class="renderer-research">
        ...
        <article>
          <h2>Patch Support</h2>
          <p>Your Reforged install is detected as <code>${Z.reforged}</code>. The worker now has an experimental CASC mount path, while legacy remains available through classic MPQs from <code>${Z.legacy}</code>.</p>
        </article>
        <article>
...
`,bn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-object-row">
              <span>${z(e)}${P(Ie(R(e)).replace(` (${e})`,``))}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,xn=(e,t=`No parsed data yet`)=>e.length?e.map(([e,t])=>`
            <div class="statistics-text-row">
              <span>${P(e)}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>${P(t)}</strong></div>`,Sn=()=>{let e=ht(),t=e.humanGames||1,n=q(e.boughtItems,10),r=q(e.foundItems,8);return`
    <section class="statistics-page" aria-label="Replay statistics">
      ...
      <div class="statistics-metrics" aria-label="Replay data summary">
        ${Q(`Parsed replays`,B(e.parsedReplays),`Allowed map pool only`)}
        ${Q(`Human player-games`,B(e.humanGames),`One Human player in most rows`)}
        ${Q(`Average Human APM`,B(e.averageApm),`Parsed APM samples`)}
        ${Q(`Item actions`,B(e.totalItemActions),`Buys, uses, and item commands`)}
      </div>
      ...
      <section class="statistics-panel statistics-hero-panel">
        <div class="statistics-panel-header">
          ...
          <span>${P(B(e.humanGames))} Human games</span>
        </div>
        <div class="statistics-hero-grid">
          ${G.map(n=>{let r=e.heroPicks.get(n)||0,i=e.firstHeroPicks.get(n)||0,a=e.humanGames?r/t*100:0,o=e.humanGames?i/t*100:0;return`
                <article class="statistics-hero-card">
                  ...
                  <dl>
                    <div><dt>Picked</dt><dd>${P(B(r))}</dd></div>
                    <div><dt>Pick rate</dt><dd>${P(qe(a))}</dd></div>
                    <div><dt>First hero</dt><dd>${P(B(i))}</dd></div>
                    <div><dt>First rate</dt><dd>${P(qe(o))}</dd></div>
                  </dl>
                </article>
              `}).join(``)}
        </div>
      ...
      <div class="statistics-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(Be(e.knownBoughtGold))}</span>
          </div>
          <div class="statistics-list">
            ${bn(n,`No shop purchases found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(B([...e.foundItems.values()].reduce((e,t)=>e+t,0)))}</span>
          </div>
          <div class="statistics-list">
            ${bn(r,`No non-shop item events found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${bn(q(e.unitCounts,10),`No unit stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${bn(q(e.buildingCounts,10),`No building stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${bn(q(e.upgradeCounts,10),`No upgrade stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${xn(q(e.mapCounts,8),`No map stats found`)}
          </div>
        </section>
        ...
        <section class="statistics-panel">
          ...
          <div class="statistics-list">
            ${xn(q(e.matchupCounts,8),`No matchup stats found`)}
          </div>
        </section>
      </div>
    </section>
  `},Cn=()=>{let e=U(),t=e.map(e=>({replay:e,analysis:L(e.sourceSlug,e.id)})).filter(e=>e.analysis?.status===`parsed`),n=new Map,r=new Map,i=new Map,a=0,o=0,s=0,c=0,l=0;return t.forEach(({replay:e,analysis:t})=>{K(n,t.version||e.version||`Unknown`),K(r,e.map||t.map?.file||`Unknown map`),a+=t.chat?.length||0,s+=t.chat?.length||0,(t.players||[]).forEach(e=>{o+=1,Number.isFinite(e.apm)&&(c+=e.apm,l+=1),Object.entries(e.actions||{}).forEach(([e,t])=>{K(i,e,t||0)}),s+=(e.heroes||[]).length,s+=(e.order?.buildings||[]).length,s+=(e.order?.units||[]).length,s+=(e.order?.upgrades||[]).length,s+=(e.order?.items||[]).length})}),{totalReplays:e.length,parsedReplays:t.length,parsedPlayers:o,eventCount:s,chatMessages:a,averageApm:l?Math.round(c/l):0,versionCounts:n,mapCounts:r,actionCounts:i}},wn=e=>e.length?e.map(([e,t])=>`
            <div class="w3gjs-action-row">
              <span>${P(e)}</span>
              <strong>${P(B(t))}</strong>
            </div>
          `).join(``):`<div class="analysis-empty"><strong>No action data parsed yet</strong></div>`,Tn=e=>e.length?e.map(e=>`
          ...
          `).join(``):`<div class="analysis-empty"><strong>No timeline events parsed yet</strong></div>`,En=()=>{let e=Cn(),t=U().find(e=>String(e.id)===`137397`)||U().find(e=>L(e.sourceSlug,e.id)?.status===`parsed`)||U()[0],n=t?L(t.sourceSlug,t.id):null,r=n?.status===`parsed`?n.players:[],i=Lt(n),a=i.slice(0,12),o=i.filter(e=>(e.ms||0)>=18e4).slice(0,12),s=q(e.actionCounts,12),c=q(e.versionCounts,4),l=q(e.mapCounts,6);return`
...
const result = await parser.parse("replay.w3g");</code></pre>
      ...
      <div class="statistics-metrics w3gjs-metrics" aria-label="w3gjs parsed data summary">
        ${Q(`Replay files`,B(e.totalReplays),`From the local replay cache`)}
        ${Q(`Parsed replays`,B(e.parsedReplays),`Built from w3gjs output`)}
        ${Q(`Parsed players`,B(e.parsedPlayers),`Names, races, teams, APM`)}
        ${Q(`Timeline events`,B(e.eventCount),`Builds, units, items, heroes, chat`)}
      </div>
      ...
      <section class="w3gjs-showcase">
        <div class="w3gjs-replay-card">
          ...
          ${t?`
                <h2>${P(t.sourceFlag)} ${P(t.sourceName)} on ${P(t.map)}</h2>
                <div class="analysis-modal-matchup">${J(t,r)}</div>
                <dl>
                  <div><dt>Game</dt><dd>${P(n?.gameName||`Unknown`)}</dd></div>
                  <div><dt>Patch</dt><dd>${P(n?.version||t.version||`Unknown`)}</dd></div>
                  <div><dt>Build</dt><dd>${P(n?.buildNumber||`Unknown`)}</dd></div>
                  <div><dt>Duration</dt><dd>${P(n?.durationLabel||t.durationLabel||`Unknown`)}</dd></div>
                  <div><dt>Map file</dt><dd><code>${P(n?.map?.file||t.map||`Unknown`)}</code></dd></div>
                  <div><dt>Local file</dt><dd><code>${P(t.localPath||n?.sourceFile||``)}</code></dd></div>
                </dl>
                ${H(`Analysis coming soon`)}
              `:`<p>No replay cache loaded yet.</p>`}
        </div>
      ...
      <div class="w3gjs-grid">
        <section class="statistics-panel">
          <div class="statistics-panel-header">
            ...
            <span>${P(B(e.averageApm))} avg APM</span>
          </div>
          ${Ct({...n,players:r})}
        </section>
...
`,jn=()=>{let e=me?.players||[];return`
    <section class="money-page" aria-label="Player earnings">
      ...
      <div class="money-table" role="table" aria-label="Human earnings leaderboard">
        ...
        ${e.length?e.map(e=>`
                    <div class="money-row" role="row">
                      ...
                      <span>${P(e.flag||``)} ${P(e.country||`Unknown`)}</span>
                      <span>${P(e.race||`Human`)}</span>
                      <strong class="money-amount">${P(e.earningsLabel)}</strong>
                    </div>
      ...
      <p class="money-note">${P(me?.scope||``)}</p>
    </section>
  `},Mn=()=>{let e=document.querySelector(`.replay-pager`);e&&(e.innerHTML=Xt(),zn(),Vn())},Nn=()=>{y=1},Pn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{let t=e.dataset.playerFilter===h;e.classList.toggle(`is-selected`,t),e.setAttribute(`aria-pressed`,String(t)),e.closest(`.player-card`)?.classList.toggle(`is-selected`,t),e.classList.contains(`ranking-row`)&&e.classList.toggle(`is-selected`,t)})},Fn=e=>{h=h===e?`all`:e,Nn(),Pn(),Mn()},In=(e=null)=>{document.querySelectorAll(`.dropdown-filter.is-open`).forEach(t=>{t!==e&&(t.classList.remove(`is-open`),t.querySelector(`.filter-dropdown-button`)?.setAttribute(`aria-expanded`,`false`))})},Ln=()=>{let e=document.querySelector(`.filter-popover`);e&&(e.classList.remove(`is-open`),e.querySelector(`.filters-toggle`)?.setAttribute(`aria-expanded`,`false`),In())},Rn=()=>{let e=document.querySelector(`.filters`);e&&(e.outerHTML=rn(),Un())},zn=()=>{document.querySelectorAll(`.pagination-button[data-page]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.page);!Number.isFinite(t)||e.disabled||t===y||(y=t,Mn(),document.querySelector(`.replay-column`)?.scrollIntoView({block:`start`,behavior:`smooth`}))})})},Bn=()=>{b=null,T=`overview`,$()},Vn=()=>{document.querySelectorAll(`.replay-row[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`a, button`)||(b=e.dataset.replayDetail,T=`overview`,$())}),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||t.target.closest(`a, button`)||(t.preventDefault(),b=e.dataset.replayDetail,T=`overview`,$())})}),document.querySelectorAll(`.replay-analysis-button[data-replay-detail]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation(),b=e.dataset.replayDetail,T=`overview`,$()})}),document.querySelectorAll(`[data-analysis-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{T=e.dataset.analysisTab||`overview`,$()})}),document.querySelectorAll(`[data-modal-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Bn()})}),document.querySelectorAll(`[data-theater-close]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&Rt()})}),document.querySelector(`[data-theater-play]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Bt()}),document.querySelector(`[data-theater-reset]`)?.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),C=0,S=!1,w&&=(clearInterval(w),null),$()})},Hn=()=>{document.querySelectorAll(`[data-player-filter]`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.ranking-stream`)||Fn(e.dataset.playerFilter)}),e.addEventListener(`keydown`,t=>{e.tagName!==`BUTTON`&&(t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Fn(e.dataset.playerFilter)))})})},Un=()=>{let e=document.querySelector(`#replay-search`);if(!e)return;e.addEventListener(`input`,e=>{he=e.target.value,Nn(),Mn()});let t=document.querySelector(`.filter-popover`),n=document.querySelector(`.filters-toggle`);n?.addEventListener(`click`,e=>{e.stopPropagation();let r=t?.classList.toggle(`is-open`)||!1;n.setAttribute(`aria-expanded`,String(r)),r||In()}),t?.addEventListener(`click`,e=>{e.stopPropagation()}),document.querySelectorAll(`.dropdown-filter`).forEach(e=>{let t=e.querySelector(`.filter-dropdown-button`),n=e.dataset.filter;t?.addEventListener(`click`,n=>{n.stopPropagation();let r=e.classList.toggle(`is-open`);In(e),t.setAttribute(`aria-expanded`,String(r))}),e.querySelectorAll(`.filter-dropdown-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.value||`all`;n===`map-filter`&&(g=t),n===`hero-filter`&&(_=t),n===`matchup-filter`&&(v=t),Nn(),In(),Rn(),Mn()})})})},Wn=()=>{let e=document.querySelector(`.player-card-strip`);if(!e)return;let t=!1,n=0,r=0,i=!1;e.addEventListener(`pointerdown`,a=>{if(a.button===0){if(a.target.closest(`a, button, input, select, textarea, label`)){t=!1,i=!1;return}t=!0,i=!1,n=a.clientX,r=e.scrollLeft,e.classList.add(`is-dragging`),e.setPointerCapture(a.pointerId)}}),e.addEventListener(`pointermove`,a=>{if(!t)return;let o=a.clientX-n;Math.abs(o)>4&&(i=!0),e.scrollLeft=r-o});let a=n=>{t&&(t=!1,e.classList.remove(`is-dragging`),e.hasPointerCapture(n.pointerId)&&e.releasePointerCapture(n.pointerId))};e.addEventListener(`pointerup`,a),e.addEventListener(`pointercancel`,a),e.addEventListener(`click`,e=>{i&&=(e.preventDefault(),e.stopPropagation(),!1)},!0)},Gn=e=>`
  <ul class="app-info-link-list">
    ${e.map(e=>`
          <li>
            <a href="${P(e.href)}" target="_blank" rel="noreferrer">${P(e.label)}</a>
            <span>${P(e.description)}</span>
          </li>
        `).join(``)}
  </ul>
`,Kn=e=>`
...
`,qn=()=>D===`analyze`?`
  ...
  <aside class="app-info-panel ${E?`is-open`:``}" id="app-info-panel" aria-hidden="${!E}" aria-label="App info">
    ...
    <div class="app-info-tabs" role="tablist" aria-label="App info sections">
      ${Se.map(e=>`
            <button class="${e.id===D?`is-active`:``}" type="button" role="tab" aria-selected="${e.id===D}" data-info-tab="${P(e.id)}">
              ${P(e.label)}
            </button>
          `).join(``)}
    </div>
...
`,Xn=()=>{document.querySelector(`[data-info-toggle]`)?.addEventListener(`click`,()=>{E=!E,$()}),document.querySelector(`[data-info-close]`)?.addEventListener(`click`,()=>{E=!1,$()}),document.querySelectorAll(`[data-info-tab]`).forEach(e=>{e.addEventListener(`click`,()=>{D=e.dataset.infoTab||`sources`,$()})})},Zn=()=>{document.querySelector(`[data-warsmash-embed]`)?.addEventListener(`click`,()=>{O=!0,$()}),document.querySelectorAll(`[data-warsmash-watch]`).forEach(e=>{e.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),b=null,T=`overview`,O=!0,$()})}),document.querySelectorAll(`[data-warsmash-edition]`).forEach(e=>{e.addEventListener(`click`,()=>{k=e.dataset.warsmashEdition||`legacy`,localStorage.setItem(`wc3-warsmash-selected-edition`,k),O=!1,$()})}),document.querySelectorAll(`[data-warsmash-auto-stage]`).forEach(e=>{e.addEventListener(`click`,()=>{_n(k)})}),document.querySelector(`[data-reforged-prepare]`)?.addEventListener(`click`,()=>{vn()})},Qn=()=>{ge=`${navigator.userAgent||``} ${navigator.platform||``}`.toLowerCase().includes(`win`)?`windows`:`mac`},$=()=>{let e=Me(),t=e.type===`all`;document.querySelector(`#app`).innerHTML=`
    ...
    <aside class="menu-panel" id="site-menu" aria-hidden="true">
      <div class="menu-panel-inner">
        <nav class="menu-nav" aria-label="Menu">
          ${Zt(e.type===`all`?`is-active`:``)}
          ${Qt(e.type===`builds`?`is-active`:``)}
          ${$t(e.type===`player-earnings`?`is-active`:``)}
          ${en(e.type===`statistics`?`is-active`:``)}
          ${tn(e.type===`resources`?`is-active`:``)}
        </nav>
        
      </div>
    </aside>

    <main class="page-shell ${t?`page-shell-home`:`page-shell-subpage`}">
      ${e.type===`builds`?un():e.type===`player-earnings`?jn():e.type===`statistics`?Sn():e.type===`renderer`?dn():e.type===`wc3-replay-viewer`?yn():e.type===`w3gjs`?En():e.type===`resources`?An():ln()}
    </main>
    ${Yn()}
    ...
    ${Vt()}
    ${Jn()}
  `;let n=document.querySelector(`.site-header`),r=document.querySelector(`.menu-panel`),i=document.querySelector(`.menu-backdrop`),a=document.querySelector(`.menu-button`),o=()=>{r.classList.remove(`is-open`),i?.classList.remove(`is-open`),r.setAttribute(`aria-hidden`,`true`),a.setAttribute(`aria-expanded`,`false`),a.setAttribute(`aria-label`,`Open menu`)};a.addEventListener(`click`,()=>{let e=r.classList.toggle(`is-open`);i?.classList.toggle(`is-open`,e),r.setAttribute(`aria-hidden`,String(!e)),a.setAttribute(`aria-expanded`,String(e)),a.setAttribute(`aria-label`,e?`Close menu`:`Open menu`)}),i?.addEventListener(`click`,o),document.querySelectorAll(`[data-page-link]`).forEach(e=>{e.addEventListener(`click`,o)}),document.documentElement.style.setProperty(`--header-height`,`${n.offsetHeight}px`),$n(),Un(),zn(),Vn(),Hn(),Wn(),Xn(),Zn(),document.querySelector(`.player-card-region.with-card-intro`)&&!ve&&(ve=window.setTimeout(()=>{_e=!0,document.querySelector(`.player-card-region.with-card-intro`)?.classList.remove(`with-card-intro`)},5200)),document.querySelector(`.brand.with-logo-intro`)&&!be&&(be=window.setTimeout(()=>{ye=!0,document.querySelector(`.brand.with-logo-intro`)?.classList.remove(`with-logo-intro`)},1400))};window.addEventListener(`keydown`,e=>{if(e.key!==`Escape`)return;if(x){Rt();return}if(E){E=!1,$();return}if(b){Bn();return}Ln();let t=document.querySelector(`.menu-panel`),n=document.querySelector(`.menu-backdrop`),r=document.querySelector(`.menu-button`);!t||!r||(t.classList.remove(`is-open`),n?.classList.remove(`is-open`),t.setAttribute(`aria-hidden`,`true`),r.setAttribute(`aria-expanded`,`false`),r.setAttribute(`aria-label`,`Open menu`))}),window.addEventListener(`click`,()=>{Ln()}),window.addEventListener(`hashchange`,()=>{O=!1,w&&=(clearInterval(w),null),x=null,S=!1,C=0,h=`all`,g=`all`,_=`all`,v=`all`,Nn(),$()}),window.addEventListener(`message`,e=>{if(e.origin!==window.location.origin)return;let t=e.data;if(!t||typeof t!=`object`||t.kind!==`hvtw-replay-request`)return;let n=t.replayKey||``;Fe(n)&&(b=null,T=`overview`,zt(n))}),window.addEventListener(`resize`,()=>{let e=document.querySelector(`.site-header`);e&&document.documentElement.style.setProperty(`--header-height`,`${e.offsetHeight}px`)}),window.addEventListener(`focus`,()=>{Me().type===`wc3-replay-viewer`&&!O&&$()}),document.addEventListener(`visibilitychange`,()=>{document.visibilityState===`visible`&&Me().type===`wc3-replay-viewer`&&!O&&$()});var $n=()=>{document.body.classList.toggle(`has-scrolled-header`,window.scrollY>32)};window.addEventListener(`scroll`,$n,{passive:!0});var er=async e=>{let t=await fetch(e,{cache:`no-store`});if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()};(async()=>{Qn();let e=Promise.resolve(null),[t,n,r,i,a,o]=await Promise.allSettled([er(`replays.json`),er(`rankings.json`),er(`replay-analysis.json`),er(`map-drop-tables.json`),er(`liquipedia-human-earnings.json`),e]);t.status===`fulfilled`?le=t.value:ue=t.reason?.message||`Replay cache unavailable`,n.status===`fulfilled`&&(pe=n.value),r.status===`fulfilled`&&(de=r.value),i.status===`fulfilled`&&(fe=i.value),a.status===`fulfilled`&&(me=a.value),$()})();